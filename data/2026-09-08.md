...
TL;DR: 该论文提出用稀疏自编码器（SAE）的可解释潜变量来预测视觉-语言模型（如 CLIP）的失败，并在预测性能与概念级解释性之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在高风险场景中部署时需要可靠的失败预测，以便进行风险控制和人工干预；现有基于置信度或辅助分类器的方法虽有效，但解释性不足，难以说明模型为何可能失败。

Method: 作者将失败预测建模为基于 SAE 稀疏潜激活的分类任务，并提出三阶段的 failure-aware 训练流程，使 SAE 学到的潜在方向既保持可解释性，又对失败预测更有信息量；同时利用 SAE 潜方向进行概念级分析，并探索运行时失败恢复。

Result: 实验表明，该框架在失败预测上优于所评估的基线方法；进一步分析显示，failure-aware 训练促使 SAE 潜方向捕获更多类别特异性概念，并揭示模型失败时表示会从类别相关概念转向更模糊或风格相关的概念。

Conclusion: SAE 可作为视觉-语言模型失败预测的可解释工具，不仅提升预测效果，还能提供模型失败机制的概念级洞察，并具备支持运行时失败恢复的潜力。
...
TL;DR: 该摘要提出个性化安全问题：VLM在缺少用户具体背景时仍倾向直接回答，导致高风险场景下安全性不足。作者构建MPS-Bench评测集，发现前沿模型普遍不善于主动请求缺失上下文，并提出PRISM输入监测器来判断何时应延迟或拒绝直接回答。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在高风险应用中常基于图像与文本给出看似合理的通用回答，但用户的身体、情绪或情境背景可能未知，导致同一回答对特定用户不安全。因此需要系统评估并提升多模态系统的个性化安全能力。

Method: 作者构建包含5,181个场景、584张真实图像、覆盖12个高风险领域且配有隐藏用户画像的MPS-Bench；评估8个前沿VLM；进一步分析多模态融合机制，提出“视觉主导”假设，并通过因果干预验证视觉信息如何早期进入文本表征并抑制文本风险信号；基于该机制设计轻量级输入监测器PRISM，采用双向跨模态调制预测查询是否需要延迟处理。

Result: 8个前沿VLM在86%到99%的情况下直接回答而非询问缺失上下文，个性化安全评分均不超过2.6/5。机制分析显示视觉情感信息会在早期层注入文本流，并通过改变后的文本表征影响最终决策。PRISM达到0.978 AUC，并在所有测试模型上严格优于原有安全-效用Pareto前沿。

Conclusion: VLM的个性化安全失败主要源于多模态融合中的视觉主导和早期表征污染，单靠后期内部修复并不可靠。面向输入阶段的轻量监测与延迟决策机制更有效，PRISM展示了在安全性与实用性之间取得更优权衡的潜力。
...
TL;DR: RA-GRPO 是一种面向扩散生成模型的强化学习偏好对齐框架，通过在优化中引入反向反思机制，提升文本到图像/视频生成的语义一致性、视觉真实感与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于策略梯度的扩散模型偏好对齐方法探索效率较低，容易陷入局部最优，并可能导致奖励黑客、语义忠实度下降和视觉质量退化。因此需要一种更稳定、更高效的后训练对齐方法。

Method: 论文提出 Reflection-Aware GRPO。其核心包括两部分：一是 Diffusion Reflection，通过使用弱估计器反向近似扩散过程，校正中间采样轨迹，将潜变量引导到更接近真实数据流形的高概率区域；二是 Counterfactual Path Synthesis，将这些被校正的轨迹隐式蒸馏到策略模型中，使模型在不增加推理开销的情况下吸收搜索式探索带来的收益。

Result: 在文本到图像和文本到视频生成模型上的实验表明，RA-GRPO 显著优于已有方法，尤其在缓解奖励黑客、提升泛化能力、保持语义一致性和视觉真实感方面表现更好。

Conclusion: RA-GRPO 为扩散生成模型的稳定偏好对齐提供了一种有效方向。该方法与具体架构无关，可无缝集成到标准生成流程中，并能在不增加推理成本的前提下提升对齐质量。
...
TL;DR: 提出 DM-Align：将视频生成模型的蒸馏与人类偏好对齐统一到单阶段 Distribution Matching 优化中，以更低计算开销提升质量与偏好一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成偏好对齐多依赖强化学习，计算成本高；RL 与蒸馏通常被割裂为两阶段流程：先 RL 再蒸馏代价过大，先蒸馏再 RL 又容易导致模型崩塌。因此需要一种同时兼顾蒸馏质量、偏好对齐和训练稳定性的统一方法。

Method: 在标准 Distribution Matching 框架中，蒸馏梯度用于缩小真实模型与伪模型分布差距，提升清晰度和保真度。DM-Align 在此基础上引入一个互补的偏好引导梯度，借鉴 DPO 和 GRPO，通过偏好样本对或组内探索构造分布差距，并将其转化为直接优化方向；最终将蒸馏梯度与偏好梯度协同组合，避免传统 RL 中多步奖励评估及复杂 ODE-SDE 转换。

Result: 在多个基础视频生成模型上的实验显示，该方法能稳定提升蒸馏质量和人类偏好对齐效果，并优于单独使用蒸馏/对齐变体以及顺序式两阶段训练流水线。

Conclusion: DM-Align 表明，基于分布匹配的单阶段样本引导优化可以有效替代高开销、易不稳定的 RL 对齐流程，为视频生成模型的高效蒸馏与偏好对齐提供了更稳健的方案。
...
TL;DR: 本文提出 CARNIVAL：利用模拟数据、前向成像模型产生的成对增强视图，以及仿真管线提供的蛋白位置与类别等特权信息，训练用于拥挤冷冻电子断层扫描体数据中蛋白注释的自监督模型。


<details>
  <summary>Details</summary>
Motivation: 真实 cryo-ET 数据采集受有限倾角和测量算子严重退化影响，且蛋白注释标注成本高；因此需要有效利用模拟数据训练能迁移到真实断层图上的蛋白分类与检测模型。

Method: 方法首先基于前向模型中的退化过程生成同一场景的领域特定成对视图，并将其用于 LeJEPA 自监督框架中的不变性目标；其次利用模拟管线提供的蛋白位置和身份信息来设计模型架构与损失函数，使语义特征在稠密特征体中定位到蛋白所在位置。

Result: 在无需微调的情况下，CARNIVAL 在真实 tomogram 的分类和检测任务上进行评估，覆盖包含多种蛋白类型和两类断层图处理方式的基准数据集；结果优于使用对比学习目标、同样基于模拟数据但未利用前向模型成对视图和特权信息的现有先进模型。

Conclusion: 将物理成像退化建模与仿真特权信息融入自监督学习，可以显著提升模拟训练模型向真实 cryo-ET 蛋白注释任务的迁移能力。
...
TL;DR: 该研究检验单一语料库所得平均依存距离是否可代表语言属性，发现跨同语种树库的一致性仅中等或更低；依存长度最小化现象稳定存在，但跨语言排序不稳健。


<details>
  <summary>Details</summary>
Motivation: 以往研究常将单个语料库估计出的平均依存距离视为某种语言的稳定特征，但这一前提很少在独立编制的同语种语料库之间得到验证。作者希望评估树库选择是否会显著影响平均依存距离估计及跨语言比较结论。

Method: 研究使用 Universal Dependencies v2.18 中38组同语种树库配对，比较其平均依存距离估计。分析方法包括一致性相关系数、Bland-Altman 分析，以及包含12种预处理设定的多重规格分析，以检验结果对数据处理选择的稳健性。

Result: 跨树库一致性最多只能达到中等水平；替换同语种树库会导致近40%的语言两两排序发生反转。树库选择解释了约29%的组间方差，且这种差异明显超过树库内部抽样误差，并在12种预处理规格下持续存在。不过，所有树库均显示标准化依存距离比值低于1，支持依存长度最小化。

Conclusion: 平均依存距离更适合被理解为受语法、语域和标注因素共同影响的语料条件化复合指标，而非稳定的语言层面参数。依存长度最小化作为定性普遍现象具有稳健性，但基于平均依存距离的跨语言序位排名并不可靠。
...
TL;DR: LETHE 是一个基于 SuperCollider 的自指涉闭环生成声音系统，通过类 GAN 结构与随机扰动优化机制，在无外部数据和无监督条件下实现声音参数的自适应演化。


<details>
  <summary>Details</summary>
Motivation: 论文旨在探索自指涉电子声学音乐中的闭环生成机制：作曲者定义参数空间与系统结构，而具体声音结果交由系统内部动态演化产生。

Method: 系统使用由两个延迟线构成的 3×3 混合矩阵处理音频；九个矩阵系数与两个延迟时间由五特征线性判别器和类似单样本 REINFORCE 的随机扰动优化器共同更新。判别器将当前能量行为与初始化状态档案进行比较，并据此引导参数变化。系统支持循环、固定和实时声源的独立混合。

Result: 在固定与循环声源实验及消融对照中，主动生成器对参数演化是必要的；消融实验中 15 次会话的 Δc22 均为 0.000，表明无主动生成机制时相关参数不发生演化。

Conclusion: LETHE 展示了一种将声音结果委托给自适应闭环系统的创作框架，其贡献在于把类 GAN 形式语言转化为无外部训练数据的声音参数演化机制，并服务于自指涉电子声学音乐实践。
...
TL;DR: 本文提出并验证了一个分布式理论，用于解释大语言模型如何将外部候选证据整合进已形成的答案分布；结果显示整合行为主要由接收模型自身分布和控制策略决定，而非简单由证据源可信度决定。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越依赖检索、工具、其他智能体和用户提供的外部证据进行推理，模型如何在已有初步判断后吸收这些证据仍不清楚。作者希望揭示外部证据对模型决策分布的影响机制，并解释为何同一证据对不同模型可能产生不同效果。

Method: 作者构建了一个分布式理论框架，认为证据会根据接收者先验权重和候选证据倾斜程度移动模型的初始答案分布，并提出三个可检验预测。随后在一千多万次实验、十二个来自四个系列的大语言模型、八个任务领域上进行验证，并结合因果干预、表示解码和 J-lens 分解分析候选答案整合与验证状态在网络中的实现机制。

Result: 实验支持三项预测：接收模型原本更可能生成的候选答案更具说服力；模型更容易整合与自身典型错误一致的错误，而非来自其他来源的错误；同一证据可能提升较弱模型却损害较强模型。研究还发现，与接收者分布一致的错误比同等错误率的随机错误更严重地降低性能。即使模型内部验证候选无效，仍可能高度整合这些候选答案。

Conclusion: 论文认为，大语言模型的证据整合是一种接收者特异性的控制策略，作用于已有答案分布，并主要由模型自身属性决定，而不是证据源的标量可信度。候选答案整合在网络后期通过一系列结构化步骤实现，而验证相关表示虽可被解码，却对最终答案影响有限；语言化验证状态与候选整合状态可以完全分离。
...
TL;DR: 本文提出 MedProb：一种轻量级探测框架，利用冻结视觉语言模型表征进行多选医学视觉问答预测，显示许多 Med-VQA 能力已隐含在通用/小型 VLM 表征中，生成式提示评测可能低估了这些信号。


<details>
  <summary>Details</summary>
Motivation: 现有 Med-VQA 通常假设需要医学领域微调、大模型或复杂多智能体流程。作者希望检验该假设是否成立，并区分模型表征中实际包含的答案相关信息与自由文本生成评测所暴露出的性能之间的差异。

Method: MedProb 不进行自由文本生成，而是在冻结 VLM 表征之上训练轻量探测器，用于多选/多分类 Med-VQA 答案预测。实验覆盖 PATH-VQA、SLAKE、VQA-RAD，并比较提示方法、医学 VLM、智能体系统，以及 14 组匹配的通用与医学 VLM。另通过拒绝采样评分过程扩展到开放式生成。

Result: MedProb 比提示方法恢复更多答案相关信号，性能强于若干医学 VLM 与智能体系统；相较提示评测，它缩小了小模型与大模型的表观差距。医学适配在 14 组模型对中并未稳定提升线性可解码性。自由文本生成存在最高约 10 个百分点的答案位置偏差，而 MedProb 也存在位置偏差但受影响机制不同。

Conclusion: Med-VQA 不一定依赖医学微调、大规模模型或复杂生成式系统；冻结 VLM 表征中已包含可恢复的医学问答信号。基于探测的评测能更直接衡量表征能力，但主要结论限于多选/多分类场景，开放式生成扩展仍需进一步验证。
...
TL;DR: 本文提出一项早期预测任务：根据志愿心理健康危机咨询师职业早期的对话适应行为，预测其未来是否会提升引导对话走向积极结果的能力。


<details>
  <summary>Details</summary>
Motivation: 心理健康咨询高度依赖会话技能，但志愿咨询师往往缺乏充分督导和结构化反馈。若能早期识别哪些咨询师未来更可能进步、哪些可能停滞，就能更有效地分配培训与支持资源。

Method: 作者基于“咨询师会在某些对话时刻遇到困难，且其再次遇到类似时刻时的应对变化能反映未来成长潜力”的洞见，设计方法来识别咨询师早期难以处理的对话情境，追踪其在后续相似情境中的回应适应，并学习哪些早期适应模式能够预测数月乃至数年后的能力提升。

Result: 尽管长期未来表现预测具有挑战性，所提出的“咨询师适应性”方法优于直接从对话转录文本中学习的基线模型。

Conclusion: 咨询师早期面对困难对话时刻的适应轨迹，是预测其长期会话能力提升的重要信号；该方法可用于更早发现需要额外支持的咨询师，并优化培训与督导资源配置。
...
TL;DR: 本文提出并评估了 VERGE，一种带有检索增强生成与验证-修正循环的自动化临床文本抽取流程，用于从自由文本病历中识别年轻成人早发结直肠癌相关六类警示症状及家族史风险状态。


<details>
  <summary>Details</summary>
Motivation: 早发结直肠癌在年轻成人中增加，但该人群警示症状的后续检测缺乏循证指南；结构化就诊数据又难以捕获症状持续时间、语境和家族史等关键细节，因此需要可靠地从临床自由文本中抽取风险信息。

Method: 研究开发 VERGE：先通过检索增强生成提出初始标签和证据，再进入有界验证-修正循环，检查文本依据与临床有效性，对错误声明进行修正并复核；无法解决的情况上报人工审核。研究在 4,033 个临床医生标注的“病历-发现”配对上，与单智能体基线、规则式临床语言处理基线及另一底层语言模型进行比较。

Result: 相较单智能体基线，VERGE 降低了假阳性，精确率从 0.764 提升至 0.849，MCC 从 0.681 提升至 0.730，并在精确率-召回率权衡中取得较均衡提升；多数标记错误可自动解决，仅 1.5% 的声明需要人工审核。

Conclusion: 有界、基于验证的工作流能够在不牺牲真阳性检测能力的前提下降低不必要的阳性发现，为面向年轻患者结直肠癌风险评估的更可靠、可信临床语言处理工具提供了可行路径。
...
TL;DR: 本文提出一个用于评估大语言模型理解中文网络评论情境化语用意义的基准，发现模型与人类仍有明显差距。


<details>
  <summary>Details</summary>
Motivation: 中文网络评论常通过间接、戏谑和依赖上下文的表达传递社会意义；现有评测多基于预设现象或受控语用类别，难以检验模型是否能在真实互动语境中辨别评论的合理解读。

Method: 作者从20多万条公开中文社交媒体互动记录中构建数据，形成4,735个人工验证的诊断题项；每个题项包含目标评论、重构的前文语境以及可能的误读选项。评估八个大语言模型，并采用跨出题模型设置，让模型既作为题目生成者也作为解题者，使用leave-writer-out准确率衡量泛化表现。

Result: 任务具有较高难度：最强模型的leave-writer-out准确率为81.42%，八个模型平均为68.70%，而人类准确率达到90.8%。案例分析显示，模型通常能识别宽泛的反讽或玩笑语气，但容易误判具体机制或互动意图。

Conclusion: 该研究表明，当前大语言模型在中文真实社交互动中的细粒度语用理解仍显不足；新的基准能更有效地揭示模型对情境化、间接和 playful 表达的理解缺陷。
...
TL;DR: 本文提出一种基于 Hüllermeier-Rifqi Index 的音标编码算法评价方案，用于衡量编码结果与 IPA 词级转写的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有音标编码算法需要一种更系统、可比较的评价方式，尤其是要判断其编码相似性结构是否符合真实 IPA 转写中的语音相似性。

Method: 将真实 IPA 转写与对应音标编码分别计算成对相似度，相似度由归一化编辑距离得到；再以两者相似度的绝对差形成不一致分数，并用具有相同字母表的随机字符串生成器结果进行校正。同时结合碰撞率评估召回能力。

Result: 该方案可用于多语言转写数据集上比较多种音标编码器，并能量化其与真实语音转写之间的一致程度；此外，它也可用于测量语言书写系统的正字法透明度。

Conclusion: 论文表明，基于广义 Rand Index 的校正不一致度量为音标编码算法提供了更可靠的跨语言评价框架，并具有扩展到书写系统透明度分析的潜力。
...
TL;DR: EXAONE Finance 是面向金融预测的时间序列基础模型，通过无注意力、线性复杂度架构和金融大规模预训练，在金融预测基准上取得领先表现。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型多面向通用领域，依赖自注意力机制，计算成本随序列长度和变量数量二次增长；同时通常假设输入完整，且预训练数据不足以覆盖金融市场的特殊动态，难以适应金融中常见的长序列、多变量、间歇缺失面板数据。

Method: 模型采用 attention-free 架构，以因果一维卷积进行时间混合，以 group-aware pooling MLP 进行变量混合，从而实现线性时间复杂度；训练中引入 masked context augmentation，使模型接触连续缺失片段以增强缺失鲁棒性；并在覆盖股票、外汇、商品、加密资产、固定收益和宏观指标的大规模金融语料上预训练。

Result: 在 FinVerse 金融预测基准上，EXAONE Finance 在点预测精度、横截面资产排序和投资组合盈利能力三个评估层级均排名第一，达到当前最优表现。

Conclusion: 该工作表明，针对金融场景设计的线性复杂度时间序列基础模型，结合缺失增强训练和广泛金融语料预训练，可以更有效地处理金融市场中的长、多通道、缺失时间序列，并提升预测与投资相关任务表现。
...
TL;DR: 本文是一篇关于招聘领域人工智能发展的系统化叙事综述，梳理了从简历/职位匹配与排序到多阶段、证据驱动、可使用工具的招聘工作流与智能体的演进，并提出相应评估框架。


<details>
  <summary>Details</summary>
Motivation: 招聘AI已从单一模型或排序任务转向复杂工作流，涉及证据检索、候选人比较、行动支持与自动执行。现有研究在评价对象、证据层级、外部有效性以及公平、隐私、安全等维度上存在不足，因此需要系统梳理技术演进和评估缺口。

Method: 作者采用截至2026年7月23日的目的性检索与编码协议，并补充截至2026年9月2日的定向更新，组织分析40篇代表性工作，同时结合产业与法律资料；从文档理解、检索、排序、测评、面试、寻源和人工交接等环节区分不同层级证据。

Result: 研究归纳出三类相互耦合的转变：从相似性到双向适配性，从单一模型到复合工作流，从离线预测到证据与生产力对齐的评估。分析发现，行为标签混淆曝光、偏好和资质；私有与合成数据限制外部有效性；最终输出指标掩盖流水线失败；在编码样本中没有研究同时评估效用、公平、隐私和安全。

Conclusion: 论文提出从评估证据到可辩护主张的分阶段映射，并倡导构建双向、证据扎根、时间受控、选择性和可审计的招聘系统。未来进展应依据工作流能否检索正确证据、保留不确定性、支持可申诉决策，并在明确成本与风险约束下改善结果来判断。
...
TL;DR: 本文提出 Harbor Adapters，用统一评测基础设施整合并适配大量智能体基准，支持任意智能体在复杂基准上的可靠、可复现评测。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准数量快速增长，但评测通常依赖复杂环境与特定智能体集成，导致跨基准、跨模型比较困难，成本高且可靠性不足。

Method: 作者构建了可适配任意智能体的基准适配器，将超过80个基准移植到统一框架，并通过代码审查与一致性实验验证；随后在54个基准上对8个不同能力层级模型进行大规模评测，每个模型结合 Terminus-2 及3种原生 harness 之一运行；最后从适配套件中经难度筛选、AI与人工审计、审计-修复循环，整理出 Harbor-Index。

Result: 研究完成了超过80个基准的适配，并在54个基准上获得大规模评测结果；提出的 Harbor-Index 包含82个困难、多样且高质量任务，覆盖29个基准。所有被评测的模型-harness 配置通过率均不超过30%，最强配置 GPT-5.5 with Codex 达到28.0%。

Conclusion: Harbor Adapters 与 Harbor-Index 提供了更统一、低成本且高质量的智能体评测方案，有助于更可靠、全面地分析语言模型智能体的能力边界和失败模式；相关适配器、结果和分析作为开源工件发布。
...
TL;DR: 本文研究用机器学习对电力系统N-k故障情景的安全等级进行三分类，并比较不同预处理配置与分类器表现。


<details>
  <summary>Details</summary>
Motivation: 电力系统在扰动或故障情况下需要快速、准确识别安全风险等级，以支持主动调度决策，降低大规模停电和连锁故障风险；传统故障分析在实时性和可扩展性方面存在局限。

Method: 采用Newton-Raphson潮流计算从IEEE-14与IEEE-30节点系统的N-k（k=1,2,3）故障场景中提取数据，以总体性能指标OPI作为安全度量并标注为安全、中等、严重三类。预处理包括归一化、SMOTE类别均衡、PCA降维以及SMOTE+PCA组合；分类模型包括KNN、随机森林和SVM，并用精确率、召回率和F1值评估，重点关注严重故障类别。

Result: 随机森林总体表现最佳，在IEEE-30和IEEE-14系统上分别取得0.97和0.86的最高F1值；SVM在PCA处理后分类准确性显著提升；KNN更适合结合SMOTE与PCA。整体上，PCA对模型性能提升贡献大于SMOTE；SMOTE可提高召回率，但可能带来更多误报并牺牲精度。

Conclusion: 机器学习可作为传统电力系统故障安全评估的可扩展、高效替代方案，尤其适用于实时安全等级分类；其中随机森林最稳健，PCA是关键预处理手段，而SMOTE需在召回率与误报之间权衡。
...
TL;DR: 论文提出 Iris-mini 与 Iris-pro 两个开源搜索智能体及其完整数据构造、SFT/RL 训练和评测流程，在多个搜索问答基准上取得同参数规模开源模型中的领先表现。


<details>
  <summary>Details</summary>
Motivation: 现有搜索智能体在复杂多跳检索任务上的训练数据构造、推理时上下文管理与公平评测存在不足；同时很多系统依赖复杂多代理或测试时验证，难以隔离模型与训练 recipe 的贡献。本文希望构造更难、更可靠的训练任务，并提供可复现的训练与评测方案。

Method: 从网页语料的超链接结构反向构造任务：基于种子页及外链蒸馏实体图，生成多跳实体链，将非答案实体改写为描述性指代以避免字符串匹配，并筛选出参考模型闭卷失败但给定证据可解的问题。随后将问题转化为搜索轨迹，经轨迹级与轮次级过滤后进行 SFT；再用在线搜索环境进行 RL，集群内提供奖励评判器和观察摘要器，并对超长 rollout 做请求级中断与前缀续训。训练采用 SFT-RL climbing：交替进行 SFT 和 RL，将每轮 RL 中最难且最高效的成功轨迹回流到下一轮监督训练。评测时固定工具集、上下文限制和裁判，并分别报告启用/禁用上下文管理的结果。

Result: 在启用上下文管理时，Iris-mini 在 BrowseComp、BrowseComp-ZH、DeepSearchQA、HLE 上分别达到 82.2、84.8、86.9、52.3；Iris-pro 分别达到 88.6、85.1、92.9、56.4。作者称其为相应参数范围内开源搜索智能体的最强总体结果。

Conclusion: 论文表明，高质量反向构造的多跳搜索数据、严格轨迹过滤、SFT 与在线 RL 的交替爬升策略，以及受控的上下文管理评测，能显著提升单一 ReAct 搜索智能体的复杂搜索能力。作者计划发布模型权重和完整 recipe，以增强可复现性。
...
### [251] [Deep Reinforcement Learning for Optimization of STAR-RIS Phase and Energy Splitting Coefficients in OTFS-NOMA Framework](https://arxiv.org/abs/2609.04536)
*Rais. J. Gachaba,Manobendu Sarker,Anirban Bhowal*

Main category: cs.IT

TL;DR: 论文研究STAR-RIS辅助的OTFS-NOMA下行通信系统，提出基于Beta分布空间的Soft Actor-Critic深度强化学习方法，以低在线复杂度实现相移与能量分配配置并提升和速率。


<details>
  <summary>Details</summary>
Motivation: 在高移动性延迟-多普勒信道中，STAR-RIS、OTFS与NOMA联合系统需要频繁按相干区间重配置；传统交替优化计算开销大、难以满足实时性，因此需要一种能从信道观测快速映射到RIS配置的学习型方案。

Method: 将STAR-RIS相移与能量分裂设计建模为带约束的非凸和速率最大化问题；采用闭式最大比传输波束成形与固定NOMA功率分配；提出最大熵深度强化学习智能体Beta-Space Soft Actor-Critic（SAC-BSE），通过单次前向传播生成连续受限的STAR-RIS配置。

Result: 仿真显示，在每个STAR-RIS分支上有两个NOMA复用用户时，该方法收敛较快；在用户速度扩大128倍范围内，和速率退化约限制在10%；随着发射功率和STAR-RIS单元数增加，相比OTFS-only、NOMA-only、STAR-RIS-only、固定分裂和模式切换等基线均取得稳定增益。

Conclusion: 所提SAC-BSE能有效降低高移动场景下STAR-RIS配置的在线重优化负担，并在OTFS-NOMA融合框架中实现较好的鲁棒性和频谱效率提升。
...
### [252] [Ultra-High Resolution Method for Multipath Within a Co-Delay-Doppler Bin in DFT-P-OCDM](https://arxiv.org/abs/2609.04669)
*Mingxuan Han,Weile Zhang,Feifei Gao*

Main category: cs.IT

TL;DR: 本文针对DFT预处理OCDM通信感知一体化波形在同一共延迟-多普勒栅格内存在多条物理路径时的超分辨感知问题，提出两阶段超高分辨率框架TSUR，实现路径数与分数多普勒的更准确估计及信道重构。


<details>
  <summary>Details</summary>
Motivation: 在复用通信信号进行感知时，DFT-P-OCDM波形面对双选择性信道中的多径聚集问题：多条物理路径可能落入同一共延迟-多普勒bin，导致导频可分辨的延迟类别少于真实物理路径数，从而使路径数判定、分数多普勒偏移估计和信道重构变得困难。

Method: 作者推导了多径落入同一共延迟-多普勒bin时，DFT预处理Fresnel域中的逐点输入输出关系，并据此提出两阶段超高分辨率TSUR框架：第一阶段利用相位递进导频估计延迟；第二阶段利用泄漏样本估计各延迟类别内的多普勒参数和路径数量。同时推导CRLB并分析导频配置在感知分辨率与通信恢复之间的权衡。

Result: 仿真表明，TSUR能够分辨同延迟且位于同一共延迟-多普勒bin内的多条路径，对存在延迟偏移的情况保持鲁棒，并在分数多普勒估计误差方面优于顺序提取方法和离网基线方法。

Conclusion: 该工作证明了通过DPF域输入输出建模与两阶段泄漏样本利用，可以显著提升DFT-P-OCDM波形在复杂双选择性多径场景下的感知超分辨能力，同时为导频设计中的通信-感知性能折中提供理论依据。
...
### [253] [The Cross-Correlation Distribution of the Niho-Type Decimation $d=4(2^m-1)+1$](https://arxiv.org/abs/2609.04683)
*Maosheng Xiong,Haode Yan*

Main category: cs.IT

TL;DR: 本文解决了二元有限域上一类 Niho 型抽取的互相关分布问题，并给出所有频率的显式公式。


<details>
  <summary>Details</summary>
Motivation: 互相关问题是序列设计中的经典问题；对特定 Niho 型抽取 d=4(2^m-1)+1，其互相关分布此前需要通过等价的有限域单位圆上多项式根数分布来精确刻画，核心难点在于四元根集的计数。

Method: 将问题转化为统计 f_a(x)=x^7+a x^4+\bar a x^3+1 在 U_{q+1} 中的根数分布；对可能作为根集的四元素子集按元素乘积归一化，利用相关 resolvent 条件降维为若干 F_q 上方程；再通过特征和、Kloosterman 和以及 Carlitz 定理计算相关计数。

Result: 作者得到了该 Niho 型抽取在所有正整数 m 情形下的互相关分布，并给出了各频率的显式公式。

Conclusion: 该工作完成了 d=4(2^m-1)+1 这一 Niho 型抽取的完整互相关分布求解，展示了通过单位圆根集结构、resolvent 与 Kloosterman 和相结合进行精确枚举的有效性。
...
### [254] [Decoding Error Probability of the Random Code Ensemble over the Erasure Channel](https://arxiv.org/abs/2609.04688)
*Chin Hei Chan*

Main category: cs.IT

TL;DR: 本文研究擦除信道上所有 (n,M)_q 码集合的平均译码错误概率，并给出不同译码准则下的显式公式及渐近错误指数。


<details>
  <summary>Details</summary>
Motivation: 现有研究需要更精确地刻画随机码集在擦除信道中的平均性能，尤其是在无歧义译码、列表译码和最大似然译码等不同译码原则下的错误概率表现。

Method: 作者针对所有 (n,M)_q 码的整体集合，分别在擦除信道和三类译码准则下推导平均译码错误概率的闭式/显式表达；进一步令码本大小为 q^{Rn}，分析当码长 n 增大时的渐近行为并推导错误指数。

Result: 论文获得了擦除信道下三种译码原则对应的平均译码错误概率显式公式，并给出了所有 (n,q^{Rn})_q 码集合的渐近错误指数。

Conclusion: 该工作为擦除信道中随机码集合的平均译码性能提供了统一且可计算的理论刻画，并揭示了码率 R、码长 n 和译码原则对错误概率指数衰减速度的影响。
...
### [255] [Near-Field Physical-Layer Authentication Under Impersonation Attacks](https://arxiv.org/abs/2609.04879)
*Hajar El Hassani,Linda Senigagliesi,Arsenia Chorti*

Main category: cs.IT

TL;DR: 本文研究近场物理层认证中主动冒充攻击的可行性，指出近场距离维度可显著增强认证区分能力。


<details>
  <summary>Details</summary>
Motivation: 传统远场物理层认证主要依赖到达角信息，而近场信道导向矢量同时受角度和距离影响。论文旨在分析攻击者能否通过预编码伪造合法发射端的接收信号，并揭示近场条件下距离差异对抗冒充攻击的作用。

Method: 论文以合法发射端 Alice 与攻击者 Eve 在接收端 Bob 处产生信号之间的均方误差最小化为目标。分别分析单天线 Eve 与多天线 Eve：单天线场景推导最优标量预编码，并在二阶 Fresnel 近似下给出完美冒充条件；多天线场景推导最优预编码向量，并用子空间包含关系刻画完美冒充条件。

Result: 单天线 Eve 只有在与 Alice 具有相同到达角和相同距离时才可能完美冒充；多天线 Eve 只有当 Alice 的导向矢量属于 Eve 多个导向矢量张成的子空间时才可能完美冒充。仿真表明，在近场中，即便 Alice 与 Eve 到达角相同，只要距离不同也足以阻止成功冒充。

Conclusion: 近场物理层认证相较远场具备额外的距离分辨能力，可提高对冒充攻击的鲁棒性。论文的理论推导与仿真共同说明，近场空间特征为安全认证提供了更强判别依据。
...
### [256] [Minimum Schubert Codewords and Second-Minimum Grassmann Codewords](https://arxiv.org/abs/2609.04916)
*Muskan Khaneja,Prasant Singh*

Main category: cs.IT

TL;DR: 本文解决了 Ghorpade 与 Singh 于 2018 年提出的猜想，完成了 Schubert 码最小重量码字的分类，并进一步刻画和计数了 Grassmann 码的第二最小重量码字。


<details>
  <summary>Details</summary>
Motivation: Schubert 码与 Grassmann 码的码字重量结构是代数编码理论中的重要问题。已有工作提出了关于 Schubert 码最小重量码字分类的猜想，但尚未对所有有限域大小 q 和所有参数 α 给出完整证明；同时，Grassmann 码第二最小重量码字的结构与数量也需要明确刻画。

Method: 作者首先对 Schubert 码 C_α(ℓ,m) 的最小重量码字进行分类，证明并推广相关猜想适用于所有 q 和 α。随后利用该分类结果分析 Grassmann 码 C(ℓ,m) 的码字索引结构，将第二最小重量码字与外代数中可表示为“可分解 (m−ℓ−2)-向量与秩为 4 的交替 2-向量之积”的元素联系起来。

Result: 论文证明了 Schubert 码 C_α(ℓ,m) 的最小重量码字分类定理；并证明 Grassmann 码 C(ℓ,m) 的码字具有第二最小重量当且仅当其索引元素满足特定外代数分解形式；最后给出了 Grassmann 码第二最小重量码字的枚举公式或计数结果。

Conclusion: 该工作完善了 Schubert 码最小重量码字理论，解决了既有猜想，并由此得到 Grassmann 码第二最小重量码字的精确结构刻画与枚举，深化了对相关代数几何码重量分布的理解。
...
### [257] [Algebraic Geometry Codes Approach the Half-Singleton Bound with Constant Field Size](https://arxiv.org/abs/2609.05017)
...
### [258] [Latency-Optimal Geo-Distributed Storage over Structured Networks](https://arxiv.org/abs/2609.05229)
