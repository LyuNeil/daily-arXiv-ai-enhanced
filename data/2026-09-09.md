...
TL;DR: 本文提出使用稀疏自编码器（SAE）的可解释潜变量来预测视觉-语言模型（如 CLIP）的失败，并在实验中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 随着视觉-语言模型被用于高风险场景，仅依赖置信度或辅助分类器的失败预测方法虽然有效，但可解释性不足；因此需要一种既能预测失败又能解释失败原因的机制。

Method: 作者将失败预测建模为基于 SAE 稀疏潜在激活的分类任务，并提出三阶段的失败感知训练流程，使 SAE 学到的潜在方向在保持可解释性的同时，对失败预测更具信息量。

Result: 实验显示，该框架在失败预测性能上超过所评估的基线方法；进一步分析表明，失败感知训练促使 SAE 潜在方向捕捉更多类别特异性概念。

Conclusion: SAE 可为 VLM 失败预测提供兼具性能与可解释性的方案，并能揭示失败时表征从类别特异性概念向模糊或风格相关概念转移，同时可能支持运行时失败恢复。
...
TL;DR: 本文提出个性化安全评测基准 MPS-Bench，发现前沿视觉语言模型在缺失用户上下文时很少主动追问，个性化安全表现较差；作者进一步揭示“视觉主导”机制，并提出轻量监控器 PRISM 来预测何时应延迟回答。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型常被用于高风险场景，但模型通常不知道用户的个人状态、情绪或情境背景；在一般情况下看似合理的回答，对特定用户可能并不安全。因此需要系统评估和改进多模态系统的个性化安全能力。

Method: 作者构建 MPS-Bench，包含来自 584 张真实图像的 5,181 个场景，覆盖 12 个高风险领域，并为每个场景配备隐藏用户画像；评估 8 个前沿 VLM。随后通过多模态交互分析和因果干预研究失败机制，识别视觉信息如何影响文本表示。基于该机制，提出 PRISM：一种使用双向跨模态调制的轻量输入监控器，用于预测查询是否需要延迟回答或请求更多上下文。

Result: 8 个前沿 VLM 在 86%–99% 的情况下直接作答，而不是请求缺失上下文；所有模型的个性化安全评分均不超过 2.6/5。机制分析显示视觉信息早期进入文本表示，并在多模态融合中抑制文本风险信号。PRISM 达到 0.978 AUC，并在所有测试模型上改进安全—效用 Pareto 前沿。

Conclusion: 个性化安全是当前 VLM 的显著短板，主要源于早期视觉影响对文本风险线索的压制；仅依赖后期内部修复不可靠。通过输入阶段的跨模态监控与适时延迟回答，PRISM 能有效提升多模态系统在高风险个性化场景中的安全性。
...
TL;DR: 本文提出 RA-GRPO，用“反向反思”增强扩散模型的强化学习偏好对齐，在不增加推理开销的情况下提升文本到图像/视频生成的语义一致性、视觉真实感与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于策略梯度的扩散模型偏好对齐方法探索效率较低，容易陷入局部最优，并可能导致奖励投机、语义偏离和视觉质量下降。因此需要一种更稳定、更高效的后训练对齐框架。

Method: 方法包含两个核心组件：1）Diffusion Reflection：利用弱估计器对扩散过程进行反演，修正中间采样轨迹，将潜变量引导到更接近真实数据流形的高概率区域；2）Counterfactual Path Synthesis：将修正后的轨迹隐式蒸馏进策略模型，使模型在训练中吸收搜索式探索的收益，同时避免推理阶段额外开销。整体框架称为 Reflection-Aware GRPO，且与具体模型架构无关，可接入标准扩散生成管线。

Result: 在文本到图像和文本到视频模型上的实验表明，RA-GRPO 显著优于现有方法，尤其在缓解奖励投机、提升泛化能力、保持语义忠实度和改善视觉真实感方面表现突出。

Conclusion: RA-GRPO 通过在优化中结合反向反思与轨迹蒸馏，为扩散生成模型的稳定偏好对齐提供了一种有效方向，兼具性能提升、架构通用性和推理零额外开销等优势。
...
TL;DR: 本文提出 DM-Align，将视频生成模型的蒸馏与人类偏好对齐统一到单阶段分布匹配优化中，避免传统强化学习对齐的高计算开销和两阶段流程中的模型崩塌问题。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型的人类偏好对齐通常依赖强化学习，但强化学习计算成本高；若先强化学习再蒸馏，成本过高，若先蒸馏再强化学习，又容易导致模型崩塌。因此需要一种同时兼顾蒸馏质量与偏好对齐、且更高效稳定的统一优化方法。

Method: 作者基于 Distribution Matching 框架，在原有用于缩小真实模型与生成模型分布差距、提升清晰度和保真度的蒸馏梯度之外，引入 DM-Align 的偏好引导梯度。该梯度受 DPO 和 GRPO 启发，通过偏好样本对或组内探索构造分布差距，直接引导模型生成更符合人类偏好的样本，并将蒸馏梯度与偏好梯度协同优化。

Result: 在多个基础视频生成模型上的实验表明，该方法能够稳定提升蒸馏质量与偏好对齐效果，并持续优于单独使用蒸馏或对齐的变体，以及传统顺序式两阶段流水线。

Conclusion: DM-Align 提供了一种无需复杂强化学习流程、多步奖励评估或 ODE-SDE 转换的高效单阶段优化框架，可更稳健地实现视频生成模型的高质量蒸馏与人类偏好对齐。
...
TL;DR: 论文提出 CARNIVAL：利用模拟数据、前向成像模型扰动生成配对视图，并结合模拟管线中的蛋白位置与类别等特权信息，进行冷冻电子断层扫描体数据中的蛋白注释自监督训练。


<details>
  <summary>Details</summary>
Motivation: 拥挤的 cryo-ET 体数据受限倾角采集和测量算子严重破坏影响，真实标注稀缺且噪声强，导致蛋白分类与检测困难。作者希望通过更贴近成像物理过程的模拟数据训练，提高模型对真实断层图的泛化能力。

Method: 首先，利用前向模型引入的成像退化生成同一场景的领域特定增强配对视图，并将其作为不变性目标整合进 LeJEPA 自监督框架。其次，使用模拟管线提供的蛋白位置和身份信息来指导网络架构与损失函数设计，使语义特征在密集特征体中定位到蛋白所在位置。

Result: 在真实 tomograms 上无需微调，针对分类和检测任务进行评估；基准数据包含多种蛋白类型和两类断层图处理方式。CARNIVAL 优于一种使用模拟数据和对比学习目标训练、但未利用前向模型配对视图和特权信息的先进模型。

Conclusion: 结合成像前向模型的物理退化增强与模拟数据中的特权标注信息，可以有效提升 cryo-ET 蛋白注释模型的表征质量和跨域泛化能力。
...
TL;DR: 本文提出一种利用视频运动边界自动生成伪实例监督的视觉预训练框架，使静态图像编码器学习对象中心表示，并在多种几何与实例敏感任务上取得较强迁移效果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉预训练方法往往更关注语义类别特征，难以保持单个对象实例的身份、一致性和边界结构；而物理世界感知任务需要更强的对象中心表示，尤其是在深度、3D检测、占用预测和规划等场景中。

Method: 方法从原始视频中使用现成光流估计与聚类获得运动边界和伪实例掩码，用这些伪标签通过像素级成对度量学习监督单图像编码器；无需人工标注或相机标定。作者先构建1.95亿伪标注帧，再通过Motion-Verified Self-Training结合模型提议与运动证据扩展到4.21亿帧，并训练/蒸馏到Swin系列骨干网络。

Result: 在单目深度估计、3D目标检测、3D占用预测和端到端规划等任务上，所学模型相较监督和自监督预训练基线达到有竞争力或更优的性能，尤其在依赖几何结构和实例区分的任务上迁移表现突出。

Conclusion: 运动信号可作为可扩展的对象级监督来源，帮助静态图像编码器学习实例一致的视觉表示，为大规模视觉预训练提供了与语义预训练互补的新方向。
...
### [81] [How Much Does Corpus Choice Change Dependency-Distance Estimates?](https://arxiv.org/abs/2609.04223)
*Sirui Chen*

Main category: cs.CL

TL;DR: 本文检验单一语料库得到的平均依存距离（MDD）能否代表语言属性，发现跨同语种不同树库的一致性有限；依存长度最小化（DLM）作为定性普遍性成立，但跨语言排序不稳定。


<details>
  <summary>Details</summary>
Motivation: 以往研究常将单一语料库中的依存距离估计视为某种语言的稳定特征，但这一假设缺乏在独立编制的同语种语料库之间的系统检验。

Method: 基于 Universal Dependencies v2.18 中38组同语种树库配对，比较平均依存距离估计；采用一致性相关系数、Bland-Altman 分析，以及包含12种预处理设定的 multiverse 设计评估结果稳健性。

Result: 跨树库一致性至多为中等；替换树库会导致近40%的语言两两排序发生反转；树库选择解释约29%的组间方差。该分歧显著超过树库内抽样误差，并在12种预处理规格下持续存在。不过，每个树库均显示归一化比值低于1，支持依存长度最小化。

Conclusion: MDD 更适合作为受语料、语体、语法和标注因素共同影响的复合指标，而非稳定的语言级参数；DLM 的定性普遍性可以跨语料维持，但基于MDD的跨语言序数排名不可靠。
...
### [82] [Memory as transformation: LETHE, a self-referential gan-inspired architecture](https://arxiv.org/abs/2609.04289)
*Francesco Vitucci,Anthony Di Furia,Francesco Scagliola*

Main category: cs.CL

TL;DR: LETHE 是一个在 SuperCollider 中实现的自参照闭环声音生成系统，通过类 GAN 的形式框架和随机扰动优化机制，使音频处理参数在无外部数据监督的条件下自主演化。


<details>
  <summary>Details</summary>
Motivation: 该工作旨在探索自参照电声音乐中的自适应闭环生成机制，将作曲家预设的参数空间交由系统内部反馈动态驱动，从而产生随时间演化的声音结果。

Method: 系统使用围绕两条延迟线构建的 3×3 混合矩阵处理音频，其九个系数和两个延迟时间由五特征线性判别器与类似单样本 REINFORCE 的随机扰动优化器共同更新。判别器将当前能量行为与初始状态档案进行比较，并据此引导参数变化；系统支持循环、固定与实时声源的独立混合。

Result: 在固定与循环会话以及消融对照实验中，主动生成器被证明是参数演化所必需的；在 15 次消融会话中，Δc22 均为 0.000，表明无主动生成机制时相关参数不发生演化。

Conclusion: LETHE 展示了一种将声音结果委托给自适应闭环系统的创作方法，其核心贡献在于以作曲家定义的参数空间和内部判别反馈实现自参照的声音演化。
...
### [83] [Evidence Integration in Large Language Models](https://arxiv.org/abs/2609.04290)
*Sebastien Kawada,Manolis Kellis*

Main category: cs.CL

TL;DR: 论文提出一种分布式理论解释大语言模型如何在已有初始答案倾向后整合外部证据，并通过大规模实验验证：证据影响取决于接收模型自身分布，而非单一来源可信度。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越依赖工具、检索增强、其他智能体和用户提供的外部证据进行推理，模型在已形成初步判断后如何吸收这些证据仍缺乏清晰机制解释。

Method: 作者构建“接收者分布—证据倾斜”理论，认为证据会按接收者先验权重与候选证据倾斜程度移动初始答案分布；并在超过一千万次试验、12个来自4个模型家族的大语言模型、8个领域任务中验证预测，同时使用因果干预、表征可解码分析与J-lens分解研究内部机制。

Result: 实验支持三项预测：接收者本来更可能给出的候选答案更具说服力；模型更容易整合自身特征性错误而非外来源错误；相同证据可提升弱模型却损害强模型。研究还发现，与接收者一致的错误比同等错误率的随机错误更显著降低性能；模型即使内部验证候选无效，仍常将其整合进最终答案。

Conclusion: 外部证据整合不是简单的来源信任问题，而是由接收模型自身分布和控制策略决定的后期网络过程；验证表征与候选答案整合可相互解耦，说明模型“知道证据无效”并不必然阻止其被用于生成最终答案。
...
### [84] [MedProb: Probing Internal Representations of Vision-Language Models for Medical Question Answering](https://arxiv.org/abs/2609.04336)
*Erfan Nourbakhsh,Ke Yang,Anthony Rios*

Main category: cs.CL

TL;DR: 论文提出 MedProb：一种轻量级探测框架，用冻结视觉语言模型表征预测多选医学视觉问答答案，发现其能恢复比提示生成更多的答案相关信号。


<details>
  <summary>Details</summary>
Motivation: 现有 Med-VQA 通常假设需要医学领域微调、大模型或复杂多智能体流程。作者希望重新检验这一假设，并评估冻结通用/医学 VLM 表征中究竟包含多少可线性解码的 Med-VQA 信息，以及生成式评估是否低估小模型能力。

Method: 使用 MedProb 对冻结 VLM 表征进行轻量级线性/探测式解码，避免自由文本生成；在 PATH-VQA、SLAKE、VQA-RAD 等数据集上进行多选/多分类评测，并比较提示方法、医学 VLM、智能体系统，以及 14 组通用与医学适配 VLM；同时分析答案位置偏差，并用拒绝采样评分扩展到开放式生成。

Result: MedProb 相比提示生成恢复了更多答案相关信号，表现强于若干医学 VLM 和智能体系统；相较提示评估，它缩小了小模型与大模型之间的表观差距；医学适配并不稳定提升线性可解码性；自由文本生成存在最高约 10 个百分点的答案位置偏差，MedProb 也有位置偏差但受影响方式不同。

Conclusion: 冻结 VLM 中已包含大量可恢复的 Med-VQA 信号，复杂医学微调或生成式管线并非总是必要；基于探测的评估能更准确揭示模型表征能力，但主要结论集中在多选/多分类设置，开放式任务仍需进一步验证。
...
### [85] [Adapting from Downturns: Prediction of Long-Term Conversational-Skill Development in Mental-Health Crisis Counselors](https://arxiv.org/abs/2609.04350)
*Vivian Nguyen,Lillian Lee,Elizabeth A. Olson,Cristian Danescu-Niculescu-Mizil*

Main category: cs.CL

TL;DR: 本文提出一项早期预测任务：根据志愿心理健康危机咨询师职业早期的会话表现，预测其未来是否会提升引导对话走向积极结果的能力；核心方法关注咨询师在反复遇到困难对话情境时的适应变化，并优于直接基于转录文本的基线方法。


<details>
  <summary>Details</summary>
Motivation: 心理健康咨询高度依赖对话能力，但志愿咨询师通常缺乏充分督导和结构化反馈。若能在职业早期识别哪些咨询师未来可能无法有效提升，就能更合理地分配培训、反馈和支持资源。

Method: 作者将问题形式化为一个长期能力提升预测任务。方法首先识别咨询师最初容易处理不佳的特定对话时刻类型；随后追踪他们在之后遇到相似情境时如何调整回应；最后学习哪些早期适应模式能够预测数月甚至数年后的能力提升。

Result: 尽管长期未来表现预测本身较困难，所提出的“咨询师适应”建模方法相比直接从会话转录文本学习的基线模型取得了更好的预测效果。

Conclusion: 咨询师能力成长不仅体现在单次对话内容中，更体现在其面对相似困难情境时的学习与适应轨迹。建模这种早期适应过程有助于预测长期进步，并可用于更早发现需要额外支持的咨询师。
...
### [86] [VERGE: Verification-Enhanced Refinement for Grounded Extraction of Early-Onset Colorectal Cancer Symptoms in Clinical Notes](https://arxiv.org/abs/2609.04366)
...
### [87] [You Really Didn't Get That? Benchmarking Social Pragmatic Inference for Indirect and Playful Chinese Online Comments](https://arxiv.org/abs/2609.04384)
...
### [88] [Evaluation of Phonetic Encoding Algorithms on Transcription Datasets](https://arxiv.org/abs/2609.04391)
...
### [89] [The Anatomy of an ASR Hallucination](https://arxiv.org/abs/2609.04404)
...
### [90] [A Systematic Evaluation of Cross-Lingual Consistency Enhancement Methods in Multilingual Language Models](https://arxiv.org/abs/2609.04409)
...
### [91] [What Attention Recalls and Recurrence Controls in Hybrid Language Models](https://arxiv.org/abs/2609.04434)
...
### [92] [GRACE: Graph-Grounded Reflective Agent Copilot Engine for Expert-in-the-Loop Knowledge Expansion](https://arxiv.org/abs/2609.04442)
...
### [93] [TRILOGUE: A Trilingual Spoken Dialogue Fact-Checking Benchmark with Evidence and Paired Audio](https://arxiv.org/abs/2609.04452)
...
### [94] [Shared circuits predict whether LLMs generalize across formats in arithmetic reasoning](https://arxiv.org/abs/2609.04463)
...
### [95] [Safety for Whom? Boundary-Aware Self-Distillation for Controlled LLM Safety Refusal](https://arxiv.org/abs/2609.04482)
...
### [96] [Patterns of Priming in Production: Lexical, Semantic and Structural Alignment in Language Model Generation](https://arxiv.org/abs/2609.04484)
...
### [97] [Cultural Misalignment in Large Language Models: Detection, Measurement, and Mitigation Through Targeted Fine-Tuning](https://arxiv.org/abs/2609.04485)
...
### [98] [Towards Understanding Pause Token Fine-Tuning Dynamics: A Mode Retention Perspective](https://arxiv.org/abs/2609.04489)
...
### [99] [LentEx: Generalizable Latent Entity Extraction via Synthetic Data and Instruction-Tuned LLMs](https://arxiv.org/abs/2609.04511)
...
### [100] [Scale-QLoRA: Code-Invariant Adapter Merging for Native 4-bit Microscaling LLMs](https://arxiv.org/abs/2609.04526)
...
### [101] [A Calibrated Reflection Approach for Enhancing Confidence Estimation in LLMs](https://arxiv.org/abs/2609.04539)
...
### [102] [Rhythms of Work: Multi-Scale Interpretation of Human Behavioral Traces for Workplace Agents](https://arxiv.org/abs/2609.04556)
*Lin Ai,Scott Counts*
...
### [103] [When Do Internal Probes Beat Reading the Answer? Miscalibrated Readouts and Behavior-Concealed Knowledge in Language Models](https://arxiv.org/abs/2609.04582)
...
### [105] [Tracing Audio Grounding and Answer Selection in Audio LLMs](https://arxiv.org/abs/2609.04637)
...
### [106] [CAGE: Coherence-Aware Graph Encoding for Retrieval-Augmented Generation](https://arxiv.org/abs/2609.04647)
...
### [107] [ConsensusBench: Benchmark of Consensus Nodes for LLM Reasoning via Outcome Reward Densifying](https://arxiv.org/abs/2609.04648)
...
### [108] [Choosing the Right Language Mode at Inference Time for Multilingual Reliability](https://arxiv.org/abs/2609.04653)
...
### [109] [Controlling and Assessing Appropriate Persona Use in LLM-based Dialogue Generation](https://arxiv.org/abs/2609.04676)
...
### [110] [How Do Language Models Represent and Use Phonological Information for Allomorph Selection?](https://arxiv.org/abs/2609.04708)
...
### [111] [Refuse without Refusal: A Structural Analysis of Safety-Tuning Responses for Reducing False Refusals in Language Models](https://arxiv.org/abs/2609.04714)
...
### [112] [Knowing What Not to Answer: Selective Non-Compliance in Vision-Language Models](https://arxiv.org/abs/2609.04720)
...
### [113] [Beneath the Surface of Chains-of-Thought: A Mechanistic Interpretation of Reasoning Operations in LLMs](https://arxiv.org/abs/2609.04753)
...
### [114] [Vectorizing Classical Tamil: Representation Learning for Verse-Commentary Pairs](https://arxiv.org/abs/2609.04755)
...
### [115] [Can Activation Steering Capture Multidimensional Authorship Style?](https://arxiv.org/abs/2609.04792)
...
### [116] [Recurrence Is Not Enough: Causally Validating Multilingual SAE Translation Features in Gemma 2 and 3](https://arxiv.org/abs/2609.04808)
...
### [117] [A Systematic Comparison of Multilingual Interpretability Methods Reveals Anisotropy-Driven Failures](https://arxiv.org/abs/2609.04819)
...
### [118] [Reinforcement Learning for improving Large Language Models' Catalan text simplification capabilities](https://arxiv.org/abs/2609.04823)
...
### [119] [Generating Constructive Feedback on Stories via Reinforcement Learning](https://arxiv.org/abs/2609.04824)
...
### [120] [On Epistemic Diversity in Large Language Models](https://arxiv.org/abs/2609.04835)
...
### [121] [MABPD: Multi-Agent Bias Probing & Detection via Structured Argument Debate](https://arxiv.org/abs/2609.04841)
...
### [122] [MMTClinic: Multimodal, Multilingual Time Series Question Answering and Reasoning Benchmark for Clinical Domain](https://arxiv.org/abs/2609.04842)
...
### [123] [CC-Mediation: Evaluating Large Language Models for Cross-Cultural Conflict Mediation](https://arxiv.org/abs/2609.04855)
...
### [124] [Cache-Aware Joint Router Adaptation for Memory-Efficient MoE Inference](https://arxiv.org/abs/2609.04895)
...
### [125] [RefactorPlatform: An Open-Source Harness for Controlled Evaluation of Repository-Scale Refactoring Agents](https://arxiv.org/abs/2609.04898)
...
### [126] [Discourse Dependency: A Continuous Criterion for Translation Difficulty](https://arxiv.org/abs/2609.04959)
...
### [127] [BIT.UA at BioASQ 14B: Modular Retrieval with pg_textsearch and Qdrant, and Agent-Based Answer Generation](https://arxiv.org/abs/2609.04999)
...
### [128] [MoirfEolas and CríochScore: Developing Resources for and the Evaluation of Tokenization Alignment with Irish Morphology](https://arxiv.org/abs/2609.05022)
...
### [129] [Leveraging Low-Level Symbolic Competences for Unsupervised Grounding in Hallucination Detection](https://arxiv.org/abs/2609.05025)
...
### [130] [How do LLMs Evaluate Perceived Moral Agency? Investigating Moral Decision-Making in Human-Artificial Agents Interactions](https://arxiv.org/abs/2609.05037)
...
### [131] [EuroAlpaca: Task-Preserving Localisation of Instruction Data for European Languages](https://arxiv.org/abs/2609.05043)
...
### [132] [A Structured Debate-Mixture-of-Agents Framework for Complex Clinical Diagnostic Decision Support](https://arxiv.org/abs/2609.05069)
...
### [133] [Influence Score and Transformers interpretability: Measure of the Effective Impact of Attention Heads at inference time](https://arxiv.org/abs/2609.05074)
...
### [134] [Improving Language Identification for Code-Switched Utterances with Integer Linear Programming](https://arxiv.org/abs/2609.05099)
...
### [135] [NS-ST-GraphRAG: Neuro-Symbolic Spatio-Temporal GraphRAG for Literary Knowledge Processing](https://arxiv.org/abs/2609.05139)
*Zheng Kui Lin*
...
### [136] [A Human-in-the-Loop Framework for AI-Assisted Scoring in Large-Scale Writing Assessment](https://arxiv.org/abs/2609.05143)
...
### [137] [From Vision to Language: Investigating Causal Information Flow in Multimodal Decision-Making](https://arxiv.org/abs/2609.05149)
...
### [138] [Compression Beyond the Uncompressed: A Two-Stage Training Recipe for Soft Context Compression in RAG](https://arxiv.org/abs/2609.05152)
...
### [139] [Can Large Language Models Anticipate Behavioral Responses to Social Policies? A Case of Pension Enrollment Prediction among China's Flexible Workers](https://arxiv.org/abs/2609.05189)
...
### [140] [A Verifier-Guided Explainable Reasoning Framework with Gold-Anchored QLoRA, Task-Aware Mixture-of-Experts, and Group-Relative RLVR](https://arxiv.org/abs/2609.05221)
...
### [141] [Self-Supervised Lexical Representation Learning for Fast, Large-Scale Phylogenetic Inference](https://arxiv.org/abs/2609.05262)
*Tim Wientzek*
...
### [142] [LexFlip: A Dissociation Diagnostic for Legal Meaning Preservation Metrics](https://arxiv.org/abs/2609.05296)
*Gaurab Baral*
...
### [143] [WearableQA: A Benchmark for Health Reasoning over Real-World Wearable Data](https://arxiv.org/abs/2609.05405)
...
### [144] [Deep Reinforcement Learning for Optimization of STAR-RIS Phase and Energy Splitting Coefficients in OTFS-NOMA Framework](https://arxiv.org/abs/2609.04536)
*Rais. J. Gachaba,Manobendu Sarker,Anirban Bhowal*

Main category: cs.IT

TL;DR: 本文研究STAR-RIS辅助的OTFS-NOMA下行通信，通过深度强化学习快速生成相移与能量分裂配置，以提升高移动性场景下的系统和速率。


<details>
  <summary>Details</summary>
Motivation: 在OTFS-NOMA与STAR-RIS结合的高移动性通信框架中，时延-多普勒信道变化使得传统交替优化难以在每个相干区间内实时完成STAR-RIS重构，因此需要低在线复杂度、能适应信道变化的配置方法。

Method: 将STAR-RIS相移与能量分裂设计建模为带约束的非凸和速率最大化问题；采用闭式最大比传输波束成形和固定NOMA功率分配；提出Beta-Space Soft Actor-Critic（SAC-BSE）最大熵深度强化学习智能体，将观测到的信道实现通过单次前向传播映射为STAR-RIS配置。

Result: 仿真显示，在每个STAR-RIS分支包含两个NOMA复用用户的设置下，所提方法收敛较快；在用户速度变化128倍范围内，和速率退化约限制在10%；随着发射功率和STAR-RIS单元数增加，相比OTFS-only、NOMA-only、STAR-RIS-only、固定分裂和模式切换等基线均获得稳定增益。

Conclusion: 该工作表明，基于SAC-BSE的STAR-RIS配置可有效降低高移动性OTFS-NOMA系统中的逐区间重优化负担，并在复杂非凸资源配置问题中实现较好的性能与在线部署效率。
...
### [145] [Ultra-High Resolution Method for Multipath Within a Co-Delay-Doppler Bin in DFT-P-OCDM](https://arxiv.org/abs/2609.04669)
*Mingxuan Han,Weile Zhang,Feifei Gao*

Main category: cs.IT

TL;DR: 论文针对DFT预处理OCDM波形在同一共延迟-多普勒栅格内存在多条物理路径时的感知难题，提出TSUR两阶段超高分辨率框架，以提升路径数判定、分数多普勒估计和信道重构能力。


<details>
  <summary>Details</summary>
Motivation: 通信系统可复用通信发射信号进行感知，但在双选择性信道中，多个物理路径可能落入同一共延迟-多普勒bin，导致导频推断出的可分辨延迟类别少于真实路径数，从而造成路径数估计、分数多普勒偏移估计和信道重构困难。

Method: 作者推导了多路径共bin条件下DFT预处理Fresnel域中的输入输出逐点关系，并据此设计两阶段超高分辨率（TSUR）框架：第一阶段利用导频相位递进估计时延；第二阶段利用泄漏采样估计每个延迟类别内的多普勒参数和路径数。同时推导CRLB，并分析导频配置在感知分辨率与通信恢复之间的权衡。

Result: 仿真表明，TSUR能够分辨同一延迟/共延迟-多普勒bin内的多条路径，对存在时延偏移的情况保持鲁棒，并相比顺序提取和离网基线方法取得更低的分数多普勒估计误差。

Conclusion: 该工作证明了通过建立DPF域输入输出关系并结合两阶段泄漏样本分析，可以显著增强DFT-P-OCDM通信感知一体化系统在复杂双选择性信道中的超分辨感知能力，同时揭示了导频设计对通信与感知性能的影响。
...
### [146] [The Cross-Correlation Distribution of the Niho-Type Decimation $d=4(2^m-1)+1$](https://arxiv.org/abs/2609.04683)
*Maosheng Xiong,Haode Yan*

Main category: cs.IT

TL;DR: 本文确定了有限域上 Niho 型抽取 d=4(2^m-1)+1 的互相关分布，并给出所有频率的显式公式。


<details>
  <summary>Details</summary>
Motivation: 互相关问题是序列设计中的经典问题；该工作旨在解决特定 Niho 型抽取在任意正整数 m 下的互相关分布计算问题。

Method: 将互相关分布问题等价转化为统计多项式 f_a(x)=x^7+a x^4+\bar a x^3+1 在单位圆群 U_{q+1} 中的根数分布；重点枚举可能作为根集出现的四元子集。作者通过对子集按乘积归一化，并利用相关 resolvent 条件，将枚举化为有限域 F_q 上若干方程，再用特征和、Kloosterman 和以及 Carlitz 定理求值。

Result: 成功计算了所需枚举量，处理了剩余的混合 Kloosterman 和，并最终得到该 Niho 型抽取互相关分布中所有频率的显式公式。

Conclusion: 论文完整解决了 d=4(2^m-1)+1 在 F_{2^{2m}} 上的互相关分布问题，展示了代数归一化、resolvent 分析与指数和方法结合的有效性。
...
### [147] [Decoding Error Probability of the Random Code Ensemble over the Erasure Channel](https://arxiv.org/abs/2609.04688)
*Chin Hei Chan*

Main category: cs.IT

TL;DR: 本文针对擦除信道上所有 $(n,M)_q$ 码的集合，给出三类译码准则下平均译码错误概率的显式公式，并进一步推导固定码率情形的渐近错误指数。


<details>
  <summary>Details</summary>
Motivation: 擦除信道中不同译码准则（无歧义译码、列表译码、最大似然译码）会导致不同的错误概率表现。作者希望从全体码集合的平均意义出发，建立可精确计算的性能表达式，并刻画码长趋于无穷时的指数衰减规律。

Method: 研究全体 $(n,M)_q$ 码在擦除信道上的随机编码集合，分别针对无歧义译码、列表译码和最大似然译码推导平均译码错误概率的闭式或显式公式；随后令码字数满足 $M=q^{Rn}$，分析这些公式在大码长极限下的指数阶，得到相应的渐近错误指数。

Result: 论文获得了三种译码原则下平均译码错误概率的显式表达，并给出了所有 $(n,q^{Rn})_q$ 码集合在擦除信道上的渐近错误指数。

Conclusion: 该工作为擦除信道上一般 $q$ 元分组码集合的平均性能分析提供了统一且精确的理论结果，并揭示了不同译码机制在随机码集合平均意义下的渐近可靠性表现。
...
### [148] [Near-Field Physical-Layer Authentication Under Impersonation Attacks](https://arxiv.org/abs/2609.04879)
*Hajar El Hassani,Linda Senigagliesi,Arsenia Chorti*

Main category: cs.IT

TL;DR: 本文研究近场物理层认证中的冒充攻击，证明近场的距离维度可显著提升抗冒充能力。


<details>
  <summary>Details</summary>
Motivation: 远场物理层认证主要依赖到达角区分用户，但近场信道导向矢量同时受角度与距离影响。作者希望明确攻击者在近场条件下能否通过预编码伪造合法用户信号，以及近场距离信息对认证安全性的作用。

Method: 将冒充攻击建模为最小化合法发射端 Alice 与攻击者 Eve 在接收端 Bob 处信号之间的均方误差问题。分别分析单天线 Eve 与多天线 Eve：单天线情形推导最优标量预编码，并在二阶 Fresnel 近似下给出完美冒充条件；多天线情形推导最优预编码向量，并用子空间关系刻画完美冒充条件。

Result: 单天线 Eve 只有在与 Alice 相对于 Bob 具有相同角度和相同距离时才可能完美冒充。多天线 Eve 只有当 Alice 的导向矢量属于 Eve 多天线导向矢量张成的子空间时才可能完美冒充。仿真表明，即便 Alice 与 Eve 具有相同到达角，只要距离不同，近场条件下也能阻止成功冒充。

Conclusion: 近场物理层认证相比远场认证具有额外的距离分辨能力，能够有效增强对冒充攻击的鲁棒性；理论推导与仿真结果一致，表明近场信道结构可作为安全认证的重要资源。
...
### [149] [Minimum Schubert Codewords and Second-Minimum Grassmann Codewords](https://arxiv.org/abs/2609.04916)
*Muskan Khaneja,Prasant Singh*

Main category: cs.IT

TL;DR: 本文解决了 Ghorpade 与 Singh 于 2018 年提出的猜想，完成了 Schubert 码最小重量码字的分类，并刻画与计数了 Grassmann 码的第二最小重量码字。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解 Schubert 码与 Grassmann 码中低重量码字的结构。最小重量与第二最小重量码字直接关系到线性码的距离分布、译码性能和代数几何码的组合结构；其中 Schubert 码最小重量码字的完整分类此前仍依赖一个未解决猜想。

Method: 作者首先对任意有限域大小 q 和任意参数 α，证明并推广 Ghorpade–Singh 猜想，从而给出 Schubert 码 C_α(ℓ,m) 的最小重量码字分类。随后利用该分类结果，将 Grassmann 码 C(ℓ,m) 中第二最小重量码字与外代数元素的结构联系起来，证明其索引元素当且仅当可分解为一个可分解的 (m−ℓ−2)-向量与一个秩为 4 的交替 2-向量的乘积。

Result: 主要结果包括：1）完成所有 q 与所有 α 情形下 Schubert 码最小重量码字的分类；2）给出 Grassmann 码第二最小重量码字的充要刻画；3）进一步枚举 Grassmann 码中第二最小重量码字的数量。

Conclusion: 该论文完善了 Schubert 码低重量码字理论，并将其应用于 Grassmann 码第二重量层的结构刻画与计数，为研究 Grassmann/Schubert 码的重量分布提供了关键基础。
...
### [150] [Algebraic Geometry Codes Approach the Half-Singleton Bound with Constant Field Size](https://arxiv.org/abs/2609.05017)
*Neehar Verma,Camilla Hollanti,Razane Tajeddine*

Main category: cs.IT

TL;DR: 本文提出一个用于分析评价码随机删余在插入/删除错误下性能的通用框架，并将其应用于 Reed--Muller、Reed--Solomon 与代数几何码，证明可在不同域规模条件下逼近 half-Singleton bound。


<details>
  <summary>Details</summary>
Motivation: 插入/删除错误模型下的线性码构造与分析较困难，尤其需要理解结构化评价码在随机删余后能否达到接近最优的编辑距离界。已有工作对随机 Reed--Solomon 等代码的分析依赖较复杂的最长公共子序列论证，且参数依赖仍有改进空间。

Method: 作者从评价码视角建立一般分析框架，将编辑距离控制归约为两个核心量：评价域大小，以及底层函数空间中非零函数的最大零点数。该框架分析评价码的随机 puncturing，并避免对最长公共子序列进行细致分析。随后分别实例化到 Reed--Muller 码、Reed--Solomon 码和来自渐近良好函数域塔的代数几何码。

Result: 该框架推广并简化了 Con、Guo、Li、Zhang（ICALP 2025）的结果。应用于 Reed--Solomon 码时，恢复了随机 Reed--Solomon 码在线性大小域上逼近 half-Singleton bound 的结论，并将关于加性间隙 ε 的依赖从 2^{O(1/ε^2)} 改进为 2^{O(1/ε)}。应用于代数几何码时，得到常数大小域上的随机化结构化线性码族也能逼近 half-Singleton bound。

Conclusion: 论文的核心贡献是给出统一且更简洁的评价码随机删余分析工具，提升了已有 Reed--Solomon 码参数，并扩展到常数域代数几何码，表明结构化线性码在 insdel 错误模型下也可接近理论极限。
...
### [151] [Latency-Optimal Geo-Distributed Storage over Structured Networks](https://arxiv.org/abs/2609.05229)
*Madhura Pathegama,Viveck Cadambe*

Main category: cs.IT

TL;DR: 论文研究地理分布式存储系统中的低延迟文件分配问题：一般情形下当文件数固定且 k≥3 时为 NP-hard，但在树、部分加权环以及高最小度单位权图等拓扑上可高效构造最优分配。


<details>
  <summary>Details</summary>
Motivation: 地理分布式存储中，节点间通信延迟不同，文件如何放置会直接影响用户从各节点检索原始文件的平均时间。作者希望刻画最小化平均检索延迟的文件分配复杂性，并找出可高效求解的网络结构。

Method: 将系统建模为加权图，边权表示通信延迟，每个节点存储一个原始或编码文件；目标是在所有节点与文件的均匀请求下最小化平均检索时间。论文通过从 domatic number 问题归约证明一般问题 NP-hard；同时定义一种结构化的无编码最优分配性质：对每个节点，其包含自身在内的 k 个最近节点可存储互异原始文件，并针对特定图类设计算法。

Result: 证明对任意固定文件数 k≥3，求解延迟最优文件分配是 NP-hard。另一方面，证明所有加权树、某些加权环、以及最小度足够大的单位权图均存在上述结构化无编码最优分配，并给出高效构造算法。

Conclusion: 论文给出了该文件分配问题的复杂性边界：一般图上难以求解，但若网络拓扑具有树形、特定环形或高连通度等结构，则可以利用无编码、局部最近邻互异分配实现全局延迟最优。
...
### [152] [EXAONE Forecast for Finance](https://arxiv.org/abs/2609.04239)
...
TL;DR: EXAONE Finance 是面向金融预测的时间序列基础模型，通过无注意力线性架构、缺失上下文增强和大规模金融语料预训练，在金融预测基准 FinVerse 上取得领先表现。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型多面向通用领域，依赖自注意力导致计算复杂度随序列长度和变量数二次增长；同时通常假设输入完整，且预训练数据难以覆盖金融市场特有动态，因此不适合金融场景中常见的长序列、多变量、间歇缺失面板数据。

Method: 提出 EXAONE Finance：采用 attention-free 架构，用因果一维卷积进行时间混合，用具备分组感知池化的 MLP 进行变量混合；引入 masked context augmentation，使模型在训练中接触连续缺失片段；并在覆盖股票、外汇、商品、加密资产、固定收益和宏观指标的大规模金融语料上预训练。

Result: 在 FinVerse 金融预测基准上，EXAONE Finance 在点预测精度、横截面资产排序和组合盈利能力三个评估层级均排名第一，达到当前最佳表现。

Conclusion: 该报告表明，针对金融时序特点设计的线性复杂度架构、缺失鲁棒训练策略和金融专域预训练数据，能够显著提升金融预测基础模型的实用性与性能。
...
### [153] [From Matching Models to Recruiting Agents: A Systematized Narrative Review of AI Recruitment Systems, Evaluation, and Governance](https://arxiv.org/abs/2609.04286)
*Ziyi Zhao,Guanzheng Wei*

Main category: cs.AI

TL;DR: 本文是一篇关于招聘领域人工智能发展的系统化叙事综述，梳理了从简历匹配与排序到由大语言模型和工具型智能体驱动的多阶段招聘工作流的演进，并强调评估应从单一预测分数转向证据、流程、风险与结果约束下的综合判断。


<details>
  <summary>Details</summary>
Motivation: 招聘AI正在从自动化单点任务转向支持或执行复杂招聘流程，但现有研究在数据有效性、行为标签解释、流程级评估以及风险治理方面存在明显不足。作者希望系统梳理该领域的发展脉络，并明确不同证据层级能够支撑何种强度的结论。

Method: 作者采用系统化叙事综述方法，基于截至2026年7月23日的目的性检索与编码协议，并补充截至2026年9月2日的定向更新，整理40篇代表性研究及相关产业和法律资料；分析三类转变：从相似度到双向适配，从单模型到复合工作流，从离线预测到证据与生产力导向评估。

Result: 综述发现，招聘AI已覆盖文档理解、检索、排序、评估、面试、候选人搜寻和人工交接等环节；证据可分为字段、配对、列表、案例、轨迹和结果等层级。同时，行为标签混杂曝光、偏好与资质，私有及合成数据限制外部效度，最终输出指标掩盖管线失败，且编码集合中未见同时评估效用、公平性、隐私和安全性的研究。

Conclusion: 作者提出从评估证据到可辩护主张的分阶段映射，并倡导构建互惠、基于证据、时间受控、选择性与可审计的招聘AI系统。未来进展应依据系统是否能检索正确证据、保留不确定性、支持可争议决策，并在明确成本与风险约束下改善结果来判断。
...
### [154] [Harbor Adapters and Harbor-Index: Infrastructure and a Curated Meta-Dataset for Large-Scale Agentic Evaluation](https://arxiv.org/abs/2609.04298)
...
TL;DR: 本文提出 Harbor Adapters，一个用于统一评测智能体基准的基础设施，并发布适配器、评测结果与 Harbor-Index。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准数量快速增长，但通常依赖复杂环境和不同智能体集成方式，导致跨基准、跨模型的可靠评测成本高、可比性差。

Method: 作者构建了可适配任意智能体的基准适配器，将 80 多个基准移植到统一评测框架；通过代码审查与一致性实验验证适配正确性；在 54 个基准上评测 8 个不同能力层级模型，并结合 Terminus-2 与 3 种原生 harness；进一步通过难度筛选、AI 与人工审计、审计-修复循环，构建 Harbor-Index。

Result: 完成了大规模模型-基准评测，覆盖 54 个基准；提出包含 82 个高难、多样、高质量任务、跨 29 个基准的 Harbor-Index。所有模型-harness 配置在 Harbor-Index 上通过率均不超过 30%，最强配置 GPT-5.5 with Codex 达到 28.0%。

Conclusion: Harbor Adapters 与 Harbor-Index 能降低智能体评测成本，提高评测覆盖面、可复现性和可比性，为语言模型智能体的可靠、综合评估提供开源基础设施。
...
### [155] [Data-Optimized Contingency Screening: A Machine Learning Approach to Power System Security](https://arxiv.org/abs/2609.04300)
*Joshua Salako,Folajimi Osikomaiya,Olakorede Olamiju*

Main category: cs.AI

TL;DR: 本文利用机器学习对电力系统N-k事故场景进行安全等级分类，随机森林表现最佳，PCA整体贡献大于SMOTE。


<details>
  <summary>Details</summary>
Motivation: 电力系统在扰动或故障情况下需要快速、准确评估安全状态。传统事故分析方法在实时性和可扩展性方面存在局限，因此需要借助机器学习实现对安全、一般和严重事故等级的高效分类，以支持主动决策并降低大范围失效风险。

Method: 研究采用Newton-Raphson潮流计算从IEEE-14和IEEE-30节点系统的N-k事故场景（k=1、2、3）中提取系统数据，并以总体性能指标OPI作为安全度量。预处理阶段使用归一化、SMOTE类别平衡、PCA降维以及SMOTE+PCA组合四种配置。随后训练并比较KNN、随机森林和支持向量机模型，使用precision、recall和F1 score评估性能，重点关注严重事故类别。

Result: 随机森林取得最佳综合表现，在IEEE-30系统上F1分数达到0.97，在IEEE-14系统上达到0.86。SVM在使用PCA后分类准确性显著提升，KNN更适合结合SMOTE和PCA的预处理方式。总体上，PCA对模型性能提升的贡献高于SMOTE；SMOTE可提升召回率，但可能带来更多误报，从而影响准确性。

Conclusion: 机器学习可作为传统电力系统事故分析的可扩展替代方案，用于实时安全评估。随机森林在该任务中最具优势，PCA是关键的有效预处理手段，而SMOTE需在提高严重类别召回率与增加误报之间进行权衡。
...
### [156] [Iris: Climbing to the Search Frontier](https://arxiv.org/abs/2609.04304)
*Ziyuan Liu,Hengqi Liu,Zichuan Wang,Yang Qin,Jiachen Liang,Xu Chu,Shaowei Chen,Yuantao Gu,Mu Chuan*

Main category: cs.AI

TL;DR: 论文提出 Iris-mini 与 Iris-pro 两个开源搜索智能体，并给出从数据构造、SFT、RL 到评测的完整训练流程；在多个复杂搜索问答基准上取得同参数规模开源搜索智能体中的领先结果。


<details>
  <summary>Details</summary>
Motivation: 现有搜索智能体在复杂多跳信息检索任务上依赖数据质量、训练流程与上下文管理，但相关系统往往缺少可复现的数据构造与训练细节；同时，不同系统评测时上下文管理差异会显著影响结果，导致比较不够公平。

Method: 从网页语料的超链接结构反向构造任务：基于种子页及外链抽取实体图，生成多跳推理链，并将非答案实体改写为描述性指代以避免字符串匹配捷径；仅保留闭卷失败但给定证据可解的问题。随后将问题转为搜索轨迹，经轨迹级和轮次级过滤后进行监督微调；再使用实时搜索环境进行强化学习，集群内提供奖励评判器与观察摘要器，并对过长 rollout 进行中断与续接。训练采用 SFT-RL climbing：交替进行 SFT 与 RL，并将每轮 RL 中最难且高效的成功轨迹回流到下一轮监督训练。

Result: 在统一工具、上下文长度和评判器条件下，并分别报告开启/关闭上下文管理的结果。开启上下文管理时，Iris-mini 在 BrowseComp、BrowseComp-ZH、DeepSearchQA、HLE 上达到 82.2/84.8/86.9/52.3；Iris-pro 达到 88.6/85.1/92.9/56.4，成为相应参数范围内开源搜索智能体的最强总体结果。

Conclusion: 论文表明，高质量反向构造的多跳搜索数据、SFT 与 RL 的交替爬升式训练、以及公平控制的上下文管理评测，对提升搜索智能体性能至关重要；作者计划公开模型权重及完整的数据、训练和评测流程。
...
### [157] [A Removal Based Approach to Improve LLM Faithfulness at Test-Time](https://arxiv.org/abs/2609.04343)
...
### [158] [Why Better Models Can Create Riskier Systems: Evidence from LLM Agents in Financial Markets](https://arxiv.org/abs/2609.04373)
...
### [159] [Corporate Language Model (CLM): Transforming Tacit and Fragmented Enterprise Knowledge into a Sovereign, Auditable, and Executable Corporate Intelligence Layer](https://arxiv.org/abs/2609.04377)
...
### [160] [HarvestBench: Measuring Whether LLM Agents Will Pay to Avoid Killing Animals](https://arxiv.org/abs/2609.04444)
...
### [161] [PerfReasoning: How Well Do LLMs Reason on Hardware Performance?](https://arxiv.org/abs/2609.04476)
...
### [162] [When Quantization Breaks Memory: Recurrent-State Write-Back in Low-Precision Temporal Inference](https://arxiv.org/abs/2609.04490)
...
### [163] [ResLearn-XR: Residual Learning for Network Traffic and Quality-of-Experience-Aware Modeling in Extended Reality](https://arxiv.org/abs/2609.04493)
...
### [164] [Rethinking Indirect Prompt Injection as a Test-Time Search Problem](https://arxiv.org/abs/2609.04495)
...
### [165] [BioSync: Transformer-Based Cross-Modal Fusion for a Multimodal Physiological Digital Biomarker](https://arxiv.org/abs/2609.04504)
...
### [166] [What Does Multi-Harness RL Learn? Credit Assignment and Portability in Coding Agents](https://arxiv.org/abs/2609.04518)
...
### [167] [MaxKernel: Agentic Kernel Generation for TPUs](https://arxiv.org/abs/2609.04523)
...
### [168] [Towards a universal language of concepts: A survey](https://arxiv.org/abs/2609.04528)
*Aishni Parab*
...
### [169] [Data-Driven Discovery of Composition-Dependent Constitutive Models for Hyperelasticity and Viscoelasticity of Digital Materials](https://arxiv.org/abs/2609.04541)
...
### [170] [From Answers to Interpretations: Rethinking Ambiguity-Induced Aleatoric Uncertainty Estimation in LLMs](https://arxiv.org/abs/2609.04543)
...
### [171] [IPGeoAI: Transformer-Based Geolocation with LLM Semantic Fusion](https://arxiv.org/abs/2609.04559)
...
### [172] [Reducing Hallucinated Transcripts in Whisper via Hallucination Space Projection](https://arxiv.org/abs/2609.04561)
...
### [173] [La Agente Óptima: Towards Agentic Self-Driving Laboratories](https://arxiv.org/abs/2609.04564)
...
### [174] [Extremely Sparse Supervision Incentivizes Reasoning Ability](https://arxiv.org/abs/2609.04565)
...
### [175] [Does the Selected Object Reach the Reader? Auditing Identity Handoffs in Grounded Language-Model Pipelines](https://arxiv.org/abs/2609.04579)
...
### [176] [$τ^τ$-Bench: An Environment for End-To-End, Realistic Agent Construction](https://arxiv.org/abs/2609.04611)
...
### [177] [Leveraging Imperfect Restoration for Data Availability Attack](https://arxiv.org/abs/2609.04627)
...
### [178] [SiLR: Structure-Preserving Admission and Process Reward for LLM Tool Agents](https://arxiv.org/abs/2609.04629)
...
### [179] [A Cost-Aware Agentic Architecture for NL-to-SQL over Nested Enterprise Schemas, with a New Benchmark](https://arxiv.org/abs/2609.04641)
...
### [180] [Continual Graph Memory for Adaptive Recommendation under Intent Drift](https://arxiv.org/abs/2609.04651)
...
### [181] [Harness-agnostic detection and immunization of reward hacking in self-evolving language models](https://arxiv.org/abs/2609.04665)
...
### [182] [ERPBench: Evaluating LLM Agents for Enterprise Decision-Making Across Competitive Market Ecologies](https://arxiv.org/abs/2609.04667)
...
### [183] [Train What You Deploy:Token-Faithful Post-Training of a Production Coding](https://arxiv.org/abs/2609.04678)
...
### [184] [Predicting Spatiotemporal Mobile Sensing-Based PM2.5 Concentrations Using Low-Rank Adapted Spatially Attentive Graph Neural Network](https://arxiv.org/abs/2609.04693)
...
### [185] [SQL-Zero: Self-Evolving Text-to-SQL](https://arxiv.org/abs/2609.04697)
...
### [186] [Model Retirement Creates Reproducibility Risk in Biomedical AI Publications](https://arxiv.org/abs/2609.04699)
...
### [187] [FinalityBench: An Effect-Level Benchmark for Agent Decisions Under Delayed and Conflicting Financial Finality](https://arxiv.org/abs/2609.04706)
*Abhishek Sharma*
...
### [188] [PLUME: Parameter-Efficient Personalization of Large Language Models via Low-Rank User Modulation in Shared Subspaces](https://arxiv.org/abs/2609.04715)
...
### [189] [Aplaud: Adaptive Personalized Low-Rank Decomposition for User-Specific LLM](https://arxiv.org/abs/2609.04738)
...
### [190] [DCFA: Dual-view Causal-inspired Attribution for Failure Reasoning in LLM-based Multi-agent Systems](https://arxiv.org/abs/2609.04749)
...
### [191] [Shadow Queries for Private Retrieval in Vector Databases](https://arxiv.org/abs/2609.04767)
...
### [192] [Diffusion Language Models for Mobile Edge Agentic AI: Foundations, Applications, and Challenges](https://arxiv.org/abs/2609.04778)
...
### [193] [DODR: Deterministic Operator-Driven Reasoning in Latent Space](https://arxiv.org/abs/2609.04782)
*Weicai Huang*
...
### [194] [ProtLingo: Efficient Protein Language Modeling via Conditional Memory and Expert Routing](https://arxiv.org/abs/2609.04793)
...
### [195] [Whose record is this? Diagnosing and authorizing record use in personalized multimodal models](https://arxiv.org/abs/2609.04801)
...
### [196] [Hierarchical Possession-Aware Graph Pointer Network for Pass Receiver Selection](https://arxiv.org/abs/2609.04803)
...
### [197] [MedFlow: Class-Aware Multi-Scale Generation for Medical Time-Series Synthesis](https://arxiv.org/abs/2609.04804)
...
### [198] [When Financial Fine-tuning Fails: A Three-Level Detectability Analysis of Numerical Hallucination in Domain-Adapted Language Models](https://arxiv.org/abs/2609.04806)
...
### [199] [CPR-IE:A Compression-Prediction-Resource Intelligence Efficiency Metric](https://arxiv.org/abs/2609.04809)
*Xiantao Jiang*
...
### [200] [Long Horizon Transformer Quantile Fault Prediction for Multi Site Industrial Predictive Maintenance](https://arxiv.org/abs/2609.04840)
...
### [201] [ElderBench: Benchmarking Autonomous Mobile Agents for Older Adults](https://arxiv.org/abs/2609.04850)
...
### [202] [MM-IFEval-Pro: A Multilingual and Attack-Resistant Benchmark for Instruction-Following in Vision-Language Models](https://arxiv.org/abs/2609.04859)
...
### [203] [MZ-Rain: Moisture-Budget-Guided Zero-Inflated Model for Station-Level Precipitation Nowcasting](https://arxiv.org/abs/2609.04864)
...
### [204] [CoSkill: Joint Reinforcement Learning of Reasoning and Meta-Skill Agents for Hierarchical Skill Evolution](https://arxiv.org/abs/2609.04865)
...
### [205] [LLM-Assisted Behavioural and Scenario Augmentation for Agent-Based Energy Adoption Models](https://arxiv.org/abs/2609.04866)
...
### [206] [From Interaction Traces to Persistent Skills: Online Evolution for Computer-Use Agents](https://arxiv.org/abs/2609.04869)
...
### [207] [CHAMP: Cross-domain Hybrid Architecture for Matchmaking and Prediction in Online Multi-Player Games](https://arxiv.org/abs/2609.04870)
...
### [208] [AutoLR: Automating the Path from Research to Launch Review in Industrial Recommender Systems](https://arxiv.org/abs/2609.04871)
...
### [209] [MARLA: A Conceptual Scaffold for Regulatory Learning under the EU AI Act](https://arxiv.org/abs/2609.04877)
...
### [210] [Reinforcement Learning for Sequential Solar PV Policy Design under Uncertainty: An Agent-Based Approach](https://arxiv.org/abs/2609.04880)
...
### [211] [From Language Models to World-Acting Systems: Progress and Limits of Agentic AI across Digital, Social, Virtual, and Physical Environments](https://arxiv.org/abs/2609.04894)
...
### [212] [Compact-Memory LLM Agents via Online Max-Member Clustering and Atom-Aware Packing](https://arxiv.org/abs/2609.04915)
...
### [213] [Artificial Intelligence in Equity and Crypto Markets: Progress, Profitability Evidence, and the Limits of Automated Investing](https://arxiv.org/abs/2609.04917)
*Linsen Zhu,Mengqing Cai*
...
### [214] [Solving Hard XAI Queries Based on a Compiled Dual-Rail Encoding](https://arxiv.org/abs/2609.04931)
...
### [215] [Why We Care About Understanding: Competence through Predictive Compression](https://arxiv.org/abs/2609.04962)
...
### [216] [Global to Local: Topology-Preserving Adaptive Graph Pooling via Granular-Ball](https://arxiv.org/abs/2609.04978)
...
### [218] [Language models judge war differently when tested for alignment](https://arxiv.org/abs/2609.05009)
*Maxim Chupilkin*
...
### [219] [TROVE: Adaptive Agent Skill Orchestration via Trace-Grounded Route Validation and Editing](https://arxiv.org/abs/2609.05019)
...
### [221] [Towards Efficient Evaluation of Evolutionary Transfer Optimization: Case Studies on Task-Parameterized Applications](https://arxiv.org/abs/2609.05040)
...
### [222] [MePo++: Unifying Representation Refinement and Reconciliation for General Continual Learning](https://arxiv.org/abs/2609.05075)
...
### [223] [TruthInsightBench: An Evidence-Grounded Benchmark for Automated Evaluation of Open-Ended Scientific Discovery Agents](https://arxiv.org/abs/2609.05079)
...
### [224] [Measuring AI Accountability Through Argumentation Analysis: Can Model Reasoning Withstand Scrutiny?](https://arxiv.org/abs/2609.05088)
...
### [226] [Constructing and Evaluating Clinical Reasoning Trajectories for Medical Agent](https://arxiv.org/abs/2609.05090)
...
### [228] [LLM-Guided Program Evolution for Circle Packing: Breaking 10 Packomania Records for $28](https://arxiv.org/abs/2609.05093)
*Wes Sander*
...
### [232] [Compact Bellman-Grounded Cognitive Maps for Cost-Aware Navigation](https://arxiv.org/abs/2609.05104)
...
### [233] [Unifying ICL, SFT, KL-Regularized RL Through a Bayesian Lens](https://arxiv.org/abs/2609.05111)
*Junxin Fan*
...
### [234] [SciDocBench: A Workflow-Centered Benchmark and Data Pipeline for Scientific Document Understanding](https://arxiv.org/abs/2609.05141)
