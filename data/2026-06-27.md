...
TL;DR: DocArena 是一个面向多模态文档搜索与问答的自动化数据构建流水线，可从原始文档无人工标注地生成搜索智能体训练环境；基于该数据训练的智能体在多模态文档场景与文本问答基准上取得更优表现。


<details>
  <summary>Details</summary>
Motivation: 现有搜索智能体训练通常依赖由问题、答案、证据组成的元组作为环境，但这些环境多为纯文本，难以可控、可扩展地构建，并且不能充分覆盖多模态文档中的视觉与跨页信息。因此，需要一种能自动从真实文档集合中生成高质量、多模态、推理密集型训练环境的方法。

Method: 论文提出 DocArena 流水线：首先利用多模态大语言模型进行视觉感知，对原始文档进行结构化与索引；随后分析跨页面信息分布，构造需要多步检索与推理的问答对；再通过级联式质量保证机制筛选与验证数据。作者构建 DocArena-79K 数据集，包含来自 8,336 份文档、覆盖 16 个领域和 49 种语言的问答对。同时提出 Doc-Search 智能体架构，将视觉感知模块与策略模型解耦，使文本 LLM 能作为多模态文档检索和问答的推理核心。

Result: 在统一评测框架下，实验覆盖六个多模态文档场景和七个文本问答基准。结果显示，使用 DocArena 数据训练的智能体在检索准确率和问答质量上均取得最佳性能。进一步的搜索行为分析表明，该训练环境能够有效塑造并控制智能体的搜索策略。

Conclusion: DocArena 证明了自动化、多模态文档环境构建对于训练搜索智能体的有效性。通过高质量、可扩展、可控的数据生成机制，以及视觉感知与策略推理解耦的智能体架构，该方法提升了多模态文档检索问答能力，并展现出良好的跨场景泛化潜力。
...
TL;DR: 论文提出一种低计算资源的水果新鲜度评估方法：先用图像处理量化腐烂程度，再结合CNN二分类结果训练逻辑回归，最终用逻辑回归将图像处理输出映射为鲜/腐分类。


<details>
  <summary>Details</summary>
Motivation: 水果腐败会造成农业领域显著经济损失，实际应用需要一种实时、低成本、低算力需求且准确的水果新鲜度检测方案。

Method: 研究设计了图像处理算法，将水果腐败程度输出为0到100的百分比分数；同时训练CNN对水果图像进行新鲜/腐烂二分类；再用逻辑回归融合图像处理输出与CNN结果，以提升预测准确率。随后利用该逻辑回归模型校准图像处理算法，使其仅凭百分比输出即可完成二分类，从而在实时应用中不再依赖CNN。

Result: 该方法在苹果和橙子数据集上实现了实时性能，验证准确率超过90%，且不需要高计算资源。

Conclusion: 简单图像处理与机器学习结合可为农业水果新鲜度检测提供实用方案；但当前方法要求水果与白色或透明背景隔离，未来可引入更先进的分割模型实现自动背景去除并提升适用性。
...
TL;DR: 论文提出 LCG，用于长上下文多图文生图，在连续图像序列中提升角色、语义与布局一致性，并通过新数据集 LCCD 验证其优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在单张图像合成上表现良好，但在漫画、分镜和视觉叙事等连续多图生成任务中，容易出现角色外观、语义关系和布局信息随序列推进而漂移的问题；同时，长上下文条件下直接建模多图关系计算成本高、扩展性不足。

Method: 作者提出 Long-Context Generation（LCG）框架。其核心包括：1）Sparse Relational Attention（SRA），选择性关注长视觉上下文中的关键特征，以低成本传播语义与布局信息；2）Routing Consistency Constraint（RCC），利用身份感知掩码对齐不同生成分支中的结构模式，减少多角色复杂场景中的外观漂移；3）构建 Long-Context Consistency Dataset（LCCD），包含 60 万训练序列和 1000 条测试序列，每条序列含 6 到 20 张图像，用于训练和评估长上下文一致性。

Result: 实验表明，LCG 在长上下文多图生成任务中，相比基线方法在提示词对齐和角色一致性方面表现更好，并且能够处理包含多个角色的复杂场景。

Conclusion: 该工作表明，通过稀疏关系注意力和路由一致性约束，可以在保持计算可控的同时提升长序列图像生成的一致性与可扩展性；LCCD 也为该方向提供了较大规模的训练与评估资源。
...
TL;DR: 该研究提出一种无需人工标注的城市/非森林树木冠层级地上生物量制图框架，融合机载 LiDAR 与近红外 RGB 正射影像，在安大略 810 km² 区域生成 2018/2023 双时相冠层级 AGB 数据库。


<details>
  <summary>Details</summary>
Motivation: 城市树木和森林外树木的生物量空间显式量化不足，现有方法多依赖清查数据或粗分辨率产品，难以解析单株树冠与细尺度异质性，限制了管理尺度上的碳储量评估。

Method: 使用 2018 与 2023 年叶落期机载 LiDAR 和高分辨率近红外 RGB 正射影像；以规则生成的伪标签训练双流交叉注意力网络，识别建筑、针叶树和阔叶树；在树木区域内采用多尺度分水岭进行树冠分割，并基于由 21,921 株清查树的物种异速生长方程校准的树冠面积—高度幂律代理模型估算 AGB；同时使用深度集成生成不确定性图。

Result: 语义标注在独立标注测试瓦片上达到 precision 0.86、recall 0.83、Dice 0.84。AGB 预测在使用清查冠层几何时 R²=0.609，在实际分割条件下 R²=0.570，表明树冠分割是主要剩余不确定性来源。30 m 聚合后，2018 年总 AGB 为 1.73 Tg，2023 年为 1.81 Tg，对应碳储量 811–850 Gg C，五年净增 39 Gg C，局部密度最高约 140 Mg ha⁻¹。

Conclusion: 该框架利用标准省级遥感数据，在无人工标注条件下实现管理相关分辨率的冠层级 AGB 制图，可用于森林外树木碳储量监测；主要改进空间在于树冠分割精度与代表性不足地类的不确定性控制。
...
TL;DR: 本文提出一种用于激光深熔焊在线质量评估的多任务深度学习模型，可基于焊池图像与焊接参数同时预测熔透状态、熔深和焊缝截面形貌，并在测试集上取得较高精度。


<details>
  <summary>Details</summary>
Motivation: 激光深熔焊中，熔透状态和焊缝形貌直接决定焊接质量；传统检测方法难以实现高效、准确的原位在线监测，因此需要一种能够融合过程视觉信息与工艺参数的智能评估方法。

Method: 研究利用CMOS相机采集焊接过程中的顶部焊池图像，并结合焊接参数，构建基于卷积神经网络与状态空间模型的多任务深度学习框架，以提取和处理空间—时间特征；同时提出数据集构建方法以提升模型鲁棒性和泛化能力。

Result: 测试集验证显示，模型对熔透状态的预测准确率达到99.35%，熔深预测误差为1.79 mm，焊缝横截面重构准确率达到95.65%。

Conclusion: 该方法能够较准确地实现激光深熔焊过程中的多维质量预测，为焊接系统的原位质量控制和智能化监测提供了新的技术思路。
...
TL;DR: 本文将基于遮蔽 fMRI 重建的认知任务分类从单源迁移扩展到多源迁移，并用布尔整数规划分析监督预算受限下的任务分配。


<details>
  <summary>Details</summary>
Motivation: 既有重建式 fMRI taskonomy 多关注单一源任务到目标任务的一对一迁移，难以刻画多个任务状态共同支持目标任务时的多对一关系，也缺少在有限监督预算下如何选择任务的全局优化分析。

Method: 作者基于 Human Connectome Project 的 23 个任务状态，训练 1,127 个任务特异模型与迁移模型；使用遮蔽 fMRI 重建作为自监督目标来量化任务间迁移关系，并进一步采用布尔整数规划进行预算约束下的任务监督分配分析。

Result: 单源迁移呈方向性并受实验范式结构影响：运动任务在运动范式内部迁移较好，但对多数非运动目标支持有限。多源迁移强烈依赖源任务组合，说明成对 taskonomy 不能完全解释多对一关系。在不同监督预算下，BIP 反复将直接监督分配给若干 0-back 与 2-back 工作记忆状态，尽管这些状态并不总是最强单一源任务。

Conclusion: 研究揭示了跨范式迁移受限的运动任务簇，以及在全局任务分配目标下具有高优先级的工作记忆状态；该工作把重建式 fMRI taskonomy 推进到多源迁移和预算约束任务依赖分析。
...
TL;DR: 本文提出 GeMoE，一种基于门控熵的不确定性感知自适应路由方法，用于 MoE 架构的大型视觉语言模型，在基本保持性能的同时显著提升专家激活稀疏性。


<details>
  <summary>Details</summary>
Motivation: 传统 MoE 通常采用 Top-k 静态路由，无法根据不同输入 token 的复杂度自适应选择专家数量，导致计算资源利用不充分；作者希望在保持 LVLM 性能的同时降低不必要的专家激活。

Method: 作者将 token 路由视为信息编码任务，并把动态路由形式化为最小描述长度（MDL）问题；通过建立 MDL 与 MoE 门控熵之间的联系，使用 gating entropy 衡量 token 复杂度，并据此动态决定每个 token 需要激活的专家数量。

Result: 在多种 backbone 和 benchmark 上，GeMoE 相比原始静态路由保留了 99.5% 的平均性能，同时将平均专家激活稀疏性提升了 36.5%。

Conclusion: GeMoE 提供了一种理论驱动的动态 MoE 路由机制，能够显式平衡模型复杂度与任务性能，在 LVLM 中实现更高效的专家使用。
...
TL;DR: 本文提出真实浑浊水下图像数据集 TUB 与基于相位一致性的 PCD 指标，用于评估浑浊环境下结构信息损失及其对实例分割模型性能的影响。


<details>
  <summary>Details</summary>
Motivation: 水下浑浊环境会显著降低可见性，但其对计算机视觉模型的具体影响尚不清楚；现有研究较依赖合成浑浊数据，可能无法真实反映现实场景中的信息损失。

Method: 作者构建包含 1,320 张极端浑浊条件下真实采集图像和超过 16,000 个高置信度分割标注的 TUB 数据集；同时提出 PCD 指标，该指标由相位一致性图推导而来，具有对比度不变性，用于刻画真实浑浊导致的结构信息损失。

Result: 实验表明，PCD 与实例分割模型在真实和合成浑浊图像上的性能高度相关；相比之下，领域内常用指标与模型性能的相关性较弱或几乎不存在。

Conclusion: TUB 数据集为真实浑浊水下视觉研究提供了基准，PCD 可作为比传统指标更有效的图像退化/结构信息损失度量，并能更好预测视觉模型在浑浊环境中的表现。
...
TL;DR: 本文研究视觉提示调优中图像 token 与提示 token 的融合方式，提出通过可微架构搜索自动选择混合融合策略，在多个数据集上优于现有提示调优基线，并取得较好的精度、延迟与参数量权衡。


<details>
  <summary>Details</summary>
Motivation: 现有视觉提示调优通常采用拼接或相加等单一融合方式，但尚不清楚哪种融合方式更优，以及不同层是否应采用不同融合策略。作者希望系统研究提示与图像 token 的融合机制对 ViT 迁移性能的影响。

Method: 作者将提示学习与融合策略选择建模为双层优化问题，并使用可微架构搜索联合优化可学习提示和融合架构。搜索空间包含拼接、相加，以及新增的仿射变换和交叉注意力四类融合方式，从而为不同层选择更合适的融合方案。

Result: 在 VTAB-1k、FGVC 和 HTA 共 34 个数据集上的实验显示，该方法相较多种 prompt-tuning 基线具有稳定性能提升。在冻结 ViT 主干的条件下，相比 VPT-Deep 及近期变体，在准确率、推理延迟和参数效率之间取得更优折中。

Conclusion: 提示 token 与图像 token 的融合方式是影响视觉提示调优效果的重要因素。混合融合策略能够更好利用 ViT 不同层的语义特征，为视觉提示调优提供了新的研究视角。
...
TL;DR: 本文提出用冻结的通用自监督表征加线性探针来审计深度伪造检测基准，发现其性能接近专用检测器，说明许多基准可能主要衡量通用模态理解而非真正的取证理解。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测器在公开基准上表现良好，但在真实场景中常失效；基准可能诱导复杂工程化模型，却未必反映真实威胁。因此需要先判断这些基准到底在测量什么。

Method: 作者在视频、图像、音频三类深度伪造基准上进行审计：使用冻结的通用自监督表示，仅训练线性探针，并将其与专门设计的深度伪造检测器比较；同时分析生成器层面的难度是否可由同一表征空间中的 Frechet 几何解释。

Result: 跨三种模态，基于通用自监督表征的线性探针能够接近专用检测器性能；不同生成器造成的检测难度差异也可部分由该表征空间中的 Frechet 几何解释。

Conclusion: 高基准分数不应直接被解释为模型具备真实的取证理解；在评价深度伪造检测器前，应先审计基准中有多少性能可由通用表征能力解释。
...
TL;DR: 该论文提出 VIGIL，一种用于多模态大语言模型对齐的强化学习后训练框架，通过增强回答与视觉输入之间的因果关联来减少视觉幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 虽然具备视觉感知与语言推理能力，但在生成时常过度依赖语言先验，忽视已编码的视觉证据，产生与图像矛盾的幻觉。传统偏好优化方法多基于文本结果奖励，容易强化语言捷径，缺乏对视觉 grounding 的直接约束。

Method: VIGIL 将对齐目标从单纯数值奖励拟合转向因果视觉 grounding，引入几何约束以最大化视觉输入与生成回答之间的互信息。具体做法是构造反事实“盲态”：屏蔽文本-视觉注意力，并惩罚模型在缺失视觉证据时仍保持不当高置信度的“blind confidence”现象。

Result: 实验表明，VIGIL 在幻觉抑制与推理基准上持续优于近期对齐方法，同时不损害纯文本能力。它仅使用 25% 偏好数据即可达到现有最优方法的全数据性能，并在无显式边界框监督下展现出涌现的空间 grounding 能力。

Conclusion: VIGIL 证明了显式强化视觉信息增益与因果视觉依赖，比仅优化文本偏好奖励更能缓解 MLLM 的视觉幻觉问题，并提升数据效率与空间理解能力。
...
TL;DR: DinoLink 是一种面向 V2X 车辆-云协同推理的语义压缩框架，用离散语义 token/索引替代原始像素传输，在显著降低码率的同时保持较好的远程感知精度。


<details>
  <summary>Details</summary>
Motivation: 高精度远程感知依赖车辆与云端之间的数据传输，但 V2X 网络带宽受限，直接传输原始图像/像素会造成高码率、低时效和部署困难。因此需要一种在窄带环境下仍能支持高保真感知的高效通信表示。

Method: 论文提出 DinoLink：采用 token-centric compression 思路，将视觉特征转换为离散语义通信内容。其核心是双稀疏架构：1）saliency-aware selector 根据显著性裁剪冗余背景 token；2）Residual Vector Quantization（RVQ）将特征压缩为紧凑码本索引。系统仅传输轻量化索引与位置先验，用于云端协同推理。

Result: 相较未压缩传输，DinoLink 实现 139× 码率降低；在 nuScenes 数据集上保持 32.8% mAP；在 LoRa 等窄带部署仿真环境中获得 34.5× 加速。

Conclusion: DinoLink 表明，以离散语义 token 为中心的压缩通信能够显著缓解 V2X 带宽瓶颈，并在受限网络条件下支持具有竞争力的远程感知性能，是一种高效、鲁棒的车辆-云协同感知前端方案。
...
TL;DR: 本文提出一种从单目视频监督信号中自监督学习隐式三维物理动态的框架，将预测瓶颈从二维图像空间转移到三维体素潜空间，以提升物理一致性和长期稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有生成式视频模型虽然视觉质量高，但缺乏三维几何基础，容易出现物理不一致、物体永久性不足等问题；而依赖显式物理模拟器的混合方法又需要引擎状态、标签或代理模型，扩展性受限。

Method: 方法利用 V-JEPA 提取视频语义特征，并结合单目深度先验将其反投影到体素化三维网格中，形成 lifted 3D Volumetric Latent Space；随后通过 Volumetric Feature Advection 学习动作条件化的状态转移算子，将物理动态建模为空间—时间状态平流问题，在高维特征中隐式跟踪材料状态。

Result: 在 CLEVERER、PhysInOne、PhysGaia 等基准上，模型在仅使用端到端视频监督与动作条件、且不访问物理引擎内部状态或标签的情况下，表现出较好的长期结构稳定性与物理合理性，并可统一模拟刚体运动、流体流动等异质现象。

Conclusion: 该工作表明，通过单目视频的被动观察学习三维体积潜表示与隐式动态转移，有望构建可扩展的通用动态世界模型，使模型内化物理世界中的三维不变量。
...
TL;DR: 本文提出一种用于遥感影像甲烷羽流分割的高效多模态深度学习模型，通过特征引导的甲烷增强机制提升检测精度，并在较低计算成本下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 准确识别与分割地球观测影像中的甲烷羽流对于大规模温室气体监测和气候影响评估具有重要意义，但现有方法在精度、效率以及物理信息融合方面仍存在不足。

Method: 提出融合RGB表征与物理意义甲烷线索的多模态模型，引入Feature-Guided Methane Enhancement（FGME）机制，在多个语义尺度将甲烷相关特征注入基于Transformer的RGB特征表示中，实现更有效的跨模态融合。

Result: 在MPDataset上取得优于当前最先进方法的表现：MIoU提升+0.92，MPrecision提升+0.87，Recall提升+1.01，同时计算成本显著低于其他高性能架构。

Conclusion: 高效的多模态融合策略能够在保持较低计算开销的同时提升甲烷羽流分割性能，适用于真实遥感场景中的大规模、可扩展甲烷监测。
...
TL;DR: APRTrack 是一个面向 RGB-Event 目标跟踪的分层扰动与检索框架，通过模拟模态失效和局部目标缺失，并结合基于历史记忆的可靠特征补偿，提高复杂退化场景下的跟踪鲁棒性。


<details>
  <summary>Details</summary>
Motivation: RGB-Event 跟踪融合 RGB 外观纹理与事件相机的高时间分辨率运动线索，具备较强定位潜力；但真实场景中常出现模态可靠性下降、遮挡、边缘截断、前景干扰等结构化退化，导致传统多模态融合方法性能受限。

Method: 方法包括两类对抗扰动分支：模态级扰动用于模拟完整模态失效，空间级扰动用于模拟局部目标区域缺失；设计分层路由机制分离两类扰动的训练路径，避免退化约束叠加导致特征坍塌；提出 Footprint-guided Channel-calibrated Hopfield Retrieval（FCHR），基于查询与记忆库的关联足迹评估检索置信度，并在 Hopfield 匹配前校准通道检索度量空间，实现受目标区域约束的历史特征补偿。

Result: 在 FE108、COESOT、VisEvent 和 FELT 数据集上的实验表明，所提出的扰动建模、分层路由与 FCHR 历史补偿策略能有效提升 RGB-Event 视觉目标跟踪性能。

Conclusion: APRTrack 针对多模态退化与目标局部缺失问题提供了系统性解决方案，在复杂真实场景下增强了 RGB-Event 跟踪的鲁棒性与可靠性，并计划开源代码和预训练模型。
...
TL;DR: 本文提出 Frame Forgetting Network（FFN），用于长视频场景下的测试时训练，通过仅利用滑动窗口中的三个关键帧并引入“惊讶度”指标来自适应调整窗口，显著降低计算成本并保持长时序上下文建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有测试时训练方法在长视频上存在两类主要问题：其一，基于滑动窗口对过去帧进行更新，计算量随窗口长度线性增长，难以扩展到数小时视频；其二，即使相邻帧高度相似仍频繁执行 TTT，造成不必要的计算浪费。

Method: 作者提出 FFN，只处理滑动窗口中的三个帧：即将离开窗口的帧、当前帧以及下一帧，以此在低计算量下保留时间上下文。同时定义数学化的 surprise metric，用于衡量新输入帧相对于历史帧包含的新信息量，并据此设计自适应窗口算法，动态决定 TTT 的有效窗口大小。

Result: FFN 在密集分割、视频分类任务上表现有效，并展示了对深度估计任务和多小时长视频的泛化能力。作者还构建了 EpicTours 数据集，包含最长约 3 小时的步行城市游览视频，显著长于以往约 5 分钟的数据集。

Conclusion: 该工作表明，通过帧遗忘机制与基于惊讶度的自适应窗口策略，TTT 可以更高效地应用于长视频推理，在减少计算开销的同时维持甚至提升多类视频理解任务的性能。
...
TL;DR: 本文提出 CRISP，一种用于评估视觉语言模型空间智能的结构化诊断范式，通过一致性来区分真实空间推理与语言先验造成的表面正确。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 评测常将语言先验与真正的视觉空间推理混淆，难以判断模型是基于图像结构理解作答，还是依赖数据偏置与语言猜测。作者希望构建一种能分离感知瓶颈与推理能力的细粒度评估方法。

Method: CRISP 使用度量化 3D 场景图和 oracle intervention 协议，考察模型隐式感知与显式推理之间的一致性；通过结构化诊断而非传统黑盒问答，分别评估模型的度量估计、结构表征利用和多跳组合推理能力。

Result: 研究发现存在系统性的感知—推理脱节：闭源/商业模型通常具备较强的潜在推理能力，但度量估计不准确，且不能充分利用其隐式结构表征；开源模型则主要受限于多跳组合推理能力不足。

Conclusion: CRISP 将评测重点从“答对问题”转向“感知、验证与推理”的一致性诊断，为超越端到端后训练的多模态对齐和空间智能评估提供了更严格的路线图。
...
TL;DR: 论文提出 PhyEditBench，用于评估指令式图像编辑模型的物理推理能力，并提出无需训练的 PhyWorld 基线，显示现有 SOTA 在物理理解上仍有明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有指令式图像编辑基准主要关注语义一致性、视觉质量或指令遵循，缺乏对真实世界物理规律与动态因果关系的系统评估；而物理推理是模型在真实场景中可靠编辑图像的关键能力。

Method: 作者构建了一个层次化物理分类体系，包含 4 个主类和 12 个子类；从视频中抽取 238 个高质量、高分辨率真实实例以保留真实物理动态，并加入 35 个合成的反物理样例。随后评估当前 SOTA 图像编辑方法，并提出训练自由的 PhyWorld：通过测试时扩展与潜变量约简策略，利用视频生成过程作为图像编辑中的物理推理机制。

Result: 实验表明，现有先进图像编辑模型在物理推理和物理一致性编辑方面存在显著局限；PhyWorld 相比可比模型取得更好表现，说明视频生成过程能够为图像编辑提供有效的推理信号。

Conclusion: 该工作补足了图像编辑评测中物理理解维度的缺口，PhyEditBench 可作为衡量物理一致性编辑能力的重要基准；同时，PhyWorld 证明了无需额外训练、借助视频生成进行测试时推理是一条有潜力的方向。
...
TL;DR: 本文提出 ForeAgent，一个用于 AI 生成图像检测的智能体取证框架，通过多视角线索融合与迭代自我进化提升检测准确率与推理质量。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造/AI 生成图像检测方法面对高真实感生成图像时表现受限；现有 MLLM 方法对细粒度取证伪迹不够敏感，并依赖来自前沿模型的静态合成监督，导致适应性不足且成本较高。

Method: ForeAgent 采用 Perception-Verdict 架构：感知模块聚合语义、空间与频域等多视角线索，判决模块利用 MLLM 融合证据并生成逻辑化判定。进一步提出 Hindsight-Driven Self-Refining 策略，遵循 Sampling-Reflection-Evolution 范式：在训练样本上进行推理 rollout，借助真实标签进行事后反思，重生成更高质量推理轨迹，并通过双专家质量门控筛选后用于持续微调。

Result: 在 Chameleon benchmark 上达到 82.18% 准确率，相比 AIDE 提升 16.41%；在 AIGCDetect-Benchmark 的 16 个生成器上达到 93.3% 平均准确率；外部评估显示其推理比 GPT-5 和 GPT-5-mini 更一致且更具因果依据。

Conclusion: ForeAgent 通过多域取证线索融合与自我反思式数据迭代，有效提升 AI 生成图像检测性能与解释可靠性，展示了智能体化 MLLM 在视觉取证任务中的潜力。
...
TL;DR: 本文提出一种用于非刚性三维形状匹配的混合自监督方法，通过粗到细策略和双分支函数映射学习，在复杂变形与拓扑噪声场景下实现高效且先进的匹配性能。


<details>
  <summary>Details</summary>
Motivation: 非刚性三维形状匹配是计算机视觉与图形学中的基础问题，但在非等距形变、拓扑噪声等挑战条件下，现有方法往往难以同时保证匹配精度、鲁棒性与计算效率。论文希望通过自监督学习与多基函数表示提升复杂场景下的对应关系质量。

Method: 方法采用粗到细框架：先生成粗粒度映射，再通过 refinement module 细化对应关系，并约束二者保持一致。网络为双分支对称结构，包含两个函数映射学习流：一个基于 Laplacian basis，另一个基于 elastic basis。同时引入对比能量以增强特征判别性，并可与已有方法集成。

Result: 大量实验表明，该方法在保持计算效率的同时，在多种挑战场景中达到当前先进性能，尤其适用于非等距形变和拓扑噪声。论文还展示了对比能量能够促进特征区分，并且与现有方法结合后可带来稳定性能提升。

Conclusion: 该工作证明了粗到细自监督策略、Laplacian/elastic 双基函数分支以及对比能量的结合，对非刚性三维形状匹配具有显著效果，能够提升鲁棒性、精度和泛化能力。
...
TL;DR: SpaceRipple 是面向地球观测卫星网络的轻量级语义传输与星上处理框架，通过协同压缩、转发、恢复和语义推理，在带宽受限条件下减少数据传输并提升任务相关语义检测效果。


<details>
  <summary>Details</summary>
Motivation: 地球观测卫星产生大量高分辨率图像，但星间链路和下行带宽有限；许多时敏任务更需要任务相关语义信息而非完整原始图像，因此需要从像素级传输转向语义导向传输。

Method: 提出 SpaceRipple 框架：感知卫星执行自适应压缩与元数据生成以降低星间传输负载；边缘计算卫星对接收表示进行恢复并提取任务相关语义信息；整体联合协调压缩、转发、图像恢复和语义推理，并引入压缩感知的 MoE 增强模块以提升退化视觉输入下的鲁棒性。

Result: 实验显示该方法获得较好的图像重建质量，提升语义检测性能，并显著节省带宽。

Conclusion: SpaceRipple 表明，在卫星网络资源受限场景下，面向任务的语义交付比传统保真度驱动的完整图像传输更高效且可靠，适合时敏地球观测任务。
...
TL;DR: 论文提出 DiCoBench，一个面向高分辨率多图像细粒度感知的基准，用于评估 MLLM 自主发现跨图像隐式视觉差异与共性的能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型评测多依赖显式文本线索或低分辨率图像，难以衡量模型在高分辨率场景中自主捕捉隐式、微尺度视觉线索的能力，尤其缺少跨图像细粒度感知评估。

Method: 构建包含 765 个精心标注样本的 DiCoBench，图像分辨率接近 2K，采用多项选择题形式以减少评估指标偏差；基准分为 Differential Visual Cues 与 Commonality Visual Cues 两个递进轨道，覆盖 8 类感知任务，并评测 18 个不同 MLLM。

Result: 实验显示当前先进 MLLM 与人类准确率 98.3% 存在显著差距，表现最好的模型在微尺度细节捕捉方面仍明显不足。

Conclusion: DiCoBench 为高分辨率、多图像、自主细粒度视觉感知提供了更具挑战性的测试平台，有助于推动未来 MLLM 在跨图像细节理解方面的研究。
...
TL;DR: 本文提出 LogicIR：首个面向图像复原任务的全逻辑门网络，在显著降低计算成本的同时保持较强复原性能。


<details>
  <summary>Details</summary>
Motivation: 图像复原模型的计算需求持续上升，轻量化与高效推理成为重要方向；逻辑门网络具备基于 NAND、XOR 等基本逻辑操作的高效计算潜力，但在图像复原领域尚未被充分探索。

Method: 构建受 UNet 启发、完全由逻辑门组成的 LogicIR 架构；引入可微分比特解码层以支持训练与输出重建；设计索引洗牌机制以增强逻辑门之间的信息传播。

Result: 在多个图像复原基准测试上，LogicIR 取得了有竞争力的性能，并显著降低计算成本。

Conclusion: LogicIR 证明了逻辑门网络可作为图像复原任务中高效、轻量的可行替代方案，为低成本图像复原模型设计提供了新方向。
...
TL;DR: TaskTok 通过只选择性修复与下游任务相关的潜在 tokens，在提升分类、分割、检测等任务性能的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统图像复原偏重人眼感知质量，而任务驱动图像复原关注提升高层视觉任务性能。现有基于生成先验的方法常 indiscriminately 更新全部 latent tokens，导致计算低效，并可能改变语义信息。作者观察到任务相关视觉线索在 token 序列中分布不均，存在按索引的功能特化，因此没有必要修复所有 tokens。

Method: 论文提出 TaskTok 框架：首先通过潜在 token 空间分析识别任务相关信息的非均匀分布；随后引入可学习的 token switch 选择对下游任务有用的 token 子集；再使用轻量级 token refinement module 仅对被选中的任务相关 tokens 进行复原/细化，从而避免全量 token 更新。

Result: 在图像分类、语义分割和目标检测等任务上的大量实验表明，TaskTok 能显著提升下游任务表现，同时保持较高计算效率。

Conclusion: 任务驱动图像复原不必追求全图或全 token 的统一复原；通过识别并精修任务相关 tokens，可以在性能与效率之间取得更优平衡，并降低语义被不必要修改的风险。
...
TL;DR: 论文提出 Position Rebinding Cache Reuse（PRCR），通过对历史视觉 KV 缓存进行位置重绑定，实现无需 token replay 的高效视觉证据重访，并在多模态推理任务上达到或超过 replay 级性能。


<details>
  <summary>Details</summary>
Motivation: 交错式多模态推理需要在多步生成过程中反复回看视觉证据。现有方法通常通过重复前向传播选中的视觉 tokens 来实现，计算成本较高。直接复用历史视觉 KV cache 看似可行，但作者发现其存在“陈旧位置绑定”问题：缓存中的视觉 key 已与原始位置上下文绑定，在后续解码位置下直接注入会扭曲注意力，甚至导致自回归解码崩溃。

Method: 提出 PRCR 框架：保存原始视觉 KV cache 及其空间坐标；在需要视觉重访时，为选中的缓存条目重新分配与当前解码状态兼容的位置坐标；对视觉 keys 进行位置重绑定后，再注入当前活跃 decoder cache。该方法避免重复视觉 token 前向传播，同时保持文本位置连续性和视觉相对空间结构。

Result: 在多个多模态推理基准上，PRCR 达到或优于 token replay 的性能，平均准确率提升约 5%，并将视觉重访相关计算开销最高降低数万倍。

Conclusion: 直接复用历史视觉 KV cache 会因位置不匹配导致注意力异常；有效的缓存复用需要在当前解码上下文下重构视觉证据。PRCR 通过位置重绑定实现了高效且稳定的 replay-free 视觉重访，为多模态大模型的长程推理和高效推理提供了实用方案。
...
TL;DR: 本文提出 FAROS，一种基于光流引导的标签插值框架，用稀疏关键帧标注生成时序一致的密集伪标签，从而改善手术场景多任务学习中的监督不均衡问题。


<details>
  <summary>Details</summary>
Motivation: 手术场景理解中的多任务学习面临标注粒度不匹配：阶段、步骤、预测等时序任务通常有密集帧级监督，而器械分割、动作识别等空间/像素级任务因标注成本高只在少量关键帧上标注。这种监督不平衡削弱共享表征学习与联合优化效果。

Method: 作者提出 Flow-guided Annotation for Robust Operating Scenes（FAROS），结合零样本分割驱动的掩码传播与光流估计，在遮挡、烟雾、运动模糊等复杂手术条件下，将稀疏关键帧标注插值为时序一致的密集伪标签。随后将生成的器械掩码和动作标签整合到统一的 Transformer 多任务框架中，联合学习手术阶段识别、步骤识别、手术流程预测、器械分割和动作识别。

Result: FAROS 首先在 DAVIS 2017 基准上以稀疏真值协议验证了标签插值质量，表明其具有跨领域的稳健传播能力。随后在 GraSP、MISAW 和 AutoLaparo 数据集上的实验显示，该方法显著提升了跨任务表征学习效果，并改善了多个时空任务的整体手术场景理解性能。

Conclusion: FAROS 通过将稀疏空间标注扩展为密集时序伪标签，有效缓解手术多任务学习中的监督粒度不匹配问题，使时序任务与空间任务能够更均衡地联合优化，从而提升整体手术场景理解能力。
...
TL;DR: FracEvent 是一种面向事件相机的仿真器，通过分数阶松弛电压动力学刻画像素级生命周期，从而生成时间结构更真实的事件流，并提升下游任务迁移效果。


<details>
  <summary>Details</summary>
Motivation: 真实事件相机数据难以大规模采集，现有仿真器多基于对比度阈值及手工噪声/滤波模型，往往简化像素电压响应的时间动态，导致事件时序失真并影响下游模型从仿真到真实数据的迁移。

Method: 论文提出 FracEvent：给定对数强度轨迹，使用一组紧凑的松弛模式建模像素电压的分数阶松弛动态；将各模式响应组合成连续电压状态；在连续电压轨迹上定位阈值穿越以触发 ON/OFF 事件；事件触发后更新参考值，同时保留底层记忆模式，使残余电压状态继续影响后续事件时序。

Result: 在事件流相似性比较以及图像重建、光流估计等下游迁移任务上，FracEvent 在多个数据集上优于其他事件相机仿真基线，生成的事件具有更好的时间结构，并带来更强的下游迁移性能。

Conclusion: FracEvent 表明，对像素级生命周期及其记忆效应进行更细致的动态建模，能够显著改善事件相机仿真的时序真实性，并提升仿真数据在事件视觉任务中的实际价值。
...
TL;DR: 本文提出 LayersReg，一种用于 3D/2D 医学图像配准的渐进式回归框架，通过逐层搜索特征相关性并感知三维解剖结构，在大偏移和多模态条件下实现高精度、实时配准。


<details>
  <summary>Details</summary>
Motivation: 传统迭代优化式 3D/2D 配准效率低、术中失败率高；现有深度学习方法虽将配准转化为位姿回归并提升速度，但往往依赖对特定位姿特征的记忆式检索，缺乏对图像对齐过程本身的理解，导致复杂场景下泛化能力受限。

Method: 提出 LayersReg 回归范式：赋予模型 3D 解剖感知能力，并以逐层渐进方式搜索正确位姿。该方法借鉴经典配准中的迭代位姿搜索准则，在特征空间中建模移动图像与固定图像之间的相关性，捕获像素流变化趋势，逐步收敛到正确空间变换；同时引入节点级回归与渐进配准框架的耦合，以增强模型对空间位姿变化的感知。

Result: 在大位姿偏移和多模态条件下，LayersReg 在 X-ray/CT 配准任务中达到 0.68°、1.41 mm 的精度，在切片定位任务中达到 0.73°、1.55 mm 的精度，优于现有先进方法，并满足术中对精度与实时性的要求。

Conclusion: LayersReg 通过将深度回归与渐进式位姿搜索思想结合，提升了 3D/2D 配准模型的可解释对齐能力、泛化能力与实时性能，是面向术中导航场景的有效配准方案。
...
TL;DR: 本文提出多概念视频定制任务与Disco-LoRA框架，用于在文本到视频生成中同时控制内容、风格和运动。


<details>
  <summary>Details</summary>
Motivation: 现有T2V定制方法在图像风格化或视频运动定制方面已有进展，但难以同时对内容、风格、运动等多个概念进行联合、可组合且低干扰的控制。

Method: 作者定义多概念视频定制任务并构建基准；提出Disco-LoRA，两阶段处理：先将问题拆分为Content-Style与Content-Motion两个子任务，并用迭代式双LoRA解耦框架分离概念；再利用基于Z-score的统计正则化对齐LoRA权重分布，保留层级权重趋势并降低不同LoRA组合时的干扰。

Result: 实验表明Disco-LoRA在多概念视频定制中表现优越，能够较好保持外观、风格与运动特征，实现更可控的文本到视频生成。

Conclusion: 该工作为多概念视频定制提供了任务定义、评测基准与统一解决框架，证明通过LoRA解耦和统计正则化可以提升多概念组合生成的可控性与一致性。
...
TL;DR: 论文提出 DeCoFlow，用于无历史数据的连续异常检测，在 Normalizing Flows 中通过冻结通用基座与任务特定低秩适配器隔离参数更新，以避免灾难性遗忘并保持可逆性。


<details>
  <summary>Details</summary>
Motivation: 工业场景中产品类别持续到来，模型需要在无法访问旧数据的情况下持续学习异常检测任务。现有 Normalizing Flows 虽能精确密度估计，但跨任务更新会扭曲密度流形并导致灾难性遗忘；直接参数隔离又必须满足 NF 的严格可逆性与雅可比有效性约束。

Method: 作者利用仿射耦合层在子网络参数化变化下仍能保持变换有效性的性质，提出 DeCoFlow：将耦合层子网络分解为冻结的通用基础模块和任务特定低秩适配器，实现任务间更新隔离。同时引入 Task-Specific Alignment、Auxiliary Coupling Layers 和 Tail-Aware Loss，以缓解冻结基座带来的表达刚性并增强尾部分布建模。

Result: 在 MVTec-AD 上达到 98.40% 图像级 AUROC，在 VisA 上达到 93.00% 图像级 AUROC；在正确任务路由下实现参数级零遗忘，FM 为 0.00%，且每个任务仅需 2.27M 参数。

Conclusion: DeCoFlow 在保持 Normalizing Flows 可逆性和雅可比合法性的同时，通过任务特定低秩适配实现连续异常检测中的高性能、低参数开销和零遗忘，展示了 NF 在工业持续学习异常检测中的有效性。
...
TL;DR: PhysEditWorld 是一个基于 UE5 的多模态游戏世界数据集，通过固定动作轨迹并在不同重力配置下重放同一场景，为可控物理编辑与世界模型训练提供显式标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有游戏世界模型虽能生成视觉上合理、受动作条件控制的 rollout，但交互行为多停留在探索或游荡层面，物理动态主要从数据相关性中隐式学习，缺乏对重力等物理规则的显式可控建模。这限制了其在人工设计游戏环境中的应用。

Method: 作者提出 PhysEditWorld 数据集，采用 UE5 replay-and-rendering 管线：记录标准化动作轨迹，并在相同初始状态、角色控制器、动作序列和相机策略下，使用多种重力配置重复渲染，从而获得可归因的物理变化。数据包含 RGB、深度、法线、音频、动作轨迹、相机轨迹、引擎状态、语义标注和重力标签等多模态信号。

Result: 数据集包含 12 个电影级 UE5 场景、超过 100 小时游戏交互和 6000 多万帧渲染 rollout。初步实验表明，该数据可提升生成视频模型对重力一致动态的建模能力，增强物理编辑下的一致性，并支持世界理解模型学习显式物理因素。

Conclusion: PhysEditWorld 为可控世界建模提供了一个规模化、多模态、带显式物理参数标签的数据基础，尤其适用于研究重力编辑、物理一致视频生成和可解释游戏世界模型。
...
TL;DR: 该论文提出一个面向颅内动脉瘤的多任务自动分析框架，可在多模态影像中同时完成解剖位置多标签分类、动脉瘤多类别分割和血管多类别分割，并在RSNA 2025挑战中取得第2名。


<details>
  <summary>Details</summary>
Motivation: 颅内动脉瘤破裂前常无症状，但破裂死亡率高；临床风险评估与治疗规划不仅依赖是否检测到动脉瘤，还依赖其形态特征和解剖位置。现有自动化方法多停留在二分类检测层面，缺乏精细解剖位置分类和多类别分割能力。

Method: 作者设计两阶段多任务框架：首先采用快速2D三轴向ROI提取方法定位候选区域；随后使用3D多任务nnU-Net骨干网络处理CTA、MRA、T2、T1增强等四种模态。模型包含双解码器以缓解动脉瘤与血管体积极度不平衡问题，并引入交叉注意力池化和模态特异辅助头，以提升异质多模态输入下的特征学习能力。

Result: 该方法覆盖13个解剖位置，支持多标签分类、动脉瘤多类别分割与血管多类别分割。其两折集成模型在RSNA 2025 Intracranial Aneurysm Detection挑战中获得第2名，并公开了代码、模型权重和3D Slicer插件。

Conclusion: 该研究将颅内动脉瘤分析从简单检测扩展到位置感知、多模态、多任务的精细化建模，具有较强临床辅助潜力；同时开源工具增强了方法的可复现性与应用推广价值。
...
TL;DR: 该论文提出 M2C，将冻结的 SAM3 通过少量标注样本自动学习可迁移概念嵌入，用于医学图像少样本自动标注，并结合不确定性主动纠错提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有自动提示医学分割方法通常依赖外部特征匹配器或辅助网络，带来额外架构开销并限制扩展性；SAM3 虽支持文本概念分割，但医学场景中缺乏细粒度临床知识，且人工文本描述存在歧义，因此需要一种无需外部模块、无需重训练、无需手工文本工程的医学少样本标注方案。

Method: 提出 Mask to Concept（M2C）框架：在冻结的 SAM3 内部初始化可学习概念嵌入，用该嵌入作为提示进行分割，并通过最小化概念分割误差的梯度更新嵌入，从少量标注图像中搜索可迁移视觉概念。同时提出 Hybrid Uncertainty Estimation（HUE），结合预测熵与概念预测映射回框提示后的概念-几何提示不一致性来估计不确定性；高不确定样本交由人工校正，校正掩码再反馈给 M2C，形成自增强标注闭环。

Result: 在医学分割基准上，M2C 实现了当前最优的少样本分割性能，并表现出较高的标注效率，能够以较少专家干预获得高质量标注。

Conclusion: M2C 展示了在不引入外部模块、不进行参数重训练、不依赖人工文本设计的条件下，将基础分割模型转化为医学自动提示标注器的可行性，为可扩展医学图像标注提供了实用且高效的路径。
...
TL;DR: NeuMatEx 提出一种结合大型材料重建模型初始化与可微逆路径追踪优化的方法，用于从图像中提取空间变化的神经材质。


<details>
  <summary>Details</summary>
Motivation: 神经材质能够紧凑表示复杂镜面反射与散射效果，但其采集和编辑困难；同时，神经材质潜空间高度非线性，使得直接用朴素逆渲染优化容易失败。

Method: 作者训练 Large Material Reconstruction Model（LMRM），从图像直接预测初始基础颜色、神经材质潜变量和偶然不确定性引导；随后以这些预测作为先验和初始化，通过逆路径追踪进行可微优化，并利用不确定性在高置信区域更强地锚定预测结果。

Result: 在合成与真实资产实验中，NeuMatEx 相比基于 PBR 的方法能提取更复杂的材质，视觉质量和材质分解效果更好。

Conclusion: 通过学习型材料先验、不确定性约束和物理一致的逆渲染优化相结合，NeuMatEx 有效缓解了神经材质反演中的优化困难与光照/高光烘焙问题。
...
TL;DR: 本文指出现有不可学习样本的有效扰动主要集中在高频，容易被低通滤波削弱；提出 FUSE 通过频谱均衡生成跨频段均有效的扰动，从而提升数据保护鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有不可学习样本依赖不可感知扰动来阻止模型学习有效表征，但在低通滤波后保护效果显著失效，说明其不可靠且过度依赖高频信号。作者希望构造在完整频谱范围内都保持有效的不可学习样本。

Method: 提出 Full-spectrum Unlearnable examples via Spectral Equalization（FUSE）。核心包括：1）Random Spectral Masking（RSM），在生成器训练中随机移除连续频带，迫使剩余频段也能维持不可学习性；2）Cross-Band Guidance（CBG），约束高频与低频组件之间的一致性，增强低频不可学习性，同时调节高频扰动以保持图像语义保真度。

Result: 在多个数据集、模型架构和频谱滤波设置下，实验表明 FUSE 相比现有方法具有更强、更稳定的数据保护效果，尤其在频谱扰动或滤波条件下仍能保持不可学习性。

Conclusion: FUSE 证明了不可学习样本不应只依赖高频扰动，而应具备全频谱鲁棒性；通过频谱均衡与跨频段一致性约束，可生成更可靠且语义保真度更好的数据保护样本。
...
TL;DR: 本文通过受控合成视觉退化系统分析开放词汇目标检测器的鲁棒性，发现鲁棒性主要由视觉骨干与图像域决定，并提出轻量级即插即用方法提升真实场景表现。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测器结构复杂，真实世界噪声对其性能影响及鲁棒性退化机制尚不清楚；需要明确噪声如何在模型层级中导致特征崩塌，以及哪些因素真正决定鲁棒性。

Method: 作者提出 Robust Onion，一项经验性分析框架，使用受控合成视觉退化逐层剖析 OV-OD 模型，系统研究特征崩塌的位置、原因与过程；同时比较不同视觉骨干、预训练策略、架构差异、字幕监督、数据集图像域与标注因素，并提出轻量级 NN & TK0 即插即用鲁棒性增强方法。

Result: 实验发现，具有相似视觉骨干的模型表现出相近鲁棒性，且在相似层发生类似特征崩塌；预训练策略、架构细节和 caption 监督影响较小。鲁棒性主要受图像域而非标注决定，因此 COCO 与 LVIS 上鲁棒性影响相似，而 ODinW-13 可能因大而孤立的目标造成鲁棒性被高估。NN & TK0 在 BDD100K、WiderFace 和 VisDRONE 等真实数据集上提升鲁棒性，且可训练参数比端到端训练少 96 倍。

Conclusion: 本文认为 OV-OD 鲁棒性的核心瓶颈在视觉特征层面，尤其与视觉骨干和图像域相关；通过定位特征崩塌并采用轻量适配策略，可以高效改善真实噪声下的检测鲁棒性，同时为既有鲁棒性观察提供统一解释。
...
TL;DR: 本文提出3D-anchored Light Probe（3DLP）基准，用真实HDR室内光照变化数据评估生成式图像编辑模型是否具备符合物理规律的真实世界光照理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型虽视觉质量很高，但其是否真正理解真实世界光照仍不明确；现有评测多依赖互联网图像的高层感知判断、VLM或合成数据，难以进行像素级、物理锚定的光传输分析。

Method: 作者采集了包含约1K组真实室内场景HDR图像对的数据集，通过物理开关光探针对照记录光照变化，并标注投影阴影、金属表面等特定区域；随后评估多种先进图像编辑模型，并提出两个新评分指标以补偿AI生成图像中的白平衡调整等摄影效应。

Result: 不同模型整体表现差异明显；在镜面高光区域差异略小。最佳模型与真实物理光照具有较高一致性，但仍存在改进空间。所有模型在接受光探针照明较弱的区域更容易出错；VLM不适合进行像素级光传输评估。

Conclusion: 3DLP提供了一个基于真实世界HDR数据和区域级标注的细粒度光照编辑评测基准，有助于推动生成式图像编辑模型在物理一致性与真实光传输建模方面的研究。
...
TL;DR: 论文提出一种面向实时交互的流式视频编辑框架，可逐帧因果编辑并保持背景与非编辑区域稳定，推理速度达到 12.66 FPS。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频编辑在实际部署中面临两大瓶颈：一是难以长期稳定保持背景和未编辑区域；二是延迟较高，难以满足实时交互与增强现实应用需求。同时，现有流式视频生成方法主要面向合成任务，缺乏编辑所需的严格内容保留和区域级控制能力。

Method: 方法核心是三阶段蒸馏流程，将强大的双向基础模型的编辑能力逐步迁移到高效的单向流式编辑器中，以支持因果、逐帧处理。另提出 AR 导向的 mask cache，复用跨帧区域相关计算，减少冗余处理并加速推理。作者还构建了专门的流式视频编辑基准用于评估。

Result: 实验表明，该方法在流式基线中实现了最先进的视觉质量，同时显著提升推理速度，达到 12.66 FPS，具备实时交互潜力。

Conclusion: 该工作证明通过渐进式蒸馏与区域计算缓存，可以在保证编辑质量、内容保留和时序稳定性的同时，实现低延迟流式视频编辑，适用于交互式和增强现实场景。
...
TL;DR: 本文提出一种基于视觉-毫米波雷达融合的多模态点云生成方法，通过图像语义约束、空间对齐与稀疏补全提升雷达点云质量，并在检测与跟踪任务中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达点云具有稀疏、噪声大、结构不完整等问题，限制了其在目标检测、跟踪等下游感知任务中的性能；因此需要借助多传感器信息提升点云密度、结构完整性与任务鲁棒性。

Method: 方法利用视觉图像中的语义信息对雷达点云施加结构约束并实现空间对齐，同时引入稀疏补全策略以增加点云密度、恢复缺失几何结构；随后将生成点云用于目标检测和跟踪任务进行评估。

Result: 实验表明，该方法能够有效改善毫米波雷达点云质量，并提升复杂环境下感知模型的检测精度与鲁棒性，同时对跟踪任务也具有积极作用。

Conclusion: 视觉-雷达融合结合语义约束和稀疏补全是提升雷达点云可用性的一种有效方案，可为多传感器点云生成与智能感知系统提供实用技术路径。
...
TL;DR: 该论文提出一种基于半平衡最优传输的容量控制框架，用于提升3D Gaussian Splatting多视角风格化的一致性、稳定性与表现力。


<details>
  <summary>Details</summary>
Motivation: 现有3D风格化方法通常对每个渲染视角独立施加2D特征匹配损失，容易导致风格分配不稳定、多对一特征复用以及跨视角风格不一致。论文旨在解决3DGS风格化中视角间风格一致性与局部风格匹配可控性不足的问题。

Method: 作者将局部风格匹配重构为半平衡最优传输问题，并引入显式列容量约束及可调强度，以限制风格特征被过度复用，实现可控的风格特征分配。同时提出跨视角匹配引导，以约束场景内容与风格模式之间的对应关系；此外加入多种几何正则化，增强原始3DGS中高斯基元表达细粒度纹理的能力。

Result: 大量实验表明，该方法能够显著提升多视角风格一致性，生成更稳定、更具表现力的3D风格化结果，同时保留场景的核心语义结构。

Conclusion: 论文的核心贡献在于将容量受控的最优传输机制引入3DGS多视角风格化，为风格覆盖、多样性和跨视角一致性之间的平衡提供了更有原则性的优化方式。
...
TL;DR: ProtoKV 提出一种用于流式视频理解的常量占用记忆机制，在长延迟查询场景下提升准确率并控制显存与延迟。


<details>
  <summary>Details</summary>
Motivation: 流式视频理解中，查询异步到达且视频 token 持续输入；当查询延迟较长时，关键视觉线索可能早已短暂出现并被后续更新挤出或稀释。现有有限容量 token 保留方法难以在严格 GPU 显存和查询时延预算下稳定保存远历史信息。

Method: 方法保留精确的近邻窗口 KV cache，同时将更早历史聚合为固定容量的语义-空间 prototype bank，并维护残差统计信息。查询时，每个 prototype 通过有界 pseudo-token 接口暴露，可与标准 attention 机制即插即用兼容，从而以常量内存表示远历史。

Result: 在相同预算和相近查询时开销下，ProtoKV 在流式视频理解基准的长延迟场景中，相比 token-retention baseline 最高提升 12.5 个百分点；且查询延迟越长，收益越明显。

Conclusion: ProtoKV 表明，将远历史压缩为固定容量的语义-空间原型，比简单保留 token 更适合长延迟流式视频理解，可在受限显存与低延迟约束下更可靠地保留关键历史线索。
...
TL;DR: 本文提出 CHOIR，一种用于多维数据恢复的隐式神经表示方法，通过谐波叠加替代传统函数复合，并引入感知频谱校准，以提升深层网络优化稳定性并缓解频谱偏置。


<details>
  <summary>Details</summary>
Motivation: 现有使用周期激活函数（如 Sine）的 INR 方法多依赖函数复合，随着网络加深容易产生优化不稳定；同时缺乏合适的物理先验，难以有效缓解频谱偏置，限制了在多维数据（如多光谱图像、视频）恢复任务中的性能。

Method: 作者提出 Calibrated Harmonic Overlaid Implicit Neural Representation（CHOIR）。其核心包括两部分：1）Coordinated Harmonic Superposition（CHS），用协调谐波叠加替代常规 INR 中的函数复合机制，从而在增加网络深度时保持优化稳定；2）Perceptual Spectrum Calibration（PSC），将自然图像中普遍存在的幂律频谱先验嵌入模型，并将全局固定频谱校准为更符合物理规律的对数均匀分布，以减轻频谱偏置。

Result: 在多种多维数据恢复问题上的大量实验表明，CHOIR 相比当前先进方法取得了更优性能。

Conclusion: CHOIR 通过结构层面的谐波叠加设计和频谱层面的物理先验校准，改善了周期型 INR 的优化稳定性与频谱表达能力，是一种更适合多维数据恢复任务的隐式神经表示框架。
...
TL;DR: 本文提出一种用于4D医学影像（3D+时间）的可控生成式数据增强框架，通过解耦静态解剖结构与时间运动来合成解剖一致、时序连贯的4D影像，以提升跨设备场景下的下游分割鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 4D医学影像AI模型开发受限于标注数据稀缺、不同设备/厂商间的域偏移以及隐私限制。现有数据增强或生成方法难以同时保证可控的解剖结构、严格的时间一致性和对下游任务的实际增益，因此需要一种可扩展、可控且适合跨域泛化的4D医学影像合成方法。

Method: 方法包含两级核心设计：首先，使用半监督变分自编码器学习解剖体数据的紧凑潜表示，并在统一框架中联合预测对齐的分割掩膜；其次，通过级联潜扩散模型解耦解剖结构与时间动态，其中静态LDM根据临床先验（诊断、体积测量等）生成个体化静态解剖结构，运动LDM进一步估计残差潜空间运动，从而生成具有严格时间一致性的4D序列。实验以电影心脏MRI为代表应用进行验证，并在多数据集和跨厂商设置下评估。

Result: 实验显示，该框架对静态解剖结构具有较强可控性（Pearson r > 0.8），并能保持较好的时序一致性（FVD = 288.08）。在跨厂商泛化实验中，将合成4D序列用于训练增强显著提升下游分割性能。使用nnU-Net时，相比仅使用真实数据训练，平均Dice提高1.4%，Hausdorff Distance降低3.0mm；左心室分割Dice提高2.8%，边界误差降低5.4mm。

Conclusion: 该研究表明，可控的4D医学影像生成可有效缓解标注稀缺和跨设备域偏移问题。通过将静态解剖与动态运动分离建模，框架能够生成解剖一致且时间连贯的合成数据，并在下游心脏MRI分割中带来实际性能提升，具有作为可扩展医学影像数据增强方案的潜力。
...
TL;DR: 论文提出 LearniBridge，一种用于扩散 Transformer 特征缓存推理加速的可学习校准机制，通过轻量 LoRA 更新跨时间步修正缓存特征，在少量训练样本下实现高倍率加速并降低误差累积。


<details>
  <summary>Details</summary>
Motivation: DiT 在图像和视频生成中效果突出，但推理计算成本高。现有特征缓存方法通常复用历史中间特征，虽然实现简单，但在高加速倍率下会产生严重误差累积，影响生成质量。因此需要一种更有效的缓存特征校准方法。

Method: 作者分析所需特征修正的结构，发现最优校准更新在不同提示词之间共享低秩子空间。基于这一观察，提出 LearniBridge：使用轻量级 LoRA 更新在多个时间步之间进行可学习校准，从而修正缓存特征。该方法仅需 3–5 个训练样本即可适配。

Result: 在图像与视频生成任务上，LearniBridge 分别在 FLUX、HunyuanVideo 和 WAN2.1 上达到最高 5.87×、5.75× 和 4.10× 加速。在 WAN2.1 上，4.10× 加速条件下相比此前 SOTA 的 VBench 提升 1.28%。

Conclusion: LearniBridge 利用跨提示共享的低秩校准结构，有效缓解高倍率特征缓存中的误差累积问题，在极少训练样本下实现高质量、高效率的 DiT 推理加速。
...
TL;DR: 本文提出一种事件相机驱动的主动视觉系统，用于在真实球类运动场景中实时跟踪未改装球体并估计其旋转。


<details>
  <summary>Details</summary>
Motivation: 传统相机难以在比赛中估计高速、小尺寸、快速旋转球体的自旋，主要受限于时间分辨率、运动模糊、目标成像尺寸和对焦稳定性。

Method: 系统结合事件相机、高速 pan/tilt 振镜和低延迟可调焦长焦镜头；跟踪采用2D事件检测居中与3D球定位重初始化的混合策略；离线阶段提出球面上的对比度最大化方法 s-CMax 生成高精度旋转估计；在线阶段以乒乓球为案例，使用不确定性感知CNN学习离线伪真值，并结合GPU加速批量对比度最大化进行细化。

Result: 离线方法在乒乓球、棒球、网球和高尔夫球等静态球体上达到先进精度，旋转幅值平均误差为2.1%，旋转轴误差为4.0度；在线系统在职业乒乓球比赛三视角设置中实现8.8%幅值误差、6.4度轴向不匹配、3 ms延迟和750 Hz吞吐率。

Conclusion: 该研究证明事件式主动视觉结合自适应光学跟踪和学习/优化混合估计，可在高速真实运动场景中实现低延迟、高精度的球体自旋测量，具有较强的实时应用潜力。
...
TL;DR: 本文提出 ReasonCLIP-58M，通过持续预训练将大规模视觉可验证推理监督注入 CLIP 式视觉编码器，在不改变架构和不增加推理成本的情况下提升视觉常识、组合推理与零样本检索能力。


<details>
  <summary>Details</summary>
Motivation: 现有 CLIP 及其变体主要依赖描述性图文对齐预训练，虽然适合作为多模态系统视觉骨干，但在需要视觉扎根的常识推理和组合推理任务中能力不足；作者希望验证并增强 CLIP 式编码器在不改架构条件下的推理表征能力。

Method: 提出两阶段持续预训练框架：先渐进式融入推理信号并保持原有描述性图文对齐，再进行类别结构化推理监督。为此构建 ReasonLite-42M（开放式、视觉可验证推理描述）、ReasonPro-16M（类别特定推理监督）以及诊断评测基准 RCLIP-Bench，并训练 ReasonCLIP 系列模型。

Result: ReasonCLIP 在视觉扎根常识推理和组合推理上取得提升，同时增强零样本检索性能；作为 LLaVA-NeXT 等多模态大语言模型的即插即用视觉编码器，也能带来稳定增益，且无额外推理开销。

Conclusion: 结构化推理监督可以有效增强 CLIP 式视觉表征的表达能力，说明无需修改模型架构，仅通过持续预训练与高质量推理数据即可提升多模态系统中的视觉推理能力。
...
TL;DR: NaviCache 是一种用于视频扩散模型的即插即用测试时自校准加速方法，通过更准确判断可跳过计算来降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有 VDM 加速方法存在局限：离线校准方法依赖校准数据、耗时且易受分布偏移影响；离线免校准方法虽避免这些问题，但依赖瞬时零阶近似，容易受观测噪声影响，并忽略扩散轨迹中的内在动量。

Method: 论文将特征演化重新建模为惯性导航系统问题，提出 NaviCache。其核心包括：建模输入与输出变化之间的相对耦合关系；采用双状态估计结构自适应跟踪特征变化比及其潜在漂移；通过初始对准阶段初始化状态；结合时间相关噪声调度与不确定性感知的测量更新机制，实现有误差界的计算跳过。

Result: 在 HunyuanVideo、Wan 和 Open-Sora 系列模型上的实验表明，NaviCache 能更准确地判断哪些计算可被跳过，并取得优秀的综合加速与质量表现。

Conclusion: NaviCache 为视频扩散模型推理加速提供了一种无需离线校准、具有测试时自适应能力且理论上更稳健的方案，能够在降低计算成本的同时维持生成质量。
...
TL;DR: 本文提出一种面向恶劣天气下多模态图像融合的掩码引导方法，通过伪真值、掩码生成、跨模态交叉注意力以及退化感知学习，实现特征恢复与模态互补的协同优化，并在多类数据集和下游任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态图像融合依赖不同模态的互补信息增强场景表征，但恶劣天气会造成图像退化，破坏特征表达。现有方法难以同时完成退化特征恢复与跨模态有效交互，导致实际场景性能受限。

Method: 方法包含四个核心部分：引入伪真值以简化训练并促进快速有效的特征学习；基于融合结果与源图像之间的映射关系生成掩码，用于量化各模态在融合过程中的相对贡献；设计掩码引导的跨模态交叉注意力机制，使网络选择性关注信息量更高的特征；进一步提出掩码引导学习策略和任务耦合的退化感知学习策略，以平衡特征恢复与跨模态交互。

Result: 在合成数据集和真实世界数据集上的大量实验表明，该方法在视觉质量、定量评价指标以及下游任务表现方面均超过当前先进方法。

Conclusion: 该工作表明，在恶劣天气条件下，将退化恢复、模态贡献建模和跨模态注意力交互结合起来，可以显著提升多模态图像融合的鲁棒性与实用性能。
...
TL;DR: 本文构建了真实户外复杂光照下的地面平台大蒜幼苗检测数据集，并提出基于对抗式增强策略学习的光照鲁棒检测框架，在不增加推理开销的情况下提升幼苗检测与缺苗定位性能。


<details>
  <summary>Details</summary>
Motivation: 早期幼苗精准检测对补种与作物管理至关重要，但现有研究多在无人机影像或温室等较稳定成像条件下评估；地面户外监测中严重且空间异质的光照变化尚未充分解决。同时，许多光照鲁棒方法依赖额外增强或特征模块，增加推理成本，且不一定适配幼苗检测及后续缺苗定位任务。

Method: 作者采集并构建了真实田间复杂光照条件下的大蒜幼苗数据集；提出一种对抗式增强策略学习框架，联合优化随机增强策略智能体与目标检测器，使检测器在训练中学习更鲁棒的视觉表征；同时引入结构惩罚项，避免增强产生不真实畸变，并鼓励生成具有挑战性的训练样本。

Result: 实验结果显示，该方法在幼苗检测上达到91.6%的AP50，比基线提高0.9个百分点，比此前最佳方法提高0.2个百分点；在下游缺苗定位任务中，精确率达到75.0%，F1值达到67.0%，分别较基线提升4.8和2.0个百分点。

Conclusion: 该框架能有效提升复杂户外光照条件下地面农业监测的幼苗检测与缺苗定位鲁棒性，并且无需在推理阶段引入额外计算开销，具有较强的实际应用价值。
...
TL;DR: 本文提出 AnomNOVIC，一种面向机器人开放世界场景的两阶段、免提示词开放词汇目标检测框架，通过异常检测式 MAE 生成通用候选框，再由实时开放词汇分类器 NOVIC 对显著区域进行分类。


<details>
  <summary>Details</summary>
Motivation: 真实环境中的机器人需要识别训练阶段未见过的物体；现有开放词汇检测方法往往依赖提示词、候选类别列表或计算成本较高，难以满足机器人连续部署中的免提示、实时和开放世界需求。

Method: 方法采用两阶段框架：首先训练掩码自编码器（MAE）用于异常检测，在已知工作空间中产生与类别无关的物体边界框；随后使用 NOVIC 对这些显著图像区域进行实时、免提示词的开放词汇分类。该设计避免预定义候选类别列表，同时也支持在提供类别候选时提升性能。

Result: 在包含 NICOL 类人机器人与桌面物体的环境中，AnomNOVIC 在免提示识别下达到 47.1% AP / 57.5% AP50；若提供类别候选，则达到 59.0% AP / 72.5% AP50。在额外数据集上，包括含 48 个独特物体的野外测试集，其免提示检测与分类准确率最高达到 82.6%，超过 YOLO-World-v2、OWLv2 和 YOLOE 等开放词汇基线。

Conclusion: AnomNOVIC 表明，将类别无关的异常检测候选框生成与强实时开放词汇分类器结合，可以显著提升机器人场景中的免提示开放词汇物体识别能力，并具备较好的连续部署潜力。
...
TL;DR: 本文提出 LFNet，通过动态融合 SSM（VMamba）与 CNN（ConvNeXt）的互补频谱偏好，用于通用显著性目标检测，并在多种 SOD 任务上取得领先性能与较好效率。


<details>
  <summary>Details</summary>
Motivation: 现有 SOD 方法虽受益于状态空间模型，但普遍忽视不同网络范式的固有频谱偏置。作者通过数据集层面的频谱分析发现，CNN 与 SSM 在频率偏好和语义表征上具有互补性，因此希望通过异构特征融合弥合频谱偏差，提升通用显著性检测能力。

Method: 提出 Liquid Fusion Network（LFNet），采用 VMamba 与 ConvNeXt 双骨干提取特征；受液态神经网络动态信息传播启发，设计 liquid fusion，将 VMamba 连续特征视为演化状态、ConvNeXt 特征视为外源刺激，并通过动态门控进行内容感知融合。该状态-刺激范式还可扩展到多模态线索。同时提出 Saliency-Guided Upsampling（SGU）算子，以频谱-空间协同设计向浅层传播特征，抑制上采样伪影并保持语义信息。

Result: 在 RGB、RGB-D、RGB-T、视频 SOD（VSOD）和 VDT 五类任务上的大量实验表明，LFNet 达到当前最优或具有竞争力的性能，并在检测精度与模型效率之间取得更优平衡。

Conclusion: 论文表明，显式建模并融合 CNN 与 SSM 的互补频谱特性有助于提升通用显著性目标检测。LFNet 的动态液态融合与显著性引导上采样为多模态、多任务 SOD 提供了有效且高效的统一框架。
...
TL;DR: 本文提出一种基于仿射对应的滚动快门两视图相对位姿估计方法，将最小样本数从20个点对应降至7个仿射对应，使RANSAC鲁棒估计更实用。


<details>
  <summary>Details</summary>
Motivation: 消费级设备普遍使用滚动快门相机，但现有RS相对位姿求解器至少需要20个点对应，导致RANSAC迭代次数随样本规模指数增长，计算代价过高，难以实际应用。

Method: 论文将仿射对应引入RS两视图几何，推导RS校正仿射约束；每个对应除标准极线约束外额外提供两个方程。基于RS参数物理上较小的假设，对约束进行线性化，通过零空间投影消去12个RS未知量，并使用action matrix求解剩余20次系统，形成仅需7个仿射对应的线性化代数求解器。

Result: 求解器运行约1.2 ms。在TUM RS基准上，相比测试方法取得最佳位姿与RS参数精度，并能准确估计平移速度；在全局快门EuRoC MAV数据集上，其精度与标准5点法相当。

Conclusion: 该方法显著降低RS相对位姿估计的最小样本需求和RANSAC成本，使RS-aware鲁棒估计具备实际可用性，同时在RS与GS场景下均表现稳定。
...
TL;DR: 论文提出 SpatialFlow-GRPO，通过空间细粒度奖励反馈改进在线强化学习图像编辑训练，显著优于仅使用整图奖励的 Flow-GRPO 类方法。


<details>
  <summary>Details</summary>
Motivation: 现有 Flow-GRPO 风格方法通常依赖单一整图奖励，隐含空间一致性假设，无法区分不同图像区域对编辑质量的贡献，导致细粒度、多区域编辑优化困难。

Method: 提出 SpatialFlow-GRPO 框架，将区域感知奖励转换为语义区域级优化信号，并在策略更新中把区域优势与对应潜空间位置对齐；同时训练区域感知奖励模型 SFReward，构建含区域标注编辑样本的数据集 SFReward-14K，并提出 MultiEditBench 用于评估多区域编辑能力。

Result: 在 OmniGen2 和 FLUX.2-klein-4B 上，SpatialFlow-GRPO 在 GEdit-Bench、ImgEdit-Bench 和 MultiEditBench 上均优于 Flow-GRPO。

Conclusion: 空间细粒度奖励能够将局部反馈转化为空间对齐的更新信号，从而缓解整图奖励的局限，提升图像编辑质量，尤其有助于多区域和细粒度编辑任务。
...
TL;DR: 该论文提出一种视觉辅助毫米波网络中的RIS辅助主动切换方案，通过优化RIS单元数量，在满足切换时序约束的同时降低能耗并维持链路质量。


<details>
  <summary>Details</summary>
Motivation: 毫米波网络易受视距阻塞影响。视觉辅助通信可提前触发主动切换，但当附近无可用基站时，仅依赖基站切换难以恢复连接。RIS可辅助建立替代链路，但其大量反射单元带来的配置时间和处理复杂度会影响时延敏感场景中的及时切换，因此需要在链路质量、能耗和切换时序之间权衡。

Method: 作者提出RIS辅助主动切换框架，将RIS配置时间纳入切换决策，并通过优化分配的RIS反射单元数量来平衡信号处理复杂度与链路质量。在建模上，构建端到端RIS链路配置优化问题，并采用粒子群优化算法离线求解，以避免在线优化造成额外时延。

Result: 实验结果表明，在RIS单元数量减少12%的情况下，系统耗散能量降低约10%，同时不损害信噪比表现。此外，在阻塞区域中，RIS辅助链路可带来约15–30 dB的性能提升，并保持准确的主动切换时序。

Conclusion: 该工作证明了在视觉辅助毫米波通信中引入RIS可有效应对无邻近基站可切换时的阻塞问题。通过离线优化RIS单元分配，可在满足实时切换要求的同时提升能效并保持可靠链路质量。
...
TL;DR: 本文提出 OTF-CBM，将概念对齐建模为动态跨模态最优传输过程，以提升概念瓶颈模型在视觉-语言场景中的可解释性与分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言概念瓶颈模型通常依赖预对齐编码器或全局余弦相似度，难以实现细粒度概念定位，也不能充分刻画视觉补丁与文本概念之间真实的语义几何关系。

Method: 方法包括：通过逆最优传输学习数据驱动的语义代价函数，用于度量跨模态距离；随后采用非平衡最优传输的流匹配来建模视觉补丁与文本概念之间的语义转移；最后利用基于速度的概念激活，在无需 ODE 积分的情况下捕获可解释的几何关系。

Result: 实验表明，OTF-CBM 在分类准确率和概念忠实性方面优于现有方法，并能更好地反映跨模态概念对齐关系。
...
TL;DR: 本文提出 MICViT，一种用于多模态 3D 脑 MRI 的视觉 Transformer，通过同时建模模态内特征与跨模态交互，在脑龄预测任务上优于现有 CNN 和 Transformer 基线。


<details>
  <summary>Details</summary>
Motivation: 脑 MRI 机器学习面临高维 3D、多模态、样本相对有限以及解剖/病理/采集差异大的挑战；多模态 MRI 虽能提供互补信息，但如何有效融合不同模态仍是关键难题。

Method: 提出 Multimodal Intra- and Cross-Context Vision Transformer（MICViT），结合四类注意力机制：模态特异的局部注意力与全局注意力用于模态内表示学习，跨模态局部注意力与全局注意力用于捕获不同 MRI 模态之间的交互。

Result: 在 UK Biobank、SOOP、Cam-CAN 三个异质数据集上进行脑龄预测评估，使用 T1、FLAIR、DWI、SWI 等多种 MRI 模态。MICViT 在 3D 设置下持续优于先进 CNN 与 Transformer 基线，并且随着输入模态增加获得更显著性能提升。

Conclusion: 显式建模模态内与跨模态交互对于释放多模态脑 MRI 的潜力至关重要，MICViT 为神经影像表征学习提供了有前景的方向。
...
TL;DR: 本文提出一种利用合成数据增强的猕猴示踪组织学纤维束自动分割框架，在减少人工标注需求的同时保持接近先进方法的性能。


<details>
  <summary>Details</summary>
Motivation: dMRI tractography 可无创重建白质通路，但受限于间接、低分辨率测量；非人灵长类示踪注射组织学可作为验证金标准，但纤维束人工标注耗时，因此需要更高效的自动分割方法。

Method: 以离体 dMRI tractography 作为生成先验，合成用于训练的二维图像 patch；将合成的前景纤维纹理与 blockface 照片背景组合，并通过域随机化增加多样性；使用真实与合成 patch 混合训练二维 U-Net。

Result: 在留出脑样本上的实验表明，与仅使用真实数据训练相比，混合合成数据可提升跨脑样本和不同纤维束密度条件下的泛化能力；仅用合成数据训练表现较差；总体性能接近当前先进水平，同时人工标注数据需求减少约 3 倍。

Conclusion: 合成数据可有效作为真实监督数据的补充，但不能完全替代真实标注；基于 dMRI 先验的合成增强策略能够降低组织学纤维束分割的标注成本并保持可靠性能。
...
TL;DR: 本文提出 Robust-TO，通过显式建模每帧视频的可信度并将其融入工具调用、证据融合与强化学习优化，在干净与受扰动视频推理场景中均提升准确率与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理语言模型通常默认所有输入帧同等可靠，导致在运动模糊、眩光、遮挡等真实扰动下出现显著性能下降，且模型无法意识到视觉证据已退化，即 Blind Trust Problem。

Method: Robust-TO 将多种视觉感知工具组织在统一证据接口下：根据原问题拆分子查询，并利用可靠性-相关性分数选择可信帧；各工具输出统一格式的证据，包括具体预测、时间定位与校准可靠性分数；推理阶段基于高/中/低三层证据可信度进行加权综合，并设计 confidence-cost GRPO 奖励以联合优化正确性、证据可靠性和效率。

Result: 在两个覆盖八项任务的视频推理基准上，Robust-TO 在干净输入上达到 56.4% 平均准确率，比最强开源基线高 10.6 个百分点，并优于 Gemini-2.5-Pro 的 46.2%；在五类真实扰动下保持 54.3% 平均准确率，比最强开源基线高 5.8 个百分点，且干净到扰动场景的性能下降最小。

Conclusion: 显式引入逐帧可信度估计和可靠性校准证据融合，能够有效缓解视频推理模型对退化视觉输入的盲目信任问题，并显著提升复杂真实场景下的视频理解鲁棒性与效率。
...
TL;DR: 论文提出 Qwen-Image-Agent，用智能体式流程弥合用户请求与文生图模型所需充分生成上下文之间的“上下文鸿沟”，并在多个基准上取得领先表现。


<details>
  <summary>Details</summary>
Motivation: 现有文生图模型虽能力显著提升，但面对真实用户请求时常因指令不完整、含有隐含意图或依赖最新知识而表现不佳。核心问题在于用户提供的上下文不足以支撑高质量图像生成。

Method: 作者提出统一的智能体框架 Qwen-Image-Agent，围绕上下文构建整合规划、推理、搜索、记忆与反馈。框架包含 Context-Aware Planning，用于识别缺失上下文并规划获取与使用方式；以及 Context Grounding，用于通过推理、搜索、记忆和反馈逐步补全生成上下文。

Result: 作者构建了 Image Agent Bench（IA-Bench），覆盖规划、推理、搜索和记忆四类核心图像智能体能力。实验表明，该方法在 IA-Bench、Mindbench 和 WISE-Verified 上优于强基线，并达到当前最优性能。

Conclusion: 该工作表明，将上下文感知的智能体机制引入文生图系统，可有效提升模型处理真实复杂请求的能力，尤其适用于信息不完整、隐含需求和需要外部知识的生成场景。
...
TL;DR: PhysRAG 通过检索增强生成将物理知识注入视频扩散模型，并基于筛选后的高质量物理视频数据提升视频生成的物理一致性与视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以准确建模热学、力学、光学等多样物理现象，同时高质量、物理相关训练数据有限，导致生成视频常违反物理规律。

Method: 作者提出 PhysRAG 流水线：首先在 WISA-80K 数据集上设计两阶段数据过滤流程，筛选出 7K 高质量物理视频；随后构建物理视频数据库，并通过检索增强生成机制与可学习查询将检索到的物理信息注入视频扩散模型；同时进行多项消融实验验证数据过滤、RAG 机制和物理信息提取方法的贡献。

Result: 该方法在 PhyGenBench 和 VBench 等基准上取得当前最优表现，在视觉质量和物理规则遵循能力方面均超过已有模型。

Conclusion: PhysRAG 表明，结合高质量物理视频数据筛选、物理知识检索与可学习注入机制，可以有效增强视频生成模型的物理感知能力，并为后续物理一致视频生成研究提供代码、数据和模型资源。
...
TL;DR: PortraitGen 通过在 GRPO 后训练中引入真实图像样本、结合图像反演获取其潜变量与转移概率，并使用双奖励机制提升人像生成的真实感与细节可信度。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像后训练方法常偏向表层审美优化，例如颜色更鲜艳，却难以修复 AI 伪影、皮肤油腻感、生物结构不合理等细粒度问题。作者认为主要原因是缺少真实图像参与后训练，导致采样受限于原生成分布；同时奖励信号缺乏对人像细节与伪影的针对性约束。

Method: 提出 PortraitGen 框架：首先将真实图像直接引入 GRPO 的采样组中，并通过图像反演获得其潜变量和转移概率，以突破模型原有生成边界；其次设计互补的双奖励机制，包括用于总体质量评估的 OmniReward 和用于人像真实性、人类中心细节保真的 AI-Portrait；此外构建面向人像生成评测的 PortraitBench 基准。

Result: 大量实验表明，PortraitGen 相比现有基线方法在照片级人像生成上表现更优，能够更有效抑制 AI 伪影，并显著提升人像真实感。

Conclusion: 该工作表明，在强化学习式图像后训练中引入真实图像样本并设计细粒度人像奖励，可以缓解仅优化表层审美的问题，推动文本到图像模型生成更自然、可信的照片级人像。
...
TL;DR: 该论文提出 SharpMoE，用干净潜变量特征指导扩散 MoE 的路由决策，缓解噪声潜变量导致的显著 token 资源分配不准问题，并在视觉生成中提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型中的 MoE 路由器依赖去噪过程中的含噪潜变量特征进行专家/计算资源分配，随机噪声会掩盖结构与纹理等关键信息，使路由器难以识别显著 token，导致计算资源没有准确分配给更重要的区域。

Method: 提出后训练框架 SharpMoE，引入 saliency-harnessing accurate routing mechanism，使用干净潜变量特征作为无噪声指导信号来辅助路由，绕开噪声输入对路由判断的干扰；同时设计 trajectory routing loss，在多步去噪轨迹中约束计算分配，保证生成全过程的资源分配一致且精确。

Result: 大量实验表明，SharpMoE 可作为通用的即插即用方法，在已有预训练且收敛的 MoE 扩散模型上进一步提升视觉生成效果，并达到当前最优性能。

Conclusion: 论文认为扩散 MoE 的关键瓶颈之一在于噪声干扰下的路由失准；通过引入干净特征进行显著性指导并约束去噪轨迹中的路由行为，SharpMoE 能更可靠地将计算资源分配给显著 token，从而提升生成质量与效率。
...
TL;DR: 本文提出 TraMP-LLaMA，用于面部表情质量评估，联合进行严重程度评分预测与结构化文本报告生成，以提升可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有 FEQA 方法通常只输出严重程度分数，缺少对可观察面部运动证据的显式说明，导致模型在帕金森病评估场景中的可解释性与可审查性不足。

Method: 作者构建统一多模态框架 TraMP-LLaMA，融合 RGB 外观信息与面部关键点轨迹信息；采用解耦式指令微调策略，降低严重程度预测与语言生成之间的任务干扰；同时扩展 PFED5 数据集，加入专家指导的文本运动描述，形成 PFED5-plus。

Result: 在 PFED5-plus 上，TraMP-LLaMA 在报告生成方面优于竞争性视频-语言基线；在联合多表情训练设置下，其严重程度预测性能最佳，Spearman 等级相关系数相较所有竞争方法至少提升 4.39%。

Conclusion: TraMP-LLaMA 通过联合建模面部运动线索、评分预测与文本解释，在提升 FEQA 性能的同时增强了输出可解释性；PFED5-plus 也为该方向提供了带文本描述的新基准。
...
TL;DR: 论文提出 OmniRef-Bench 用于评估复杂多参考图像生成，并提出 DyRef 两阶段训练框架，通过监督微调与奖励优化提升模型在混合类型、多数量参考图像场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有个性化图像生成虽有进展，但多参考图像生成仍困难；已有基准难以覆盖复杂参考类型组合和大量参考图像，限制了对模型能力的准确评估与方法发展。

Method: 构建 OmniRef-Bench 基准，覆盖复杂参考图像类型组合和大量参考图像；提出 DyRef 两阶段框架：第一阶段用监督微调获得复杂 MRIG 基础能力；第二阶段引入 Difficulty-aware Advantage Reweighting 动态调整优化目标，并用 Discriminative Reward Scaling 放大组内奖励差异以提升策略优化效率。

Result: 评测显示主流开源模型在复杂 MRIG 场景中表现不佳，且随混合类型参考图像数量增加显著退化；DyRef 在 OmniRef-Bench 和单图编辑基准上显著提升开源模型性能。

Conclusion: OmniRef-Bench 能更全面评估复杂 MRIG 能力；DyRef 有效缓解多类型、多参考图像带来的性能退化，并具备较好的泛化能力。
...
TL;DR: 本文提出 Dota2-Vis：一个基于职业 Dota 2 比赛视频的可见性分析数据集，并构建了以目标检测为核心的基线流程，用于估计比赛中队伍实际可见的对手玩家信息。


<details>
  <summary>Details</summary>
Motivation: 现有 MOBA 分析多依赖结构化数据，但这类数据难以直接反映比赛过程中每支队伍在特定时刻实际能够观察到的信息；因此需要一种基于视频视角的数据与方法来研究“可见性”这一关键战术因素。

Method: 作者收集 The International 2025 的 144 场比赛，并从双方队伍视角录制，共 288 个 Full HD 视频；同时人工标注 2,477 张小地图图像。随后评估多个现代目标检测器变体，用于识别小地图上的玩家图标，并使用最佳模型生成随时间变化的对手可见玩家存在估计。

Result: YOLO11l（large）取得最佳整体表现，在密集、视觉干扰较强的小地图场景中仍能较可靠地检测玩家图标。基于检测结果生成的可见性曲线揭示了玩家、英雄、角色和团队层面的行为模式。

Conclusion: Dota2-Vis 及其基线流程为 MOBA 可见性分析提供了新的视频数据基础，能够补充传统结构化数据分析，捕捉更难从常规数据中获得的战术与行为差异。
...
TL;DR: 本文提出几何梯度校正（GGR），在开放集半监督学习中不再主要依赖样本筛选或软加权，而是在梯度层面约束辅助无标签目标的更新方向，以减少错误伪标签带来的梯度冲突。


<details>
  <summary>Details</summary>
Motivation: 开放集半监督学习的无标签数据可能包含分布外样本。现有方法通常要么过滤可疑样本，要么对无标签损失进行软加权，但二者都存在权衡：过滤过强会丢弃困难但有价值的分布内样本；利用过多则可能因错误伪标签产生与监督学习目标冲突的辅助梯度。

Method: 作者提出即插即用框架 Geometric Gradient Rectification（GGR）。该方法以监督梯度作为锚点，将与其冲突的辅助梯度投影到梯度空间中的可接受区域，使辅助更新在被校正的坐标块内一阶意义上不与监督更新相反，同时保留正交分量以维持潜在有用的表征学习信号。进一步提出子空间感知校正，以缓解小批量监督梯度噪声导致的锚点不稳定。

Result: 在 CIFAR 和 ImageNet 基准上，GGR 在多数设置中提升了代表性开放集半监督学习基线方法，并同时改善闭集泛化能力与开放集鲁棒性。

Conclusion: GGR 将开放集半监督学习的核心问题从样本级选择转向梯度级控制，能更稳健地利用含有分布外样本的无标签数据，减少辅助目标与监督目标的冲突，并可作为通用插件增强现有方法。
...
TL;DR: 本文提出 Unison，一个用于评估统一多模态模型“理解—生成”协同能力的综合基准，并配套 Unison-Judge 以提升评测与人类判断的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型虽同时具备理解与生成能力，但主流评测往往将两者割裂考察，难以衡量二者之间的协同、相互促进以及统一建模带来的真实收益。

Method: 构建包含 2,169 个高质量统一任务样本的 Unison 基准，覆盖内部一致性、理解引导生成、生成引导理解和相互增强四个维度；同时设计统一轨道与解耦轨道，以诊断理解或生成模块的具体失败来源；并提出与人类评判对齐的 Unison-Judge 评估模型。

Result: 对当前先进统一多模态模型进行系统评测后，作者发现这些模型在联合理解与生成任务中仍存在关键短板，并可通过 Unison 的诊断设置量化统一建模带来的增益与失败模式。

Conclusion: Unison 为统一多模态模型提供了更全面、可诊断且更贴近人类偏好的评估框架，有助于推动未来模型在理解与生成协同方面的发展。
...
TL;DR: 论文提出 EVIS，将视频按文本相关的简单事件进行可学习分解，并结合事件感知特征与对象-像素混合学习，以提升指代表达视频分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有指代表达视频分割方法通常把整个视频视为单一事件，忽略视频中往往包含多个 distinct events。这会迫使模型同时理解复杂的视频与文本内容，容易造成语义混淆和幻觉。作者希望通过将复杂视频拆解为更简单、文本相关的事件片段，实现更稳健的层级理解。

Method: 方法核心是 EVIS（Event-Aware Video Instructed Segmentation Assistant）。首先利用文本引导的可学习 Event Queries 将视频划分为若干简单事件，并提取事件感知的视觉-文本特征，从而进行逐事件理解。其次提出 Object-Pixel-Hybrid Learning，将细粒度像素特征与先验 object queries 结合，使多模态大模型能够在长视频中更稳定地跟踪目标。

Result: 在 5 个公开基准上的大量实验表明，EVIS 在指代表达视频分割任务中取得了较强性能，验证了事件分解和对象-像素混合学习的有效性。

Conclusion: 该工作说明，将视频从单一整体理解转向文本指导下的事件级层次理解，可以降低复杂视频推理难度，并提升长时序目标分割与跟踪能力。
...
TL;DR: 本文提出一种基于遥感基础模型（ResNet RSFM）与未训练FPN的无监督变化/异常检测方法，用于在连续轨道观测影像间识别潜在语义变化。


<details>
  <summary>Details</summary>
Motivation: 遥感任务受限于星载功耗、算力和存储资源，且灾害监测等场景缺乏昂贵标注数据。作者希望利用RSFM的通用表征能力，实现无需监督训练、可泛化的高效异常检测，从而支持卫星自主触发高分辨率成像或动态调整任务参数。

Method: 方法采用ResNet形式的遥感基础模型作为统一编码器提取高保真潜在特征，并结合未训练的FPN结构进行多尺度特征融合与高分辨率变化映射。系统通过比较连续轨道过境影像在潜在空间中的细微语义偏移来检测异常，整体强调training-free、image-level生成，相比以往patch-based且需训练的方法更轻量。

Result: 摘要声称该方法能够识别广泛类型的异常，在无需定制训练和大量开发工作的情况下取得与专门训练模型相当的结果，并具备跨地形、跨传感器的泛化能力。

Conclusion: 该工作表明，RSFM结合未训练FPN的无监督变化检测可作为灾害监测与星载自主决策的有效方案，在降低标注和训练成本的同时保留较好的分辨率映射与泛化性能。
...
TL;DR: PanoImager 是一个无需 SfM 初始化的稀疏全景 3D 重建框架，通过先验估计、扩散式视图补全和深度引导的 3D Gaussian Splatting 优化，提高弱视差、旋转主导场景下的重建稳定性。


<details>
  <summary>Details</summary>
Motivation: 全景感知具有宽视场优势，但在仅有少量全景图像、且相机运动以旋转为主、视差较弱时，传统 SfM/SLAM 初始化容易病态或失败，导致后续 3D 重建不稳定。

Method: 方法将少量全景图像分解为局部透视视图，利用前馈式位姿与深度先验提供初始几何信息；再通过几何条件约束的扩散模型合成辅助视角以增加观测证据；最后采用深度引导的 3DGS 优化来稳定高斯表示并提升跨视角一致性。

Result: 在多个基准数据集上的实验表明，该方法在极端稀疏输入条件下比依赖 SfM/SLAM 初始化的方法具有更好的稳定性和重建一致性。

Conclusion: PanoImager 适合作为 SfM/SLAM 初始化失败时的离线或后台地图细化组件，尤其适用于稀疏全景、弱视差和旋转主导的重建场景。
...
TL;DR: 论文提出 CT-3GDINO：一种轻量级、基于查询的 3D 腹部 CT 器官定位检测器，用冻结伪文本类别 token 替代真实文本编码器，实现肝、脾、左右肾和肠道的 3D 框定位。


<details>
  <summary>Details</summary>
Motivation: 可靠的腹部 CT 器官定位可为后续创伤分析提供空间先验；现有方法需要更适合固定器官类别、轻量且可开源复现的 3D 定位基线。

Method: 模型借鉴 Grounding-DINO 的查询式检测框架，采用 Swin3D 视觉骨干、双向特征增强、伪文本引导的查询选择以及跨模态解码器，输出归一化 3D 边界框；使用由分割标注生成的器官框进行训练与评估。

Result: 在 193 例匹配 RSNA/RATIC CT 体数据上，最佳多尺度从零训练模型在 IoU 0.1–0.7 的 overall top-1 class-wise mAP 达到 0.5830，优于固定骨干和可训练骨干的分类预训练变体；IoU 0.1 AP 为 0.9649，IoU 0.7 AP 为 0.1552。

Conclusion: CT-3GDINO 可作为伪文本条件化 3D 器官定位的开源基线；其粗定位能力强，但严格框对齐仍有限，未来需探索定位感知预训练、更丰富的多模态条件和面向损伤的检测。
...
TL;DR: SubdivAR 将网格细分建模为跨尺度自回归预测，通过 MAR 表示与混合拓扑感知 Transformer，在保持细分拓扑的同时恢复更精细的几何细节。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的网格细分方法依赖固定局部规则，容易造成表面过度平滑；已有神经细分方法虽能合成细节，但仍受限于局部建模，泛化能力不足。论文希望通过全局—局部结合的学习框架提升细分质量与复杂几何鲁棒性。

Method: 提出 Mesh Autoregressive Representation（MAR），将不同细分层级的网格组织为有序尺度序列，把细分转化为下一尺度预测问题；设计 Hybrid Topology-Aware Transformer，结合全局语义注意力与拓扑约束的局部特征聚合；采用逐级预测顶点坐标偏移的方式，在保持细分拓扑结构的同时恢复细粒度几何。并构建 FII-40K 多层级监督数据集用于训练。

Result: 实验表明，SubdivAR 优于现有最先进基线方法，Hausdorff Distance 降低 18.8%，Chamfer Distance 降低 14.2%，并在复杂开放曲面几何上表现出较强鲁棒性。

Conclusion: 该工作将网格细分从局部规则或局部神经建模推进到跨尺度自回归建模，证明了全局语义建模与拓扑局部聚合结合对高质量神经细分的有效性。
...
TL;DR: 论文提出TMP树结构混合策略剪枝框架，用于压缩大规模图像生成模型，在较小质量损失下显著降低参数量与显存需求。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型为提升高保真合成能力持续扩大规模，导致参数量、计算预算和GPU显存占用过高，部署成本昂贵，尤其限制了大模型在消费级硬件上的推理可用性。

Method: 提出Tree-structured Mixed-policy Pruning（TMP）框架，面向文生图与图像条件生成任务，并兼容MoE与Diffusion Transformer架构；该方法可作为步数蒸馏模型后的最终压缩阶段，通过树结构与混合剪枝策略对模型进行结构化压缩，并结合工程优化支持低显存推理。

Result: 在HunyuanImage-3.0 instruct上将模型从80B压缩到20B，参数减少75%，生成质量仅有限下降；通过工程优化使20B版本可在单张24GB RTX 4090 GPU上推理。另在Z-Image turbo上将6B压缩到4B，减少33%，性能退化可忽略。

Conclusion: TMP展示了对不同图像生成任务和主流架构的通用性与有效性，可显著降低大规模生成模型的部署门槛，并在保持较好生成质量的同时提升推理可访问性。
...
TL;DR: FlameVQA 是一个面向无人机野火监测的多选视觉问答基准，结合 RGB 与热红外图像，用于评估多模态大模型在灾害场景中的安全关键推理能力。


<details>
  <summary>Details</summary>
Motivation: 无人机野火监测场景复杂，烟雾、尺度变化和遮挡会削弱仅基于 RGB 图像的理解能力；实际应用需要模型能够结合热信息进行检测、定位、覆盖估计和飞行规划等可靠推理。

Method: 作者基于 FLAME 3 构建 FlameVQA，使用配对 RGB 图像和辐射热 TIFF 数据；每张图像设计 34 个多选问题，覆盖六类操作能力。标注流程结合多模态大模型辅助、确定性热规则、跨问题一致性检查以及人工审核，以提升标签可靠性，并评测代表性 MLLM 作为基线。

Result: 实验显示，当问题包含明确跨模态线索时，模型表现较强；但在浓烟遮挡下的火情存在性检测以及覆盖范围估计任务上仍存在明显失败。

Conclusion: 当前多模态大模型尚不能充分满足野火/灾害监测中的鲁棒推理需求，需要面向领域的适配与训练；FlameVQA 为后续研究提供了开放基准和代码。
...
TL;DR: 论文提出一种用于自回归统一多模态文生图模型的迭代自改进码本方法，通过模型自我识别不安全生成结果并更新视觉token码本，在无需外部人工反馈的情况下提升生成安全性。


<details>
  <summary>Details</summary>
Motivation: 自回归多模态模型通过离散视觉token逐步生成图像，具备较强文本条件建模能力，但其生成图像的安全性尚不明确；由于有害映射可能隐含在视觉码本中，作者希望从码本层面降低不安全输出。

Method: 方法包含两个循环步骤：首先利用统一多模态模型自身的理解与判断能力识别不安全生成图像，构造有害与安全的图文对，并据此建立 Harmful Space 来指导码本更新、消除有害映射；其次在 Harmless Space 中使用安全图文对对码本进行自适应微调，以保持图像质量。重复该过程直至性能不再提升。

Result: 摘要声称该方法能在不依赖额外外部反馈或人工标注的情况下，迭代提升模型安全性，并得到安全增强的模型码本。

Conclusion: 该工作表明，自回归文生图模型的安全性可以通过对离散视觉码本进行自监督式、迭代式修正来改善，同时尽量维持生成质量。
...
TL;DR: 提出 HarmVideoBench，用于评估大视觉语言模型对有害视频的多层次理解，并提出 BCR 方法显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有有害视频评测多采用二分类设定，忽视视频危害的多层语义与隐含上下文；同时缺少解释性依据，难以判断模型是否真正理解危害原因。

Method: 构建包含 1,379 个视频和 4,137 道多选题的 HarmVideoBench，从 Observable Evidence、Clip-Internal Meaning、Beyond-Clip Reasoning 三个层级评估模型；同时提出 BCR，通过预测推理边界并按需动态检索外部上下文来增强理解。

Result: 在 19 个主流模型上的实验表明，BCR 将基础模型宏平均性能从 61.7% 提升至 84.4%，达到当前最佳水平。

Conclusion: 该工作强调有害视频理解应从表层识别走向分层诊断与可解释推理，HarmVideoBench 和 BCR 为更可靠的自动内容审核模型评估与改进提供了工具。
...
TL;DR: 论文提出 LISA：通过将侧分支中间特征显式对齐到近似似然分数，正则化可控生成中的双分支训练，从而加速收敛、提升生成质量，且推理无额外开销。


<details>
  <summary>Details</summary>
Motivation: 现有视觉条件可控生成广泛采用双分支范式：冻结预训练主网络，并训练侧网络编码视觉条件、融合中间特征。但侧分支在 score-based 生成建模中的具体作用及其训练效率缺乏系统理解，导致训练可能不够高效、条件建模特征不够解耦。

Method: 作者从分数生成建模视角重新解释该范式：主网络提供先验无条件分数以保持感知质量，侧网络隐式提供似然分数以实现条件控制。在此基础上提出 Likelihood Score Alignment：从侧网络指定层提取特征，经轻量解码器投影到 score latent space；构造近似似然分数作为监督目标；将解码器输出与目标之间的距离作为额外正则化损失，并与标准扩散损失共同优化侧网络与解码器。

Result: 在多种图像/视频任务、不同架构以及 diffusion/flow 模型上，LISA 都能稳定加速训练收敛，提升最终合成效果，并促进侧网络特征更适合条件建模、更具解耦性。其额外训练成本很小，推理阶段无额外成本。

Conclusion: LISA 为双分支可控生成提供了更清晰的理论解释和实用正则化策略，通过显式似然分数对齐提升侧网络学习效率与条件控制能力，是一种低成本、通用性较强的训练增强方法。
...
TL;DR: SatSplatDiff 面向卫星影像三维重建，通过几何约束与阴影引导的生成式细化，在提升高斯渲染视觉质量的同时抑制生成模型带来的几何退化。


<details>
  <summary>Details</summary>
Motivation: 卫星影像主要来自顶部视角，建筑立面监督不足，容易产生表面空洞和视觉质量下降。现有生成式细化逐视图独立处理，可能引入幻觉并破坏多视图照片一致性，从而损害几何精度。

Method: 方法基于 SatSplat 的摄影测量 DSM 初始化和 2DGS 阴影投射，加入单目深度监督与多尺度几何细化以获得更准确、正则化更好的表面表示；随后使用几何计算得到的阴影图引导生成式细化，使渲染图像在提升视觉保真的同时保持与底层几何一致。

Result: 在 IARPA2016 和 DFC2019 数据集上达到当前最优表现：几何 MAE 最多降低 18%，FID-CLIP 视觉保真指标相较基线提升 28–45%；支持最高 5 倍分辨率增强，并保持较少幻觉、传感器一致外观和跨瓦片一致性。

Conclusion: SatSplatDiff 证明了将几何监督、阴影约束与生成式先验结合，可以有效缓解卫星 3D Gaussian Splatting 中的视角监督不足问题，在大规模卫星三维重建中兼顾几何准确性、视觉质量和可扩展性。
...
TL;DR: CORTEX 是一个面向 3D 胸部 CT 的结构化推理基准，在 CT-RATE 基础上为 VQA 与报告生成任务补充经验证的四阶段诊断推理轨迹，用于训练和评估可信的医学多模态大模型。


<details>
  <summary>Details</summary>
Motivation: 现有 3D 胸部 CT 问答数据集多将放射科报告简化为答案对，缺少从影像发现到诊断结论的可追溯推理过程，也缺少患者病史等临床上下文，导致模型推理难以解释、验证和监督。

Method: 提出四阶段诊断轨迹：任务理解、视觉观察、诊断推理、答案合成；利用前沿大语言模型生成推理链，并结合自动化分阶段 rubric 评分与放射科专家审核进行过滤和验证；推理结构与评价准则由临床专家参与设计。

Result: 构建了包含 76,177 条验证推理轨迹的数据集，覆盖开放式 VQA、封闭式 VQA 和报告生成任务，并提供结构化监督与分阶段评估协议。

Conclusion: CORTEX 为 3D 胸部 CT 推理型 MLLM 的训练、解释性评估和可信验证提供了数据与协议基础，有助于推动可追溯医学影像推理模型的发展。
...
TL;DR: DanceOPD提出一种面向flow-matching模型的on-policy生成场蒸馏框架，用于统一文本生成、局部编辑、全局编辑等能力，并缓解多能力训练中的相互干扰。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型希望用单一模型同时具备T2I、局部编辑和全局编辑等能力，但这些能力往往不天然对齐，联合训练时会互相冲突，例如编辑能力可能损害T2I质量，局部与全局编辑也可能相互干扰。因此需要一种有效组合多种生成/编辑能力的训练方法。

Method: 作者将每种能力源建模为共享flow状态空间上的速度场，提出DanceOPD：对每个样本路由到一个能力场，在学生模型自身rollout产生的低噪声状态上查询专家/能力场，并使用简单的速度均方误差目标进行蒸馏训练。该框架还可吸收算子定义的场，例如classifier-free guidance。

Result: 在文本到图像、图像编辑、真实感场吸收和CFG吸收等实验中，DanceOPD改善了多能力组合效果，在增强目标能力的同时保留了基础/锚定生成质量。

Conclusion: 该工作表明，在flow-matching模型中进行on-policy生成场蒸馏是一条实用路径，可更稳定地整合多种图像生成与编辑能力，并降低能力间冲突。
...
TL;DR: 本文提出一种用于局部图像补丁描述子的确定性精确最近邻检索方法，在只对少量数据库条目进行完整向量计算的情况下，返回与穷举全向量搜索完全一致的最近邻。


<details>
  <summary>Details</summary>
Motivation: 现有ANN方法如HNSW、IVF-PQ通常以牺牲精确性换取速度，并且结果可能受运行顺序、线程数或硬件影响而不完全确定。作者希望在保持穷举搜索精确性的同时显著降低计算量，并提供可复现的确定性检索结果。

Method: 核心机制是Hierarchical Normalization（HN）：将归一化前特征向量拆分为K维major分量和128-K维minor分量，其范数分别固定为sqrt(1-alpha)与sqrt(alpha)。由于minor部分内积由Cauchy-Schwarz不等式上界为alpha，因此“major相似度+alpha”可作为完整相似度的可采纳上界。检索时先对所有数据库项扫描K维major分量，再仅对无法被该上界剪枝的候选项进行完整128维评估，形成精确的branch-and-bound扫描。作者训练HN改造的HardNet，并采用缓存优化的Structure-of-Arrays布局。

Result: 在UBC patch数据集notredame上训练，并在trevi和halfdome上测试。K=8、alpha=1/32时，相比128维暴力搜索分别获得13.7倍和12.7倍加速，仅约0.4%的条目需要完整评估。K=16、alpha=1/8时，在trevi上FPR@95从0.0062轻微升至0.0064，同时获得7.2倍加速，98.8%的条目跳过完整评估。

Conclusion: 该方法通过HN构造严格上界，实现了确定性、可证明精确且高效的补丁描述子最近邻检索。它在精度几乎不变的情况下显著减少完整向量计算，是一种介于暴力精确搜索与ANN之间、兼具可复现性和速度优势的方案。
...
TL;DR: 本文提出一种面向3D生成的RLHF微调方法，直接基于NeRF辐射场密度值学习奖励信号来改善几何形状，无需网格、形状先验、多视图渲染或文本条件。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成RLHF方法多依赖显式表面表示，常需将辐射场转换为网格，并使用大量表面监督数据；这增加了流程复杂度，也限制了对隐式辐射场生成模型的直接优化。

Method: 在预训练的3D-aware生成模型EG3D上进行微调。作者构建一个无需预训练、仅用少量偏好样本训练的奖励模型，使其直接读取NeRF连续3D密度场σ并提供纯几何奖励。同时加入密度一致性约束，以在重塑几何时尽量保持2D外观一致。

Result: 微调后生成器在几何质量上显著提升：在用户两两比较中，微调模型生成的人脸几何有74.4%的比例被偏好。代价是分布质量有一定下降但受控，FID-50k从4.09上升到6.66。

Conclusion: 该工作证明了可直接在隐式辐射场密度空间中进行基于人类偏好的3D几何优化，不依赖外部网格或形状先验；少量偏好数据即可带来稳健几何改进，但会带来一定生成分布质量损失。
...
TL;DR: 论文提出 SmellNet-V 视觉-嗅觉数据集与 See & Sniff 自监督框架，通过语义对齐的网络图像与气味样本构造跨模态学习任务，并实现气味分类、检索与像素级气味定位。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型主要覆盖视觉、语言、音频和触觉，嗅觉模态因缺乏成对视觉-嗅觉数据而研究不足。作者希望降低真实共采集成本，建立可扩展的视觉-嗅觉学习基准。

Method: 利用同一语义类别内气味身份对视觉变换相对不变的假设，将仅含气味的样本与语义匹配的网络自然图像进行合成配对，构建 SmellNet-V。基于该数据集提出 See & Sniff 自监督框架，通过稠密局部对齐学习联合视觉-嗅觉表征，并生成气味显著图以支持空间定位。同时定义像素级气味定位任务和评测基准。

Result: 该方法在仅用气味进行分类时相比 smell-only 基线提升 7%，并能泛化到跨模态检索和气味定位任务。

Conclusion: 该工作表明无需昂贵共采集也可构建视觉-嗅觉跨模态数据与任务，验证了视觉-嗅觉联合表征学习的可行性，并将其确立为多模态感知中的新研究方向。
...
TL;DR: ViQ提出一种视觉量化表示框架，旨在用离散视觉 token 同时保留高级语义与低级细节，并提升多模态训练效率。


<details>
  <summary>Details</summary>
Motivation: 统一文本与视觉的离散表示有助于简化多模态建模并提高训练效率，但将图像像文本一样离散化通常会造成信息损失；现有方法难以同时兼顾重建细节和语义表达能力。

Method: 方法包含两个阶段：首先进行文本对齐预训练，借助预训练语言模型为视觉编码器提供语义监督，并支持原生分辨率输入；随后进行特征离散化，采用近端表示学习逐步压缩特征空间，并设计位置感知的 head-wise 量化机制以灵活处理任意分辨率图像。

Result: 在多模态任务上，ViQ相较使用连续高维视觉特征的先进视觉编码器取得有竞争力的性能，同时保持较高的低层重建精度；使用视觉量化表示进行多模态训练可带来约20%到70%的加速，具体取决于基础LLM和训练配置。

Conclusion: ViQ表明离散视觉表示可以在语义、细节与效率之间取得较好平衡，有潜力作为任意视觉输入的统一通用表示，并降低多模态模型训练成本。
...
TL;DR: OctoSense 提供一个开源多传感器平台与 59 小时时间同步机器人/自动驾驶数据集，并提出 late-fusion masked autoencoder，用于多模态自监督表征学习，在多项感知与运动估计任务上优于图像-only 基础模型且具备较强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 真实机器人场景中的传感器模态多样，具有不同表示形式、采样频率、延迟和噪声特性；单一图像模型难以充分利用 LiDAR、事件相机、热成像、IMU、GPS、车辆/机器人本体感知等互补信息，尤其在夜间或传感器退化时鲁棒性不足。

Method: 构建 OctoSense 开源传感器平台，采集包含 stereo RGB、event cameras、LiDAR、thermal camera、IMU、RTK-GPS 和 proprioception 的 59 小时时间同步数据；提出 late-fusion masked autoencoder，使用模态特定 tokenizer 建模不同传感器的时空特征，并在推理时缓存各模态 token，以便异步处理新到达测量。

Result: 该方法计算表征速度较快：在 NVIDIA 5090 上为 6.68 ms，在 Orin NX 上为 112 ms；在光流、深度、语义分割、自运动估计（平移、旋转、转向角）等任务上优于现有 image-only foundation models；在夜间和传感器数据退化情况下仍能稳健预测。

Conclusion: OctoSense 展示了面向真实机器人系统的多模态自监督学习价值：通过开放数据集、硬件平台和适配多传感器特性的 late-fusion MAE，可获得高效、鲁棒且可迁移的机器人感知表征。
...
TL;DR: 本文提出 DexAC-WM，将高自由度动作条件从全局压缩改为结构化建模，并结合语义分支提升动作条件视频预测的真实感与动作一致性。


<details>
  <summary>Details</summary>
Motivation: 现有动作条件世界模型通常把完整动作序列压缩为单一表示，适合低自由度控制，但在高自由度灵巧操作中会忽略动作维度间的异质性；大幅运动与细微信号被统一聚合后导致优化不平衡，削弱细粒度动作效果建模与动作保真度。

Method: 提出 DexAC-WM：通过动作 tokenization 保留维度级语义；利用局部细化与全局调制将动作信号与视觉动态对齐；引入语义分支提供对象与场景先验，以增强世界模型的高层语义 grounding 和动态细节捕获能力。

Result: 在 EgoDex 和 EgoVerse 上，语义分支与 DexAC 结合显著提升 FID、FVD 和 PCK，表明生成视频在视觉-时间真实感和动作跟随一致性方面均有改进；此外 DexAC 可迁移到其他 backbone，体现出良好的可扩展性。

Conclusion: 高自由度控制场景下扩展世界模型不仅需要更强视觉表示或更大模型容量，还需要结构化动作建模与语义 grounding 的协同设计。
...
TL;DR: 本文提出一种几何感知的单图像目标移动方法，通过直接操控扩散 Transformer 的位置表示实现几何一致的空间重排。


<details>
  <summary>Details</summary>
Motivation: 单图像中移动目标需要同时处理遮挡、补全新显露区域，并保持阴影、反射和光照等场景级一致性；现有方法在这些方面表现不足，难以在大幅位移时保持真实感和目标一致性。

Method: 方法利用 RoPE 旋转位置编码所形成的结构化空间场，将其扩展为深度感知的 2D/3D 位置表示，从而编码三维空间结构并引导目标受控位移。模型基于合成数据和少量真实图像进行参数高效微调，直接在扩散 Transformer 的位置表征层面进行几何感知编辑。

Result: 在有限真实监督下，该方法能够在大幅空间移动中保持目标身份，合理生成新显露区域内容，并一致更新阴影、照明等场景相关效果；在标准目标移动基准上，各项指标达到当前最优性能。

Conclusion: 通过将 RoPE 扩展为深度感知空间场并显式操控位置表示，本文证明了扩散 Transformer 的位置编码可用于实现更稳定、几何一致的单图像目标移动，提升了图像编辑中的场景级一致性。
...
TL;DR: SAM2Matting 将视频目标分割跟踪器与高保真抠图模块解耦结合，在仅用图像训练的情况下实现视频抠图新 SOTA，并具备良好时序一致性与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 视频抠图同时需要高层次的逐帧目标理解/跟踪与低层次的精细边界、透明度估计，二者存在能力鸿沟。现有方法依赖成本高且覆盖范围窄的视频抠图数据集，可能限制跨域泛化并削弱跟踪鲁棒性。

Method: 提出 SAM2Matting，一种 tracker-to-matting 框架：以基础级视频目标分割跟踪器（如 SAM2/SAM3）负责时序一致的目标跟踪，通过区域提案桥接模块连接跟踪输出与抠图任务，并加入专用抠图头来恢复细粒度 alpha 细节，从而将时序跟踪与精细抠图解耦。

Result: 尽管仅在图像数据上训练，该方法在视频抠图任务上达到新的最优性能；支持多种提示类型，保持较强时序一致性，并在以人为中心及开放场景中表现出稳健泛化。

Conclusion: 该工作表明，借助强基础跟踪器并通过任务解耦与抠图专用组件扩展，可减少对昂贵视频抠图数据的依赖，同时实现鲁棒、泛化性强且高保真的视频抠图。
...
TL;DR: 本文提出 RayPE：将每个视频 token 的 6D Plücker 射线坐标以加性方式注入 Transformer 自注意力的 Q/K，从而把相机射线几何关系显式引入视频扩散模型的位置编码。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散 Transformer 多用 RoPE 在二维图像坐标与时间轴上编码位置，只描述相机采样网格，无法表达场景的三维几何结构；因此模型在相机可控性和跨帧三维一致性上受限。

Method: 作者利用两条相机射线之间的 Plücker reciprocal product 与 Transformer 注意力点积同为双线性形式这一对应关系，设计 RayPE：把 6D Plücker 坐标加性注入 query 和 key，并采用 query/key 翻转排列，使对称恒等配置精确对应 reciprocal product。注意力分数被分解为内容项、几何项以及两个内容-几何交叉项。为适应不同视频数据中的相机平移尺度差异，方法进一步解耦射线方向与矩大小，用 log-magnitude 的学习门控调节编码，并使用 RMSNorm 与 QKNorm 归一化后的内容分支对齐。模块零初始化接入预训练视频 DiT，参数增量小于 0.1%。

Result: 在四数据集混合训练中，该模块提升了相机控制能力、跨帧三维一致性和整体视频生成质量；实验还表明注意力分解中的几何项与两个交叉项均各自必要。

Conclusion: RayPE 是一种轻量、可插拔且与预训练视频 DiT 兼容的几何位置编码扩展，能够在几乎不增加参数的情况下，将显式三维相机射线关系融入注意力机制，从而改善视频生成中的几何一致性与相机可控性。
...
TL;DR: PhysiFormer 将 3D 物体运动预测表述为世界坐标中顶点轨迹的扩散生成问题，可生成物理合理且多样的刚体/弹性体未来运动。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型多在视角相关的像素空间建模，缺乏几何与视角不变性；部分神经物理方法依赖特设潜空间或显式刚性、因果约束。论文希望探索是否可直接在世界坐标的 3D 网格顶点空间中学习物理动态，并以概率方式表达不确定性。

Method: 提出 PhysiFormer：输入初始顶点位置、速度及材料类型（刚体或弹性体），用单次去噪扩散过程直接采样未来顶点轨迹。模型采用 Transformer，并将注意力在时间、空间和物体维度上因子化，以提升效率；同时支持多物体置换不变推理，无需显式对象编码。

Result: 在超过 10 万条仿真轨迹上训练后，模型能生成刚体与弹性力学运动，并泛化到混合材料、未见真实几何形状和更多物体数量的场景；相比自回归基线，在轨迹精度、刚性保持和基于动量的物理一致性方面显著更优。

Conclusion: 坐标空间扩散建模是实现视角不变、几何感知世界模型的有前景方向，可服务于机器人、图形学和物理设计等应用。
...
TL;DR: 提出一种训练无关、无需外部奖励模型的自指导机制，用于缓解条件流模型在同一条件下多次生成时的多样性坍缩。


<details>
  <summary>Details</summary>
Motivation: 现有流模型在文本或图像条件生成中质量很高，但同一条件下批量采样容易产生相似结果；已有方法要么效果有限，要么依赖外部奖励模型并带来较大推理开销。

Method: 在批量生成过程中对流模型内部特征施加 feature self-guidance，使样本特征彼此分散；同时引入流形正则化，将被分散的特征投影回数据流形附近，以保持与输入条件的一致性和生成质量。该方法可作为即插即用模块集成到预训练条件流模型中。

Result: 在多步和少步文本到图像、深度到图像、参考图像生成等多个条件流模型上，实验显示该方法显著提升生成多样性，同时保持较好的保真度和条件对齐，并仅增加较小推理成本。

Conclusion: 该工作提供了一种高效、训练无关的内部特征自指导方案，在无需外部模型的情况下缓解流模型的多样性坍缩，兼顾多样性、质量和推理效率。
...
TL;DR: 论文提出 Denoising Attention（DnA），通过正/负查询分离相关与无关特征，在注意力中降低噪声并提升视觉任务性能。


<details>
  <summary>Details</summary>
Motivation: 标准 softmax 多头注意力在视觉感知任务中可能产生噪声注意力模式，导致相关特征被无关或相近但不相关的特征稀释，从而影响模型判别能力与性能。

Method: DnA 使用正查询识别属于正确类别的图像特征，使用负查询识别与目标高度相关但无关的干扰特征；随后将两类交互投影到两个具有更大主角的不同子空间中，以增强子空间分离和特征可分性。

Result: 在 ViT-B 骨干上，DnA 相比基线在 ImageNet-1K 上取得 0.8% 的绝对增益；在视频理解任务中提升 1.8%，在视频 LLM 场景中提升 0.5%。

Conclusion: DnA 通过显式建模正负特征交互并进行子空间分离，有效降低注意力噪声，提升视觉理解模型的判别能力和跨任务表现。
...
TL;DR: 论文提出 VISE，一种无需标注、外部奖励模型或多角色机制的视觉不变性自进化框架，用于缓解大多模态模型在视觉推理中的“视觉欠条件化”问题，并在多项视觉语言基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有自进化大多模态模型常通过多角色自博弈和自一致性奖励优化答案一致性，但这种机制不能保证解码器真正关注图像内容，容易依赖语言统计先验生成看似一致的答案，导致图像描述、视觉问答等任务表现受限。

Method: 作者提出 VISE，通过两个互补的不变性奖励直接正则化模型的视觉条件化策略：几何不变性奖励要求模型在已知空间变换下保持空间一致性；语义不变性奖励通过扰动预测区域，惩罚缺乏视觉证据仍继续生成的行为，促使模型识别证据缺失。该方法仅使用单一模型，在未标注原始图像上进行无监督训练。

Result: 在 18 个基准上进行实验，VISE 使用 Qwen3-VL-2B 作为基座模型时，在 COCO 上提升 +16.85 CIDEr，在 TextCaps 上提升 +19.66 CIDEr，并将目标幻觉降低 5.0 个 Chair-I 点；同时在四个模型家族和不同尺度上具备泛化能力。

Conclusion: VISE 表明，相比单纯优化答案一致性，直接约束视觉条件化策略能更有效缓解视觉欠条件化和对象幻觉问题，从而提升无监督自进化大多模态模型的视觉语言理解能力。
...
TL;DR: 本文提出一种仅使用未标注图像的自进化训练框架，使统一大型多模态模型在视觉理解与图像生成两方面同时提升，无需人工标注、偏好标签或外部奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有支持视觉理解和图像生成的统一LMM通常依赖精心构造的后训练监督信号，例如人工标注、偏好数据或外部奖励/评审模型。作者希望探索模型是否能仅依靠未标注图像和内部一致性信号实现自主改进。

Method: 框架包含三个内部角色：Proposer生成视觉问题，Solver回答并评估问题，Generator合成图像。训练依赖自生成的一致性信号。为稳定训练，提出Solver Token Entropy（STE）作为基于token级预测不确定性的连续难度信号；针对图像生成，设计多尺度内部评估机制，结合问答保真度评分与循环一致captioning，并通过Solver将理解能力与生成评估耦合。该方法适配扩散式BLIP3o、rectified-flow BAGEL和自回归VARGPT-v1.1等架构。

Result: 在八个视觉理解指标上，该方法相较对应基座模型均取得一致提升。在BAGEL上，MMMU绝对提升3.5%，GenEval图像生成性能从82%提升到85%。代码和模型已公开。

Conclusion: 该研究表明，统一LMM可以在不依赖外部监督或奖励模型的情况下，利用未标注图像和内部一致性反馈实现视觉理解与图像生成的共同自我提升，并且该训练范式具有跨不同生成架构的通用性。
...
TL;DR: 提出 HierBias：利用文档级上下文与多任务学习进行句子级媒体偏向检测，在 BABE 和 BASIL 上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有句子级偏向检测方法通常独立分类每个句子，忽略句间上下文信号；而人工标注者会自然利用文档语境来判断句子是否存在偏向及其类型。

Method: 论文定义了上下文条件化偏向概率，并从理论上证明在句间互信息非零时，引入文档上下文可降低句子级分类的 Bayes error；同时给出多任务泛化界，说明联合训练二分类偏向检测与细粒度偏向类型分类可提升小样本效率。模型结构上，HierBias 使用句子级 RoBERTa 编码器、跨句 Transformer 聚合器，以及用于二分类和四分类类型识别的双输出头。

Result: 在 BABE 和 BASIL 数据集上，HierBias 达到 0.853 F1 和 0.723 MCC，相比当前最优偏向检测器分别提升 +2.6% F1 和 +4.3% MCC；McNemar 检验显示差异显著，p < 0.05。消融实验表明各理论组件均有独立且稳定的贡献。

Conclusion: 该工作表明，文档级上下文建模和多任务学习能有效提升句子级媒体偏向检测性能，尤其适用于标注数据有限的场景；HierBias 在理论与实证上均支持上下文条件化检测框架的有效性。
...
TL;DR: 本文提出一个面向大语言模型可靠性评估的污染感知、多区域基准，用于区分可支持回答、应 abstain 的未知问题、数据污染、提示敏感性与泛化拒答行为。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 评估容易将模型的真实知识、无依据猜测、训练数据污染、提示模板影响以及泛化拒答混为一谈，难以可靠衡量模型何时应回答、何时应 abstain。

Method: 构建包含 1,200 个样本、覆盖五个领域的多区域基准，设置冻结的构建时标签、显式 abstention 期望、污染风险元数据，并提供严格官方解析器与归一化鲁棒解析器；在 FLAN-T5、Qwen2.5-Instruct、Llama-3-Instruct 上使用固定 answer-or-abstain 提示、answer-only 对照和提示模板变体进行评估。

Result: 基准不能被简单的通用非回答策略解决；FLAN 系列在有效 abstention 上较弱，较强的指令微调模型表现出从回答到 abstain 的选择性但不完整转变。Qwen2.5-3B-Instruct 整体可靠性最好，但可回答区域仍困难，校准较差，且存在良性样本误拒答。提示与解析器鲁棒性分析保持了主要排名和定性结论。

Conclusion: 该基准提供了一个可复现协议，用于将 LLM 可靠性中的可回答性、abstention、拒答与污染风险作为相互关联但可区分的维度进行审计。
...
TL;DR: 本文评估大语言模型在机械工程静力学题目中的解题能力，发现其在纯文本题上表现较好，但加入图示和多步推理后准确率明显下降。


<details>
  <summary>Details</summary>
Motivation: 现有关于大语言模型教育影响的研究多依赖公开题库，缺少面向具体学科与题型的系统分析；机械工程教育中，尤其是静力学问题上，LLM 的能力边界尚不清晰。

Method: 研究采用模型蒸馏流程，从 ChatGPT 中提取 25 道纯文本静力学问题，并进一步构建两个扩展数据集：一个加入图示，另一个修改数值参数；随后比较 LLM 在不同题型条件下的表现。

Result: 实验显示，LLM 在纯文本静力学问题上准确率较高；当题目包含图示并需要多步推理时，准确率下降。进一步分析表明，下降主要并非来自图像识别失败，而是来自多步推理困难以及在连续解题步骤中稳定使用视觉信息的能力不足。

Conclusion: LLM 在工程教育中的基础文本解题能力较强，但面对带图示、需连续推理的静力学任务仍存在明显局限；未来需要提升其多模态信息整合与多步推理稳定性。
...
TL;DR: 该研究考察动物福利倡导文本中的语言特征如何影响经微调后的 Llama-3.2-1B 对亲动物福利推理的偏好，发现多数显性立场表达会增强该偏好，而含糊或中性描述会削弱它。


<details>
  <summary>Details</summary>
Motivation: 随着倡导性文本越来越多地进入大语言模型训练语料，研究者希望理解不同写作方式是否会系统性改变模型在相关议题上的推理倾向，尤其是哪些语言特征会强化或稀释文本中的立场信号。

Method: 研究使用词汇匹配的立场对照探针，并在保留的动物福利基准上评估；将包含十种不同语言特征的文本作为微调数据，测量每种特征对 Llama-3.2-1B 亲动物福利推理偏好的影响，并进行统计显著性检验。

Result: 十种特征中有八种产生统计显著影响。七种特征增强亲动物福利推理：坚定确定的表达、明确道德词汇、情绪词、评价性判断、叙事结构、伤害严重性的呈现、即时性时间框架。两种特征削弱该倾向：模糊/保留性措辞和具体感官描写。第一人称视角没有显著影响。

Conclusion: 论文认为，进入 LLM 训练语料的倡导性文本若想更有效传达立场，应明确表达判断和价值立场，而不是仅作中性场景描写；显性立场特征更能影响模型偏好，缺乏立场的内容则会稀释相关倾向。
...
TL;DR: 本文提出 ContextForge，通过结构化查询生成、外部记忆检索与受控合成实现长对话中的上下文回收，在不重放完整上下文的情况下复用任务相关信息。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在短上下文推理中表现较强，但在长轮次对话中受限于上下文窗口与低效 token 使用，容易出现性能下降、信息遗忘和一致性变差。因此需要一种无需扩大上下文窗口或重新训练模型的长程上下文管理方法。

Method: ContextForge 将对话中的任务相关信息转化为结构化查询，结合外部记忆机制检索历史关键信息，并通过受控合成生成当前轮次所需上下文，从而避免完整历史上下文重放并降低 token 开销。

Result: 在一个包含 15 轮对话的基准测试上，该系统覆盖多轮推理、回指理解和领域转移等场景。相较使用相同底层模型的基线代理，ContextForge 提高了回答一致性、减少了 token 消耗，同时保持了相近的响应准确率。

Conclusion: 上下文回收是一种实用的长程任务增强方案，可在不依赖更大上下文窗口或模型再训练的情况下扩展 LLM 的长对话能力。
...
TL;DR: 本文研究如何通过基于非暴力沟通（NVC）的轻量级提示约束，引导大语言模型在高情绪张力的人际对话中减少升级性回应，提升对话稳定性与可信度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全研究多关注显性危害内容的避免，但对模型在冲突、挫折和痛苦等情境中可能无意加剧矛盾的对话行为关注不足。因此，作者希望探索是否能用简单的沟通原则降低对话升级风险。

Method: 作者将NVC原则重构为面向过程的提示指南，包括避免归因责备、关注用户情绪体验、在给建议前先澄清问题等；并采用双智能体仿真框架，在多个指令微调模型和不同用户抵触水平下进行评估。

Result: 实验显示，NVC约束提示能一致性地降低对话升级程度，并在面对高抵触用户时稳定互动过程。

Conclusion: 简单的提示级沟通约束即可显著改善LLM在冲突易发场景中的对话可信度，说明面向互动过程的安全设计是LLM安全研究的重要补充。
...
TL;DR: 本文提出 NEST-V1，一个面向尼泊尔语手语的情感条件多模态原型系统，可从语音输入生成带情感表达的手语虚拟人，初步结果验证了低资源场景下的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有手语通信系统较少整合情感表达，尤其在低资源语言中研究不足；作者希望探索面向听障群体的、能保留语音情绪信息的尼泊尔手语生成框架。

Method: 研究构建轻量级多模态架构，使用共享声学编码器同时执行自动语音识别与情感分类，并将识别到的词语和情绪状态用于生成情感条件的尼泊尔手语虚拟人。实验覆盖4个常见尼泊尔词汇和3种情绪状态，数据集包含50名说话者的600条标注音频。

Result: 系统在试点数据上达到81.1%的ASR准确率和79.21%的情感识别准确率；相比独立模型架构提升37%参数效率，总参数量为22.1M，具备边缘部署潜力。

Conclusion: 该研究证明了在低资源语言环境中构建实时、情感表达型手语通信系统的技术可行性，为后续扩展到更大词汇量、更多情绪类别和更复杂手语表达奠定基础。
...
TL;DR: 本文提出 AdvCluster，用于自动分析大模型相较小模型在推理任务中的稳定优势，并发现“约束引导推理”是跨领域的核心差异。


<details>
  <summary>Details</summary>
Motivation: 尽管更大的语言模型在数学、物理、化学和编程等推理基准上持续优于较小模型，但这种性能差距背后的具体推理差异尚未被充分研究。

Method: 作者构建 AdvCluster 框架：首先识别大模型稳定优于小模型的问题；然后从大小模型成对推理轨迹中抽取细粒度优势描述；最后通过语义聚类、定量评估和评审模型筛选，形成系统化优势分类。

Result: 实验显示 Qwen3-32B 平均比 Qwen3-8B 高 6.43%，GPT-OSS-120B 平均比 GPT-OSS-20B 高 7.38%。分析得到一套大模型推理优势分类，包括跨领域通用优势和特定领域专门优势。

Conclusion: 更大模型的关键优势在于更强的约束识别、结构化组织、不可行路径排除和中间步骤验证能力，即“约束引导推理”能力。
...
TL;DR: 本文提出一种将结构化词汇资源（以 Hindi WordNet 为例）转化为专业对话式 AI 系统的流程，用于缓解低资源语言缺乏大规模语料的问题。


<details>
  <summary>Details</summary>
Motivation: 低资源语言难以获得足够的大规模训练语料，导致专用对话系统开发受限；已有专家整理的 WordNet 等结构化语言资源尚未被充分用于构建面向任务的 AI 系统。

Method: 作者将 Hindi WordNet 转换为约 125 万条多样化 instruction-response 数据，使用 LoRA 与 4-bit 量化对 12B 参数语言模型进行资源高效微调，并以印地语学习聊天机器人作为应用场景进行评估。

Result: 基于结构化知识构建的系统在教学有效性上优于通用模型，得分为 91.0，而对比通用模型为 79.4–83.6；同时保持了有竞争力的语义表现和较高的一致性。

Conclusion: 该研究验证了 WordNet 等专家词汇数据库可作为低资源语言专用 AI 系统的有效基础，为拥有 WordNet 资源的多种语言提供了一种低语料依赖、可迁移的开发范式。
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型依赖双向注意力，在文本生成中具有潜力，但其计算复杂度随序列长度呈高阶增长，且难以像自回归模型一样使用 KV cache；现有静态缓存或并行解码方法无法适配不同层、不同解码步骤中 token 动态变化，限制了长序列和实时场景部署。

Method: 提出 Dynamic-dLLM，一个 plug-and-play、training-free 的推理加速框架。其包含两部分：Dynamic Cache Updating（DCU）根据层级 token 动态自适应分配缓存更新预算；Adaptive Parallel Decoding（APD）动态校准解码阈值，在生成质量与效率之间取得平衡。

Result: 在 LLaDA-8B-Instruct、LLaDA-1.5、Dream-v0-7B-Instruct 等模型以及 MMLU、GSM8K、HumanEval 等基准上进行实验，Dynamic-dLLM 平均推理加速超过 3 倍，并能维持模型性能，优于现有加速方法。

Conclusion: 该工作表明，针对 dLLM 推理过程中的 token 与层级动态特征进行自适应调度，可以显著降低推理开销；Dynamic-dLLM 为扩散大语言模型的高效部署提供了无需训练、易集成且性能稳定的解决方案。
...
TL;DR: 本文研究在仅给出研究问题的最小提示下，不同大语言模型对计算机科学论文方法建议的默认偏向；发现模型建议显著收缩方法搜索空间，并共同偏向少数更常见的模型/方案。


<details>
  <summary>Details</summary>
Motivation: LLM正被越来越多地用于辅助科研方法设计，但其在低信息提示条件下的默认方法倾向尚不清楚。作者希望评估LLM初始建议是否会系统性偏离真实论文中的实验方法分布，从而影响研究者的方法探索范围。

Method: 作者从1,000篇近期arXiv计算机科学论文中抽取研究问题，分别提示GPT-5.1、Gemini 3 Pro和DeepSeek-V3.2生成方法建议；同时从原论文中抽取实验方法清单。随后将两类结构化方法特征映射到统一分类体系，并用Jensen-Shannon divergence、有效模型实体数、秩相关、流行度基线、BM25检索校准和论文级相似度测试等指标进行对比分析。

Result: 最大的不平衡出现在模型提供方选择维度，其Jensen-Shannon divergence约为其他分类维度的3–5倍。单次出现的其他/学术模型被低估23–24个百分点，而复用的学术/社区模型略高估4–6个百分点。LLM建议的方法范围明显更窄：有效模型实体数从论文中的1,232收缩至59–96。不同LLM之间的秩相关为0.55–0.68，通常高于LLM与论文之间的相关性0.33–0.56，说明偏差在多个模型间具有共享性。

Conclusion: 在仅基于研究问题生成方法建议时，LLM并非简单复现论文方法分布，而是给出针对查询但经过明显收缩和集中化过滤的方案。若研究者不进行交叉核验，可能会被引导到更狭窄、更集中且更偏默认流行选择的方法搜索空间。
...
TL;DR: 本文是一篇关于多模态大语言模型中统一视觉-语言感知能力的系统综述，旨在弥补现有研究将视觉与语言割裂讨论的不足。


<details>
  <summary>Details</summary>
Motivation: 现有综述多分别关注视觉或语言能力，缺少从统一视觉-语言模态角度系统审视 MLLM 感知能力演化的工作；随着新一代模型推动感知中心智能发展，亟需统一框架来梳理该领域。

Method: 作者将 MLLM 感知形式化为类似人类先天感知的内在统一视觉-语言能力，并提出五阶段分类体系，回顾各阶段代表性方法、关键里程碑，同时总结开放挑战与未来方向。

Result: 论文提供了首个面向 MLLM 统一视觉-语言感知的系统综述框架，建立了感知能力演化的阶段化图谱，并归纳了通向更通用多模态智能的研究路线。

Conclusion: 该综述为理解 MLLM 中感知能力的本质、发展脉络与未来挑战提供了基础性视角和可操作路线图，对推动统一多模态智能研究具有参考价值。
...
TL;DR: 本文基于北京普通话与台湾普通话的自发会话语料，研究普通话轻声/浮动调的语音实现，认为轻声并非单纯弱化或无调，而具有自身音高目标，并可被视为一种词汇声调。


<details>
  <summary>Details</summary>
Motivation: 普通话轻声性质长期存在争议：它被描述为弱化声调、固定词汇调或无调；其在单双音节词中的分布和语音实现也不够清楚。研究旨在用大规模自然会话语料检验轻声是否具有稳定音高目标、是否受前一音节声调影响，以及是否存在词特异性音高模式。

Method: 作者采用语料库方法，分析北京普通话和台湾普通话自发会话语料中轻声词的音高轮廓；比较双音节词中第二音节轻声与前一音节声调之间的关系；考察词特异性音高签名，并使用上下文化词嵌入预测这些音高差异及两种变体之间的差异。

Result: 研究发现轻声具有自身的音调目标；双音节轻声词的音高轮廓包含受第一音节声调影响的成分；轻声词存在词特异性音高签名，且这些签名在一定程度上可由上下文化嵌入预测。北京普通话和台湾普通话在轻声实现上既有相似性，也存在差异，部分差异可能与语料中词义使用差异有关。

Conclusion: 作者提出，鉴于轻声和普通词汇声调一样具有目标音高、上下文依赖性、词特异性以及可由语义表征部分预测的变异，轻声应被视为北京普通话和台湾普通话中的一种词汇声调，而非纯粹无调或简单弱化形式。
...
TL;DR: 本文评估小型语言模型（SLMs）在社会-物理人机交互系统综述中进行标题与摘要筛选的辅助价值。


<details>
  <summary>Details</summary>
Motivation: spHRI领域快速发展，但术语碎片化、方法不一致，导致大规模系统综述的文献筛选与综合困难；研究希望探索本地运行、低成本的SLMs是否能提升筛选效率与可持续性。

Method: 在一个大规模spHRI系统综述任务中，使用参数量小于1.5B的小型语言模型对论文标题和摘要进行筛选，并将其表现与人工审稿者进行比较，同时考察模型集成的补充发现能力与筛选速度。

Result: 单个SLM未达到人工审稿者水平，但模型可本地运行且筛选速度快数个数量级；SLM集成额外识别出39篇人工审稿者遗漏的相关论文，占最终相关数据集的10.29%。

Conclusion: SLMs目前不适合替代专家审稿者，但可作为高效辅助工具，帮助提高大规模文献综述的覆盖率、效率和可持续性。
...
TL;DR: PROFILEFOUNDRY 是一个确定性生成并固定发布的合成人物数据源，包含 10 万个跨 8 个地区的成年合成 Person Objects，用于支撑基础模型在记忆、隐私、文档理解、记录链接和智能体状态等方向的可控评估。


<details>
  <summary>Details</summary>
Motivation: 基础模型研究越来越需要涉及人的结构化与纵向数据，如用户状态、个人历史、关系网络、类联系方式字段、文档与时间更新。但真实用户数据难以负责任地共享、扰动、审计或再分发；独立伪造字段又难以保持跨字段与时间一致性，影响受控评测的有效性。

Method: 论文提出 PROFILEFOUNDRY：一个确定性生成器及固定参考发布集。每个合成人物对象包含类型化当前快照、家庭/亲属/雇主链接、与快照对齐的事件、规范化关系视图和生成溯源信息。作者对数据进行多类证据报告，包括人口边际对比、单对象不变量检查、全局引用与时间闭包检查，以及巧合/溯源筛查。

Result: 发布集包含 100,000 个成年合成人物对象，覆盖 8 个地区；同时包含 709,228 个事件、40,338 个家庭、52,491 个雇主和 518,564 条有向关系边。结果表明该数据集在结构一致性、引用完整性、时间闭包和生成可追溯性方面经过系统检查。

Conclusion: PROFILEFOUNDRY 并非人口真实性模型、渲染文本语料库或正式隐私机制，而是一个可检查、可控、负责任的合成数据源层，适合构建涉及个人记忆、隐私、文档理解、记录链接和智能体状态的下游基础模型评测。
...
TL;DR: 本文提出 ConflictScore，用于衡量模型回答是否充分承认检索/依据文档中的冲突证据，而不只是判断回答是否被支持或反驳。


<details>
  <summary>Details</summary>
Motivation: 现有事实性与忠实性指标通常将回答与依据文档之间的关系简化为支持或矛盾，难以刻画支持证据与反驳证据同时存在的情况，尤其无法识别模型在存在冲突证据时仍作出过度自信陈述的问题。

Method: 作者将模型回答分解为原子声明，并针对每个声明与每篇依据文档进行关系标注；随后聚合得到两个互补指标：ConflictScore-Count（CS-C）衡量存在冲突的声明比例，ConflictScore-Ratio（CS-R）衡量支持证据与反驳证据之间的平衡程度。同时构建 ConflictBench，覆盖歧义、矛盾、观点分歧等多种冲突类型，用于系统评测。

Result: 实验表明，ConflictScore 能够跨领域有效检测存在冲突证据时的过度自信声明，并可作为反馈机制，在 TruthfulQA 上提升模型回答的真实性。

Conclusion: 该工作将事实性评估从单一支持/反驳判断扩展到冲突证据识别与量化，为评估和改进生成模型在复杂证据环境下的可靠性提供了新的指标和基准。
...
TL;DR: 本文提出 ProvenAI，用于将检索增强问答系统的透明性拆解为答案正确性、引用忠实度与文档影响力三个可独立测量的层面，并在 HotpotQA distractor 基准上验证引用与真实影响之间可能存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成系统通常会给出引用，但引用本身并不能证明相应来源真正影响了模型输出。现有透明性评估容易将“被引用”误认为“有贡献”，缺乏对答案正确性、引用是否匹配证据以及文档对输出行为影响的分层度量。

Method: 作者构建 ProvenAI 框架，面向多跳问答任务，将透明性分为三层：答案正确性、相对于基准支持证据的引用忠实度、以及基于 leave-one-resource-out 干预的逐文档影响力。系统包含七阶段流程：数据规范化、检索索引、带引用的答案生成、归因审计、基于消融的影响估计、批量评估和交互式检查，并结合因果中介分析、数据库溯源理论和 token 级 KL 散度目标进行形式化讨论。

Result: 在 HotpotQA distractor 验证集的 7,405 个样例和 509,300 篇段落语料上，系统达到 53.53% 的答案准确率和 71.55% 的平均引用忠实度。案例分析显示，即使引用审计结果良好，也可能存在一个被引用来源影响很弱、而七个未引用来源显著改变输出的“引用—影响差距”。

Conclusion: ProvenAI 表明，检索增强问答的有意义透明性不能仅依赖表面引用，而需要同时追踪检索证据、引用证据和行为上真正有影响的证据。答案正确性、引用忠实度与文档影响力应作为三个独立但可组合的测量层面来评估。
...
TL;DR: 本文提出 AnySimLite，将多种语音邻近的自然语言分类任务统一转化为细粒度文本相似度问题，用单一轻量模型在边缘设备上实现低内存、低延迟的多任务分类。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上的端侧应用需要降低隐私风险与推理延迟；但许多应用涉及多个自然语言分类任务，若为每个任务部署专用模型，会带来较大的内存占用。因此，作者希望探索能否用一个轻量级架构同时处理多种 Speech-Adjacent 分类任务。

Method: 作者提出 AnySimLite，一种轻量相似度编码器，结合词级通道与字符级通道；同时设计数据集转换策略，将不同分类任务规约为更细粒度的文本相似度建模问题，并在多种少样本 SA 分类任务上进行评估。

Result: 实验显示，AnySimLite 在多个少样本设置下稳定达到 SOTA 或接近 SOTA 的性能，同时保持很低的模型内存占用。即使在最差情况下，相比 SOTA qLLaMA_LoRA-7B 基线的性能下降也低于 7%，而模型大小小于其 1/250。

Conclusion: 通过任务规约与轻量相似度编码，单一小模型可以有效覆盖多个 SA 分类任务，在端侧部署场景中兼顾性能、隐私、延迟与内存效率。
...
TL;DR: 论文提出 SOLAR：在监督微调中加入跨语言软 token 表征对齐目标，以英语为枢轴缓解多语言大模型在等价跨语言提示上的推理不一致问题，并在多项多语言推理基准上提升性能。


<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型虽然可能在中间层形成相对语言无关的表征，但生成阶段会逐渐受具体语言词表、脚本和离散 token 选择影响，导致语义等价的跨语言推理路径发生分歧。作者希望找到一种不强依赖单一词汇项或书写系统的跨语言对齐信号。

Method: 方法 SOLAR 在监督微调中引入辅助目标：先将输出分布转化为软 token，即对词表嵌入按概率加权得到连续表征；再以英语为 pivot，将非英语样本的软 token 摘要与对应英语样本在共享嵌入空间中对齐，从而聚合跨语言语义相关 token 的信息并减少词表差异带来的偏移。

Result: 在四个多语言推理基准上，SOLAR 相比基座模型最高提升 17.7 个百分点，相比标准监督微调最高提升 3.8 个百分点；低资源语言上的收益最大。同时，最终层跨语言相似性增强，语言簇可分性显著降低。

Conclusion: 对齐软 token 表征能够在多语言推理过程中更好地保留共享语义结构，降低语言特定生成选择导致的推理分歧，从而提升跨语言一致性与任务准确率，尤其有利于低资源语言。
...
TL;DR: 本文研究从科学论文中抽取“问题句”和“方法句”，通过公式化表达脱敏的数据增强与上下文增强 Transformer，缓解小规模标注数据导致的泛化不足问题，并在两个数据集上取得更高 Macro-F1。


<details>
  <summary>Details</summary>
Motivation: 海量科学论文使自动识别关键信息变得必要。现有问题句与方法句抽取依赖人工标注，数据规模小，模型容易学习到特定公式化表达形式而非真实语义，从而泛化能力受限。

Method: 论文从扩大数据规模、降低对特定表达形式的依赖、增强句内信息三个角度入手：提出公式化表达（FE）脱敏概念，并设计基于 FE 脱敏的数据增强器生成合成数据；提出上下文增强 Transformer，利用上下文评估目标句中词的重要性并抑制上下文噪声；同时评估基于大语言模型的上下文学习方法。

Result: 在两个科学论文数据集上的定量与定性实验表明，所提模型相较基线模型的 Macro-F1 分别提升 3.71% 和 2.67%。实验还发现，基于 LLM 的上下文学习方法并不适合该问题句与方法句抽取任务。

Conclusion: 通过 FE 脱敏数据增强和上下文增强建模，可以有效缓解小规模数据下模型过度依赖固定表达的问题，提升科学论文问题句与方法句抽取性能；相比之下，直接采用 LLM-ICL 并非该任务的理想方案。
...
TL;DR: 本文研究将脑电（EEG）与眼动信号用于微博自动关键词抽取，发现阅读过程中的认知信号能稳定提升模型性能，其中 EEG 增益最大。


<details>
  <summary>Details</summary>
Motivation: 微博文本短小、噪声大且内容分散，使自动关键词抽取具有挑战性。既有研究利用眼动信号刻画读者对显著词的注意，但眼动信号受生理因素、采集条件和特征解码能力限制，因此需要探索 EEG 是否能补充眼动信号，为关键词抽取提供更丰富的认知证据。

Method: 基于 ZuCo 认知语言处理语料库，选取 8 个 EEG 特征和 17 个眼动特征，并将其融入微博关键词抽取模型。为降低模型结构对认知信号的潜在扭曲，作者将这些特征注入软注意力层输入以及自注意力层的 query 向量中，并在不同 AKE 模型和不同认知信号组合下进行评估。

Result: 实验显示，阅读过程中产生的认知信号在不同特征组合和模型架构下均能稳定提升 AKE 性能。EEG 特征带来的提升最大；EEG 与眼动特征联合使用时，性能介于二者单独使用之间，说明两类信号存在一定互补性，但也可能包含冗余或噪声。

Conclusion: EEG 信号能够为微博自动关键词抽取提供有效的认知证据，且多模态认知信号在该任务中具有进一步研究价值；但 EEG 与眼动融合方式仍需优化，以减少冗余和噪声影响。
...
TL;DR: TempoWave提出一种面向LLM时间序列预测的时序小波数字接口，用多小波、多尺度系数构造逐数字嵌入，替代标准数值token表示，从而提升数值表达与预测性能。


<details>
  <summary>Details</summary>
Motivation: LLM具备融合异构文本上下文信号的能力，适合上下文感知时间序列预测；但其离散、面向语言的分词与嵌入机制难以准确表示连续数值，容易破坏数值顺序关系并降低预测可靠性。因此需要一种更适配连续数值与时间结构的接口。

Method: 论文提出TempoWave，一个即插即用的temporal wavelet digit interface。该方法将每个标量观测映射为由多小波、多尺度系数构成的逐数字嵌入，并直接覆盖LLM的标准token表示，使Transformer可同时接收局部细粒度波动与全局宏观结构信息，同时保持数值格式、数字身份区分以及对常见归一化操作的鲁棒性。

Result: 在五个带上下文信息的时间序列预测基准上，TempoWave相较标准数值tokenization和其他嵌入接口均稳定提升LLM预测器性能，并达到新的state-of-the-art结果。

Conclusion: 研究表明，数值接口是LLM用于精确时间序列预测的关键瓶颈；采用有原则的多分辨率数值嵌入能够更好地连接LLM的上下文推理能力与高精度预测能力。
...
TL;DR: 本文比较了两种用于德语气候新闻句子级框架识别的方法：少样本提示的大语言模型与微调的德语BERT；结果显示微调BERT在威胁框架和解决方案框架分类上均优于LLM。


<details>
  <summary>Details</summary>
Motivation: 气候变化新闻中“威胁导向”与“解决方案导向”的报道框架会影响公众参与和政策支持，但大规模人工编码成本高，因此需要可靠的自动化句子级框架检测方法。

Method: 研究在440篇奥地利报纸气候新闻语料上进行人工标注，并比较两类模型：一是使用Llama 4 Maverick进行少样本提示、链式推理、结构化输出和置信度评分；二是微调deepset/gbert-large进行句子对分类，将前一句作为目标句上下文。两种方法均采用两个独立二分类器分别检测威胁框架和解决方案框架。

Result: 微调BERT在威胁和解决方案两个任务上的F1均达到0.83；LLM方法的F1为0.78。消融实验表明，将前一句作为上下文显著提升了BERT相较于单句输入的分类性能。

Conclusion: 在该任务和语料条件下，微调编码器模型比少样本提示生成式模型表现更好；上下文信息对句子级框架分类具有重要作用。研究为计算社会科学中文本分类方法选择提供了实证比较。
...
TL;DR: 本文提出 TwoTower，将自回归上下文建模与扩散去噪解耦，在接近自回归基线质量的同时显著提升生成吞吐。


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型通常用同一个网络同时承担上下文表示和迭代去噪任务，导致模型容量与功能角色相互牵制，限制了上下文建模能力或去噪能力的充分发挥。

Method: 提出块级自回归扩散模型 TwoTower：使用冻结的 AR context tower 以因果方式处理干净 token；使用可训练的 diffusion denoiser tower，通过双向块注意力对噪声块进行迭代细化，并通过交叉注意力利用上下文信息。模型基于 Nemotron-3-Nano-30B-A3B 构建，并在约 2.1T token 上训练。

Result: Nemotron-TwoTower 保留了自回归基线 98.7% 的质量，同时实现了 2.42 倍的实际生成吞吐提升。作者还发布了代码和模型权重。

Conclusion: 将上下文建模和扩散去噪分离是一种有效的扩散语言模型设计，可在保持高生成质量的同时利用并行、迭代生成带来更高推理效率。
...
TL;DR: 本文提出 MemStrata，一种具备时间有效性管理的检索记忆机制，用确定性的主语-关系-宾语覆盖规则淘汰过期事实，显著降低 RAG 在知识演化场景中的陈旧事实错误。


<details>
  <summary>Details</summary>
Motivation: 传统 RAG 缺乏时间建模能力。当事实发生变化时，旧事实与新事实在嵌入空间中往往高度相似，导致检索同时返回过期与当前信息，使模型可能拒答或输出已被替代的事实。作者指出这不是简单调参问题，而是 RAG 结构性缺陷。

Method: 论文提出 MemStrata：像 RAG 一样存储事实以保持静态知识召回；当某个事实值被新值矛盾替代时，使用确定性的 subject-relation-object supersession 规则在双时间账本中标记旧事实失效。该过程不依赖相似度阈值，也不需要 LLM 调用。作者还构建校准数据集、六个本地基准和无标记评估协议来评测知识演化下的记忆系统。

Result: 在校准数据集上，余弦相似度区分矛盾事实与重复事实的 AUROC 仅为 0.59，接近随机。六个基准中，MemStrata 在静态知识上与 RAG 持平，在演化知识上达到 0.95-1.00 准确率，而 RAG 仅为 0.20-0.47。RAG 在必须回答时有 15-40% 概率输出过期值，MemStrata 将该错误率降至约 0%。同时 MemStrata 检索延迟约 2.1 秒，明显低于 LLM 重排序基线的 16-18 秒。

Conclusion: 论文认为，知识演化中的陈旧事实错误是传统 RAG 难以避免的结构性问题。MemStrata 通过显式维护事实的时间有效性和确定性覆盖机制，在不牺牲静态召回的前提下，大幅提升动态知识场景中的可靠性与效率。
...
TL;DR: 本文提出一种纵向文本分析框架，用于评估企业叙事性披露在时间维度上的多维质量变化，并应用于日本2019年披露改革的效果评估。


<details>
  <summary>Details</summary>
Motivation: 企业叙事性披露对资本市场具有重要信息价值，但文本质量具有多维属性，单一指标难以全面刻画其随时间变化的真实情况；某一维度改善可能伴随其他维度恶化，因此需要更综合的分析方法。

Method: 作者结合日语自然语言处理指标提取、配对检验、shift function分析以及指标间相关性分析，构建纵向文本分析框架；同时扩展既有指标体系，引入“横截面相关性”指标，用于衡量风险披露与管理战略之间的主题一致性。实证样本为2015—2024财年19,770个公司—年度观测值。

Result: 研究发现，日本2019年披露改革后，披露文本篇幅显著增加，但可读性下降；整体信息结构有所改善，但具体描述质量停滞；不同市场板块之间的适应程度存在差异。

Conclusion: 该框架能够揭示传统单指标方法容易掩盖的复杂披露变化，说明披露改革带来了数量和结构上的改善，但并未同步提升所有质量维度，尤其在可读性和具体性方面仍存在改进空间。
...
TL;DR: 本文提出“Inattentional Gap（注意盲区差距）”：当语言或视觉模型被限定执行狭窄任务时，会显著减少对同时存在但未被明确要求报告的安全关键线索的报告，从而导致评测安全性与真实安全性脱节。


<details>
  <summary>Details</summary>
Motivation: 现有AI安全评估通常检验模型是否能发现被明确指定的风险，但真实事故往往来自未被预先指定的危险。作者希望揭示模型在任务条件化下是否会忽略本来能够识别的共现安全信号。

Method: 作者在放射学、驾驶文本场景以及胸部X光视觉任务中测试多个语言模型、视觉模型和推理模型；比较模型在“狭窄任务约束”与“非约束/开放报告”条件下对共现安全关键信号的报告率，并分析模型规模、模型家族等因素的影响。

Result: 所有测试模型均出现抑制现象：在狭窄任务条件下，对未指定但安全关键的信号报告率下降；该现象没有随模型规模增大而消失，在推理模型中仍然存在；差异更多由模型家族而非参数规模决定；同一模型在非约束条件下能以显著更高比例报告这些信号。

Conclusion: 论文认为，Inattentional Gap 会使基准测试中的安全表现与真实世界安全性解耦：模型可以在指定风险评测上接近满分，却仍可能忽视真正造成危害的未指定风险。因此，安全评估应覆盖开放式、共现风险与未指定异常报告能力。
...
TL;DR: 论文提出 DiARC，通过构造偏好对让模型区分正确与近似错误解，从而提升开源大模型解决 ARC 类抽象推理任务的能力。


<details>
  <summary>Details</summary>
Motivation: ARC 任务要求模型从少量网格示例中归纳规律并预测输出。现有开源模型方法效果不佳，闭源模型成本较高；已有工作多依赖正样本监督和数据增强，但缺乏利用负样本来提升推理判别能力的机制。

Method: 作者借鉴偏好对齐思想，构建正负样本偏好对训练模型。负样本通过三种方式生成：输出层面的视觉变换、DSL 层面的规则反转、以及任务特定规则编辑。这些负样本在保持输入演示不变的同时，提供接近正确答案但存在关键错误的候选。

Result: 在多个 ARC-like 基准上的实验表明，DiARC 相比基线模型能够稳定提升性能。

Conclusion: 仅依赖正样本监督不足以充分解决 ARC 类推理问题；引入高质量近似负样本并进行偏好学习，可以增强模型对规则正确性的辨别能力，提升抽象推理表现。
...
TL;DR: 论文提出 Erase-then-Delta Attention（EDA），在 delta-rule 线性注意力的写入校正之外，引入独立的定向擦除步骤，从而更有效管理循环记忆中的过时信息。


<details>
  <summary>Details</summary>
Motivation: 现有 delta-rule 线性注意力只能在当前写地址处修正已有内容，无法主动清除存储在其他地址的陈旧信息，限制了 recurrent memory 模型的记忆更新与长上下文建模能力。

Method: EDA 将擦除位置与写入位置解耦：先沿学习得到的 erase direction 对指定记忆地址执行 targeted erase，再沿当前 write direction 执行标准 delta-style corrective write，保留 delta-rule 的校正写入机制，同时增加独立清理路径。

Result: 在 dense 2.5B 和 MoE 25B-A2.8B 语言模型预训练中，EDA 均取得最佳表现；在 MoE 模型经过 80B token 长上下文 midtraining 后，EDA 在 4k 到 128k 上下文评测中仍表现最好。

Conclusion: 实验与记忆状态分析表明，EDA 在不破坏 delta-rule 校正写入的前提下，额外提供了清除陈旧信息的能力，尤其在被动衰减较弱时更有效，说明 recurrent memory 模型应同时决定写入内容以及擦除哪些过时信息和擦除位置。
...
TL;DR: 本文研究用单个双语 LoRA 适配器改进大型预训练 TTS 模型在低资源语言上的语音质量，发现其显著提升高棉语表现，但对基座模型已擅长的韩语无收益甚至可能退化。


<details>
  <summary>Details</summary>
Motivation: 大型预训练文本转语音模型在高资源语言上接近真人，但在训练数据稀缺语言上质量明显下降。论文旨在分析这种质量差距，并探索参数高效适配是否能改善低资源语言表现，同时避免破坏已表现良好的语言。

Method: 作者以 VoxCPM2 为基座模型，构建约 26 小时的共享双语、带语言标签语料，针对高棉语与韩语同时训练一个 LoRA 适配器。该适配器加入语言模型骨干与流匹配扩散解码器，并采用零初始化，使训练起点等同于原始零样本模型。实验比较不同 LoRA rank，并结合验证损失与母语者听评 MOS 进行评估。

Result: 最佳适配器 rank 为 64 时，高棉语 MOS 从 3.85 提升到 4.23，配对 Wilcoxon 检验 p<0.001，提升高度显著，同时仅训练 0.19% 到 3.03% 的参数。但验证损失与主观听评不一致：验证损失在 rank 128 最低，而 MOS 在 rank 64 达峰。对于韩语，适配器没有带来提升，高 rank 甚至降低质量。

Conclusion: 参数高效的 LoRA 联合适配能有效改善基座模型真正薄弱的低资源语言 TTS 质量，但对基座模型已掌握较好的语言收益有限且存在负迁移风险。模型选择不能仅依赖自动损失指标，应结合人工听评；适配容量也需谨慎控制。
...
TL;DR: CAT-Q 是一种面向大语言模型的高效后训练三值量化方法，用少量校准样本即可将不同规模预训练模型压缩为高性能三值模型，并显著降低训练/校准成本。


<details>
  <summary>Details</summary>
Motivation: 现有先进三值量化方法通常依赖数据密集、成本高昂的量化感知训练，以缓解三值化带来的严重性能下降；论文旨在提出一种更简单、低成本且可扩展到多架构、多规模 LLM 的后训练量化方案。

Method: 方法包含两个耦合组件：可学习调制（Learnable Modulation, LM）通过可学习因子组合调节预训练高精度权重分布与三值阈值，使其对三值化更鲁棒；软化三值化（Softened Ternarization, ST）引入可微过渡函数，引导三值化过程稳定收敛。

Result: 在 1.7B–8B 参数预训练 LLM 上，CAT-Q 仅用 512 个校准样本即可完成三值量化，性能优于使用 100B tokens 训练的 BitNet 1.58-bit v1/v2 系列，并带来约 100,000 倍训练 token 减少；同时可在 8 张 A100-80GB GPU 上于 8–60 小时内量化 14B–235B 更大模型。

Conclusion: CAT-Q 证明了低成本后训练三值量化在大规模 LLM 上的可行性与高性能潜力，相比依赖大规模训练的三值模型路线更具效率和实用扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有个性化助手评估多关注模型是否能记住用户在对话中明确表达的偏好，但真实个性化更需要模型从用户自然留下的多模态长期行为痕迹中推断其隐含兴趣与偏好。

Method: 作者构建了 SocialPersona 基准，基于 171 名普通非推广型社交媒体用户的长期时间线，包含文本、图像、时间戳以及 2,597 个经人工验证的偏好标签，覆盖七类兴趣领域，并区分稳定兴趣与近期兴趣。基准包含两个任务：从多模态上下文构建结构化用户画像，以及基于推断画像生成个性化对话回复。

Result: 实验显示，专有和开源权重的多模态大语言模型能够识别较宽泛的兴趣领域，但在细粒度兴趣和近期兴趣识别上表现明显下降；当模型需要把推断出的画像用于个性化对话时，性能进一步退化。同时，文本与图像提供了互补的偏好信号。

Conclusion: 鲁棒的跨模态、长时间跨度用户建模仍是个性化助手的重要挑战。SocialPersona 可作为衡量和推动模型从显性行为痕迹中推断并应用用户偏好的有效基准。
...
TL;DR: 本文提出一种利用大语言模型从谬误样例及其解释中归纳抽取抽象逻辑结构与语境语言线索的方法，用于提升逻辑谬误分类性能。


<details>
  <summary>Details</summary>
Motivation: 逻辑谬误常以细微、隐含的形式出现，单纯依赖表层文本或通用提示难以准确识别；因此需要探索将抽象推理结构与上下文语言特征结合是否能改善自动分类。

Method: 研究构建了一个框架，使用大语言模型从带有解释的谬误实例中数据驱动地抽取逻辑模式，并将这些模式融入不同大语言模型的零样本与一样本分类设置中进行评估。

Result: 实验显示，该方法相较零样本基线取得统计显著提升，并优于若干竞争方法；跨数据集实验也验证了其泛化能力。

Conclusion: 数据驱动的模式抽取能够有效生成逻辑表征，将抽象逻辑结构与语境线索结合是提升逻辑谬误分类的有效途径。
...
TL;DR: ConvMemory v3 在既有检索后增加“有效性上下文层”，用于识别相关但已被后续对话更新、纠正或替代的记忆，并在保持默认检索结果不变的同时提供结构化有效性元数据；在特定模式下可显著提升当前状态检索准确性。


<details>
  <summary>Details</summary>
Motivation: 传统会话记忆检索主要优化相关性，但相关记忆可能已经过时。后续轮次可能对先前记忆进行更新、纠正或覆盖，因此系统需要在不破坏原有检索流程的前提下，显式识别记忆的有效性与被替代证据。

Method: 提出 ConvMemory v3 的有效性上下文层，置于 v1/v2 检索路径之后。核心是目标条件化关系验证与双证据门控：针对具体目标命题，对 target-source 对进行关系判断，结合 MiniLM slot head 与 DeBERTa-v3 slot head 的乘积得分，并由保守的事件/操作证据进行门控。系统提供 context mode 与 query-conditioned demote mode；前者附加结构化有效性元数据但保持候选集和排序不变，后者在特定当前状态密集任务中按需降权过时记忆。

Result: 在合成多跳有效性基准上，门控机制达到 90.12% ± 1.73 的准确率。通过真实数据反馈循环挖掘失败模式、但仅用合成样本训练，验证器零目标侧标签迁移到 Memora role binding，达到 98.8% ± 0.9 的 group-all-correct。query-conditioned demote mode 将 current-active H@1 从 45.1% 提升到 95.7% ± 1.2，并对未被替代记忆保持 99.4% 召回率。

Conclusion: ConvMemory v3 通过有效性上下文层解决“相关但过时”的会话记忆问题，在默认情况下保护原检索排序，在显式启用时提升当前状态检索表现。论文验证了多跳图传播的机制价值，同时指出严格先决边的全自动构建仍是边界问题，因为其需要反事实世界知识。
...
TL;DR: 本文系统分析了多媒体事件抽取评测中的陷阱，指出评测不一致会显著影响模型性能结论，并呼吁建立更严格、可比较的评测标准。


<details>
  <summary>Details</summary>
Motivation: 多媒体事件抽取需要跨文本与图像等模态联合识别事件及论元。尽管已有研究报告了持续进展，但这些结果是否可靠、是否可比较高度依赖一致且严谨的评测流程；现有工作缺乏对评测问题的系统审视。

Method: 作者对多媒体事件抽取中的评测问题进行系统分析，归纳出三类主要问题：数据处理不一致、任务假设不一致、评测设置过于宽松。随后在严格评测框架下设计一系列受控实验，考察不同评测选择对性能结果的影响。

Result: 实验表明，即使是看似细微的评测选择，也会导致模型性能出现较大波动，并可能高估模型在跨模态真实事件 grounding/对齐与理解方面的能力。

Conclusion: 论文强调多媒体事件抽取领域需要统一、可比较且更严格的评测标准，以避免夸大模型能力，并推动该方向形成更可靠的研究结论。
...
TL;DR: 论文提出 OPID：从已完成的 on-policy 轨迹中提取层级技能监督，并将其转化为 token 级自蒸馏优势，以增强 outcome-based RL 对语言智能体的训练。


<details>
  <summary>Details</summary>
Motivation: 传统基于结果的强化学习奖励稀疏，只在轨迹层面提供反馈，难以判断中间决策应被强化还是抑制；现有技能条件自蒸馏虽然提供密集监督，但常依赖外部技能记忆或检索到的特权上下文，维护成本高，且可能与当前策略在多轮交互中诱导的状态分布不匹配。

Method: OPID 从 on-policy 完整轨迹中提取 hindsight 技能，并将其表示为层级结构：episode-level 技能描述全局流程或失败规避规则，step-level 技能描述关键时间步的局部决策知识。通过 critical-first routing，在识别到关键决策时优先使用 step-level 技能，否则回退到 episode-level 技能。所选技能被注入交互历史，使旧策略在原始上下文和技能增强上下文下对同一采样响应重新打分；两者的 log-probability shift 形成 token-level self-distillation advantage，并与 outcome advantage 结合用于策略优化。

Result: 在 ALFWorld、WebShop 和 Search-based QA 上，OPID 相比仅使用结果奖励的 RL 和已有技能蒸馏基线，整体提升了智能体性能、样本效率和鲁棒性。

Conclusion: OPID 在保持强化学习作为主要优化目标的同时，引入了来自 on-policy 分布、与当前策略匹配的密集 hindsight 监督，有效缓解稀疏奖励问题并改进语言智能体训练。
...
TL;DR: 本文介绍了面向 IWSLT 2026 指令遵循共享任务的 SpeechLLM 系统，在短语音与长语音受限设置下均进行了建模与评估；短轨道在 MCIF 上表现较强，长轨道中固定 30 秒分段最稳健。


<details>
  <summary>Details</summary>
Motivation: 语音大语言模型需要同时处理短语音和长语音指令遵循任务，但长语音生成容易不稳定并产生幻觉，因此需要探索适合受限条件的长语音处理策略与更可靠的评估指标。

Method: 作者构建了用于短形式和长形式语音指令遵循的 SpeechLLMs。短轨道评估 MCIF 表现；长轨道比较三种语音分段方法，并引入 HIFS 指标来衡量长形式生成不稳定性。

Result: 短轨道获得 SIFS 分数 2.0708；长轨道中固定 30 秒分段取得最佳且最稳健的表现，HIFS 分数为 2.0663。进一步分析表明，幻觉主要表现为生成输出中的重复插入，显著影响 ASR 和 SSUM，而短形式能力在扩展到长形式后基本保留。

Conclusion: 固定时长分段是长语音指令遵循中的有效稳健方案；长形式 SpeechLLM 的关键挑战在于控制重复插入式幻觉，同时保持短形式任务能力。
...
TL;DR: 本文提出面向IIoT边缘部署的级联多粒度LLM剪枝框架，通过层、注意力头、前馈通道的粗到细剪枝与阶段间低秩恢复，在MHA+GELU架构上实现高压缩比且保持较好精度，并揭示GQA+SwiGLU架构在不满足SIA时会发生严重性能崩溃。


<details>
  <summary>Details</summary>
Motivation: IIoT边缘设备资源受限，部署大语言模型需要极高压缩率；现有结构化剪枝依赖一次性重要性估计，在高压缩下易失效，且不同模型架构上的可迁移性和可靠性难以预测。

Method: 提出级联多粒度剪枝框架，按粗到细顺序依次移除层、注意力头和前馈网络通道；每个阶段之间引入轻量低秩恢复以重新估计结构组件重要性；用信息论分析论证剪枝顺序，并形式化结构独立性假设（SIA）作为可检查条件，用于判断逐组件剪枝准则在特定架构上的可靠性。

Result: 在轴承故障诊断任务、88M至6.25B参数模型上，该方法在MHA+GELU架构达到13.8倍压缩，精度83.82%，比最强基线高3.70个百分点；同时发现违反SIA的GQA+SwiGLU架构出现约74个百分点精度崩溃。工业平台部署中，压缩模型推理延迟最高降低67.2%，峰值内存最高降低62.5%。

Conclusion: 级联多粒度剪枝结合低秩恢复可显著提升MHA+GELU类LLM在IIoT边缘场景下的压缩可行性与部署效率；SIA为判断架构是否适合逐组件结构化剪枝提供了诊断标准，但GQA+SwiGLU等违反SIA的架构需要不同的剪枝或恢复策略。
...
TL;DR: 本文提出 InfoKV：一种面向长上下文推理的熵感知 KV cache 压缩框架，用预测不确定性等信息论信号补充注意力分数，从而更好保留对远期上下文有影响的 token。


<details>
  <summary>Details</summary>
Motivation: 随着 LLM 推理能力增强，prefilling 与 decoding 阶段的 KV cache 规模快速增长，带来显存和计算开销。现有压缩方法多依赖注意力权重评估 token 重要性，但注意力主要反映上下文相关性，可能忽略预测不确定性、token 信息量等互补信号，尤其不足以衡量 token 对较远未来上下文的影响。

Method: 作者从前瞻性角度重新定义 token 重要性，提出 Forward Influence 指标，用于衡量被压缩 token 对未来上下文的影响。基于分析发现，注意力高分 token 更影响近邻上下文，而高预测不确定性 token 对远期上下文影响更强。由此提出 InfoKV，将 token 级预测不确定性与层级表示演化结合，得到熵相关评分，并在推理阶段与注意力分数融合用于 KV cache 压缩。

Result: 在 Llama-3.1、Llama-3.2 和 DeepSeek-R1 上的长上下文推理基准实验表明，InfoKV 在长 prefilling 和 decoding 场景中均稳定优于现有基于注意力的 KV cache 压缩方法。

Conclusion: 仅依赖注意力权重不足以全面刻画 KV cache 中 token 的长期价值；引入预测不确定性和表示演化等信息论信号，可以更有效保留对未来推理有贡献的 token，并提升长上下文 KV cache 压缩性能。
...
TL;DR: 本文评估冻结语言模型表征在自然语言理解神经数据中的预测作用，发现其具有广泛但局部的预测效用，同时强调不能直接等同于共享神经机制或语言计算过程。


<details>
  <summary>Details</summary>
Motivation: 语言模型表征可为自然语言刺激提供高维结构化标注，可能帮助解释人在听读自然语言时的神经活动；但需要在严格控制低层特征、时间因素、容量和噪声因素后，区分“预测有效”与“机制等同”。

Method: 作者使用 Brain Treebank、MEG-MASC 和 Podcast ECoG 的锁定派生数据，测试八个冻结语言模型；采用分块编码模型，并设置匹配的时间、干扰变量和表征容量控制；通过留出集预测、低层基线增益、特征消融及多类神经/声学/信号控制评估模型特征的预测贡献。

Result: 语言模型特征在源级汇总中普遍产生正的留出预测和相对低层基线的增益；在 Brain Treebank 与 Podcast ECoG 中，432 个可评估行中有 67 个满足受控的“仅预测”标准；模型侧特征消融在多数可评估源行中改变预测分数，说明不同特征成分对预测有影响。

Conclusion: 语言模型派生特征可作为自然语音和文本理解中神经活动的有用预测因子，但其优势并非在所有参与者或脑源上均匀存在；结果支持预测用途，却不足以证明语言模型与大脑共享相同表征组织或语言处理计算。
...
TL;DR: 本文系统评估多种ASR模型在印度多语言精神科访谈中的表现，并提出SamaVaani以提升识别性能与群体公平性。


<details>
  <summary>Details</summary>
Motivation: 临床会话记录中ASR使用增加，但其在印度医疗场景中面对多语言、人口统计差异与真实精神科访谈数据时的可靠性和公平性尚不清楚。

Method: 作者在Kannada、Hindi和Indian English真实精神科访谈数据上审计八个先进ASR模型；随后对表现较好的开源模型Gemma3n与OmniLingual进行多种微调，并引入公平性感知微调；最终提出统一去偏技术SamaVaani。

Result: 不同模型和语言之间性能差异显著；部分系统在Indian English上表现较好，但在区域语言语音上失败。研究还发现与说话者角色和性别相关的系统性性能差距，公平性感知微调可缓解这些问题。

Conclusion: 在多语言临床ASR部署中，仅依赖通用模型存在可靠性与公平性风险；SamaVaani能够同时改善ASR识别性能并提升不同人口群体间的公平性。
...
TL;DR: 论文提出 GAVEL：一个面向图文对的“验证—解释—定位”联合任务、数据集与基准，用于评估并改进视觉语言模型对图文不一致/幻觉的识别、解释和视觉证据定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 容易生成与图像不匹配的文本，产生幻觉或不一致输出。仅判断图文是否对齐不足以支持可靠评估，还需要解释不一致原因，并在图像中定位相关证据。

Method: 作者定义 GAVEL 任务，要求模型同时完成图文一致性验证、错误/差异解释以及视觉证据定位；构建人工标注的数据集与 benchmark；并在训练集上训练一个监督式基线模型，以检验该任务标注是否能为这些能力提供可学习的监督信号。

Result: 实验表明，即使强大的闭源模型在 GAVEL 上也表现困难；而基于人工标注训练的监督基线在 grounding 与 explanation 等指标上取得稳定提升。

Conclusion: GAVEL 为评估 VLM 的图文一致性、可解释性和视觉 grounding 能力提供了更系统的框架；结果说明该任务具有挑战性，同时人工标注监督能有效提升模型的错误解释与定位能力。
...
TL;DR: 本文提出 RedVox，一个基于真实语音的多语言语音安全与公平性基准，用于评估语音模型在不同语言和自然语音输入下的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有语音模型越来越多用于多语言真实场景，但其在非英语环境、自然语音输入条件下的安全性与公平性研究不足；作者调查发现，主流语音模型发布中仅 8% 报告了任何多语言安全分析。

Method: 作者首先系统调查先进语音模型发布中的安全报告实践；随后构建 RedVox 基准，包含英语、法语、意大利语、西班牙语和德语五种语言，使用真实人声录制，覆盖不安全请求与不公平刻板化请求；最后评估八个先进模型，并对参与数据贡献者进行问卷调查以分析语音数据收集中的个人与隐私挑战。

Result: 实验发现，即使在非对抗性条件下，模型仍存在明显漏洞；这些漏洞在非英语语言中更严重，并且当请求以语音输入形式出现时会进一步放大。参与者调查还显示，真实语音数据收集带来独特的个人身份暴露与隐私风险。

Conclusion: 该研究表明，多语言语音模型的安全与公平性评估仍严重不足；自然语音输入会放大模型风险，因此未来需要更系统的多语言、真实语音安全基准与更谨慎的语音数据治理机制。
...
TL;DR: 本文研究心理健康类对话应用中，大语言模型对语义相近但语境框架不同的输入是否表现出不一致反应，并从行为输出与内部表征两方面分析这种框架敏感性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型正被用于心理健康支持等高敏感对话场景，用户期望系统行为稳定一致。然而，同一类关切若以不同语境方式呈现，可能诱发模型不同回应；既有研究多停留在行为层面，缺乏对对齐模型内部表征机制的理解。

Method: 作者构造受控的匹配提示集，覆盖多种上下文框架条件，并在多个指令微调模型家族上测试。研究包括：行为层面的响应倾向比较；逐层 probing 分析以检测与行为相关的信息在 Transformer 深度中的可解码性；held-out framing probe 评估跨框架泛化；以及 activation steering 实验检验框架相关表征方向对下游行为的调节作用。

Result: 实验显示，不同架构中，语境框架会系统性改变模型的解释性响应倾向。与行为相关的信息在 Transformer 各层中持续可解码，但解码强度具有架构差异。即使存在较强词汇基线，保留框架探针在各架构上仍稳定高于随机水平。激活 steering 进一步表明，框架相关表征方向能够部分调节模型后续行为输出。

Conclusion: 研究表明，语境框架变化不仅影响模型外部行为，也反映在内部表征中。对于部署在心理健康导向交互中的对话式 AI，评估其一致性、可信度和鲁棒性时，应将对上下文变化的稳健性作为重要指标。
...
TL;DR: 论文提出 ReaORE：一种推理引导的渐进式开放关系抽取框架，通过“关系过滤—关系预测”的粗到细推理流程提升对未见关系类型的泛化与区分能力。


<details>
  <summary>Details</summary>
Motivation: OpenRE 需要从非结构化文本中抽取训练中未见过的头尾实体关系。现有方法存在不足：聚类方法无法生成可读关系标签且泛化较弱；直接使用大语言模型生成关系标签又缺乏足够判别力，难以区分相似或易混淆关系。

Method: ReaORE 包含两个阶段：第一阶段为关系过滤，通过多方面推理理解关系与实例，得到初始候选关系集合，并结合基于嵌入的相似度进行补充与筛选，以尽量覆盖目标关系；第二阶段为关系预测，在候选集合内进行细粒度比较推理，从而在相近关系之间做出更准确判断。

Result: 在两个常用 OpenRE 数据集上的实验表明，ReaORE 相比现有基线方法取得更优性能。

Conclusion: 该工作说明，将大语言模型的推理能力与候选关系筛选、细粒度比较机制结合，有助于提升开放关系抽取中对未见关系的泛化能力和易混淆关系的判别能力。
...
TL;DR: 该论文摘要研究情绪向量在不同开源语言模型中的可泛化性，发现效价几何可复现，但不同模型在层深上的表征形成模式存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 既有研究在 Claude Sonnet 4.5 中发现了可编码情绪概念、因果影响行为且几何结构类似人类心理结构的情绪向量；本文希望检验这些发现是否能推广到其他开放权重模型。

Method: 作者在 Apertus-8B-Instruct-2509 与 Gemma-4-E4B-it 两个模型中，基于两个模型生成语料，在所有层提取情绪对比向量，并分析主成分与情绪维度（效价、唤醒度）的相关性。

Result: 两个模型均恢复出效价几何结构，PC1 与效价的最高相关分别为 r=0.76 和 r=0.83，接近 Claude 中报告的 r=0.81；但 Gemma 的效价编码在早期层较强、后期衰减，而 Apertus 则在早期缺失、中层开始出现。唤醒度编码对语料敏感，Gemma 生成故事带来更强 PC2--唤醒度相关（最高 r=0.45），Apertus 语料则较弱（r≤0.21）。

Conclusion: 情绪向量及其效价几何在不同语言模型中具有一定可复现性，但其跨层动态和唤醒度表征强烈依赖模型架构与语料来源；开源代码和数据有助于后续可复现实验。
...
TL;DR: MinGram 是一种简化版 Unigram 分词器，保留可编辑词表表示，同时显著降低训练复杂度，并在压缩率、形态对齐和下游语言模型表现上优于或接近现有方法。


<details>
  <summary>Details</summary>
Motivation: 标准 Unigram 分词器的词表表示灵活、易于编辑，但训练过程较重且复杂，依赖后缀数组、前向-后向算法和迭代剪枝。作者希望在保留 Unigram 优点的同时，构建一个训练更简单、成本更低、效果仍然强的分词方法。

Method: 提出 MinGram（Minimalist Unigram）：使用 BPE 派生的种子词表；采用基于最少 token 路径的 Hard EM；用单次扁平分数剪枝替代迭代剪枝；以 token 数最小化作为主要目标，Unigram 分数仅作为平局决策依据。

Result: 在六种语言上，MinGram 的压缩效果优于 BPE 和标准 Unigram；其面向压缩的变体可达到最强 token-count 压缩器的水平，同时保持更高的形态学对齐。在受控语言模型训练中，Unigram 系列分词器整体在 bits-per-byte 上稳定优于 BPE，MinGram 位列最佳方法之一。

Conclusion: MinGram 通过极简训练流程实现了 Unigram 类分词器的高效化，在压缩、形态一致性和下游建模质量之间取得良好平衡，是一种兼具实用性和性能的 tokenizer 训练方案。
...
TL;DR: 本文提出 Psy-CoT 与 RAPO，用心理学启发的分步推理和角色感知强化学习优化提升通用角色扮演智能体的角色一致性与泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于监督微调的角色扮演模型主要学习表层行为模仿，缺乏类似人类的内部思考过程，导致分布外泛化较差；同时，LLM 奖励模型在强化学习中容易被通用高分短语“奖励黑客”误导，无法区分泛化套话与真正符合角色设定的表达。

Method: 提出 Psy-CoT，将回复前推理分解为 Interaction Perception、Psychological Empathy、Logical Construction 三个角色相关步骤，使模型基于角色档案动态推理；进一步提出 RAPO，利用角色档案与生成 token 之间的互信息对策略梯度加权，在正优势下放大角色特异 token 的学习信号，在负优势下减弱其惩罚信号，从而缓解奖励黑客并增强角色保真度。

Result: 在 CoSER、CharacterBench 和 CharacterEval 上的实验表明，Psy-CoT 优于已有角色扮演 CoT 方法；RAPO 在多个模型规模上稳定超过 GRPO。

Conclusion: 通过结合心理学结构化推理与角色感知的策略优化，论文证明了角色扮演智能体不仅需要表层模仿，还需要显式的角色内在推理机制与更细粒度的强化学习信号，以获得更强的角色一致性和泛化能力。
...
TL;DR: 本文提出 NuclearQAv2，用于系统评估大语言模型在核工程领域知识、定量推理与概念理解方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在通用任务上表现强，但在高度技术化领域的可靠性仍难保证；核工程问题通常同时需要事实知识、数值计算和概念理解，因此需要专门、可扩展的评测基准。

Method: 构建约 1,240 个问答对，覆盖 boolean、numeric、verbal 三类问题；数据来源包括专家编写、已有数据集以及基于领域技术语料的 LLM 辅助生成，并使用结构化提示进行自动出题与回答评估。

Result: 在多个 LLM 上评测后发现，不同任务类型之间性能差异明显；模型在事实性问题上总体表现较好，但在定量推理和概念理解任务上仍存在显著困难。

Conclusion: NuclearQAv2 展示了多维度评测框架的重要性，可作为评估 LLM 在技术领域能力的可扩展基准。
...
TL;DR: 论文提出一种法官感知的门控多任务学习架构，用于法律结果预测，在英国就业法庭数据集上优于更大规模的生成式SFT基线，并显著减少可训练参数。


<details>
  <summary>Details</summary>
Motivation: 法律结果预测需要区分客观案件事实与裁判语境：实体性裁判主要依赖事实证据，而程序性或技术性处理可能更受裁判者裁量影响。现有将法官身份、结果分类等上下文信号通过提示或自回归通道注入大模型的方法，难以有效组合这些结构化信号。

Method: 作者提出Judge-Aware Gated Multi-Task Learning架构，引入细粒度结果分类体系监督编码器，通过结构正则化分离不同语义路径；使用门控融合机制动态调节模型对法官身份信息的依赖。模型基于LoRA适配的Gemma-4编码器，并与将法官身份和分类体系作为提示token或自回归目标注入的Gemma-4 26B-A4B监督微调基线比较。

Result: 在13,937份英国就业法庭裁决数据上，该方法达到新的最佳表现；相比生成式SFT基线，所需可训练参数少一个数量级。性能提升主要集中在最模糊和最稀有的结果类别上。实验还发现，当两个上下文信号被迫通过单一自回归通道组合时，其效果较弱。

Conclusion: 对于带身份条件的法律结果分类任务，条件信息的接口设计比模型规模更关键。可微、结构化的组合方式比基于提示的组合方式更准确、更参数高效，并且通过法官嵌入与校准曲线提供一定可解释性，可定位裁判语境影响预测的案件。
...
TL;DR: 本文提出“riddle riddle”范式检验大语言模型是否具备灵活推理策略选择能力，发现LLM与人类在真假谜题上的表现呈相反模式，提示LLM的高表现可能更多来自记忆检索或表层模式匹配。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在许多认知任务上表现良好，但尚不清楚其正确率源于训练数据中的模式匹配/记忆检索，还是源于能根据问题内容灵活选择推理策略。作者希望构造一种能区分这两者的新测试范式。

Method: 研究引入“riddle riddle”范式：将常见谜题形式改写为只需字面解释即可回答的文字问题。若模型依赖表层形式，会在这些题上错误使用创造性谜题推理；若能基于内容推理，则应切换到字面策略。作者在两个实验中测试9个先进LLM与100名人类参与者，并比较真实谜题与riddle riddles上的准确率及错误类型。

Result: LLM在真实谜题上准确率显著高于riddle riddles（84.9% vs. 50.7%），而人类呈相反趋势（50.5% vs. 80.5%）。错误分析显示，LLM在riddle riddles上的错误中90.8%来自不恰当地使用创造性推理；人类在真实谜题上的错误中57.6%来自过度使用字面推理。

Conclusion: LLM在真实谜题上的强表现不一定代表灵活推理，可能反映对熟悉谜题的记忆检索或表层模式匹配。若没有专门设计能诱发策略冲突的刺激材料，研究者容易将看似推理的LLM输出误判为真正的推理能力。
...
TL;DR: 本文研究大语言模型在预测任务中是否依赖可泛化的时间推理模式，发现并干预可解释的时间相关特征可降低“偷看未来”偏差。


<details>
  <summary>Details</summary>
Motivation: 成功预测需要从历史状态中识别能够泛化到未来观测的模式。作者希望理解 LLM 在预测时究竟是在进行基于历史的时间感知推理，还是依赖隐含的未来信息/时间泄漏导致的前视偏差。

Method: 作者将 LLM 应用于多种预测任务，并使用稀疏自编码器分析模型内部状态，识别与时间感知推理和前视偏差相关的特征。随后在不同领域任务中对这些特征进行干预，观察其对预测提示和一般推理能力的影响。

Result: 分析发现了与时间感知推理和候选前视偏差相关的特征。放大时间感知特征能显著降低预测提示中的前视偏差，同时保持一般推理性能；而操控候选前视偏差特征未产生明显效果。

Conclusion: 结果表明，可解释的时间特征不仅与 LLM 的预测行为相关，还能被因果性地用于引导模型更依赖历史语境、减少前视偏差，从而实现更稳健的历史 grounded reasoning。
...
TL;DR: 本文提出用“句法信念更新”解释花园径句的阅读加工困难，相比词汇惊讶度更能拟合人类阅读时数据。


<details>
  <summary>Details</summary>
Motivation: 传统词汇惊讶度通常能预测句子加工难度，但在花园径句中表现不佳；花园径句的困难主要来自早期句法解释被关键词推翻后的重新分析，因此需要一种更直接刻画句法预期变化的指标。

Method: 作者提出一个处理器模型：在逐词输入过程中主动预测句法树的概率分布，即“句法信念”；每接收一个新词后更新该分布。若出现花园径效应，关键词会导致原有句法信念发生大幅修正。该更新幅度用广义Rényi散度衡量。该指标依赖具体词项触发的句法信息，但不依赖词项本身出现概率。

Result: 在花园径句的人类阅读时数据上，Syntactic Belief Update 比词汇惊讶度提供了更好的拟合，能够更准确反映关键位置的加工困难。

Conclusion: 花园径句的理解困难可被视为句法信念的大幅更新，而不只是低概率词导致的惊讶。研究表明，心理语言学中除词汇惊讶度外，纯非词汇性的句法预测与更新机制也是解释句子加工难度的重要方向。
...
TL;DR: 本文提出将用户意图作为安全分类器中介信号，并构建 AIMS 数据集验证意图建模能提升安全分类鲁棒性与效率。


<details>
  <summary>Details</summary>
Motivation: 现有安全分类器通常直接从提示词预测安全标签，可能忽略用户真实意图，导致在困难或模糊安全提示上表现不稳定。作者希望证明显式建模意图可作为紧凑且高质量的监督信号。

Method: 构建包含 1,724 个困难安全提示的人工标注数据集 AIMS，每条样本包含意图描述与危害标签；在监督微调、偏好学习、推理蒸馏、强化学习等训练范式下评估意图感知训练，包括基于模型生成意图错误的 DPO、意图条件蒸馏以及以意图忠实度为奖励的 GRPO。

Result: 尽管 AIMS 规模较小，仍能训练出有竞争力的安全分类器；基于意图错误的 DPO 优于 SFT；意图条件蒸馏在多数教师-学生组合中优于仅推理蒸馏；直接奖励意图忠实度的 GRPO 在五个外部安全基准上取得最强平均性能，且意图感知模型形成推理延迟-F1 的 Pareto 前沿。

Conclusion: 忠实的用户意图建模是一种高效、紧凑且高质量的监督信号，有助于提升安全分类器在复杂提示上的泛化能力、鲁棒性和效率。
...
TL;DR: 本文提出一个将形式语义学的递归组合观纳入演化建模的框架，用于解释量词意义中的语义普遍性；结果表明，保守性可作为一种兼顾概念简洁性与交际准确性的高效系统级抽象自然涌现。


<details>
  <summary>Details</summary>
Motivation: 既有语义普遍性研究往往要么把词汇表建模为固定信号结构，要么采用缺少可解释词汇组成部分的整体式组合模型，未能充分体现形式语义学中“句子意义由词汇意义递归组合而来”的核心洞见。作者希望弥合形式语义学与演化建模之间的缺口，并解释量词语义普遍性。

Method: 作者构建了一个允许词汇意义与组合函数共同演化的模型，并设置两类选择压力：概念简洁性与交际准确性。随后将该框架应用于量词意义的演化，通过分析帕累托前沿来识别在简洁性和准确性之间达到高效权衡的语义系统，并考察句法结构对结果的影响。

Result: 分析显示，量词语义中最著名的普遍性——保守性——会作为一种高效的系统级抽象在模型中涌现。该结果对句法结构敏感，并能在一定程度上调和量词可学习性的经验研究与先前演化模型之间的张力。

Conclusion: 该研究表明，形式语义学关于句子意义组合生成的理论图景可以有效融入演化建模。所提出的框架不仅解释了量词保守性的效率来源，也为研究涉及语法范畴内全局压缩、句法论元语义专门化以及词汇—组合意义共同演化的语义普遍性提供了通用模板。
...
TL;DR: 论文提出 CARVE，将擦除机制限制在 key 轴并引入内容感知门控，在保持高效 WY-form 训练求解器有效的同时减少参数和显存，性能优于 GDN-2 及多种循环基线。


<details>
  <summary>Details</summary>
Motivation: 现有领先 delta-rule 循环架构 GDN-2 存在三类耦合缺陷：擦除门只看输入 token 而不看已有记忆，导致 memory-blind gating；value 轴擦除掩码参数开销大；并且 value 轴擦除在数学上阻碍 WY-form triangular chunk solver，从而削弱循环模型与 Transformer 竞争所需的训练效率。

Method: 提出 CARVE（Content-Aware Recurrent with Value Efficiency），核心原则是仅在 key 轴执行擦除。该设计被证明是保持 WY-form solver 有效的必要且充分条件。CARVE 复用已写入 GPU 内存的 recurrent output tensor 作为擦除门的免费内容信号，并将逐 value 的写门投影替换为每个 head 一个标量；初始化时与 GDN-2 bit-identical。

Result: 在 1.3B 参数、100B tokens 训练设置下，CARVE 的 WikiText perplexity 为 15.72，相比 GDN-2 降低 0.18，报告为 4.5-sigma 效应；在九个常识推理基准上领先所有循环基线；在所有 RULER retrieval probes 上达到 state of the art。同时仅带来 0.4% 吞吐开销，峰值显存降低 13%，参数量减少 19%。

Conclusion: 摘要表明 CARVE 通过 key-axis erase 同时解决内容盲门控、参数低效和训练求解器不兼容问题，在理论上给出多项证明，并在效率与效果上实现较优折中，显示其可能是更适合长上下文检索和高效循环训练的 delta-rule 架构。
...
TL;DR: 本文提出一个用于分析协作问题解决情境中对话的概念框架，重点面向人类-AI与多智能体协作。


<details>
  <summary>Details</summary>
Motivation: 随着智能系统逐渐具备自主推理与策略性合作能力，如何理解、优化和评估协作问题解决中的对话互动变得重要；现有分析方法在刻画认知、非认知与元认知调节机制方面存在不足。

Method: 作者构建了一个层级化的双层编码方案，将认知性与非认知性问题解决过程同元认知调节机制整合起来，并在九个跨领域数据集上验证其适用性与泛化能力。

Result: 该框架能够有效分析人类与智能体如何协调知识、技能和努力来解决复杂问题，并显示元认知调节是区分深层协作的重要指标。

Conclusion: 研究表明，一个整合认知、非认知和元认知维度的对话分析框架，有助于更系统地理解和评估人类-AI及多智能体协作中的协作质量。
...
TL;DR: 本文研究语言模型是否像传统知识库一样，对同一事实的不同查询保持一致回答；结果显示，模型知识往往以任务特定方式编码，查询形式会影响可访问的知识。


<details>
  <summary>Details</summary>
Motivation: 语言模型参数常被视为事实知识库，但真正的知识库应具备“同一事实、不同查询、一致返回”的单一事实源属性。论文希望检验语言模型是否满足这一属性，并理解其内部机制。

Method: 作者从行为层面分析训练过程中同一事实在不同任务上的共同涌现情况，并通过参数定位实验考察同一事实在不同任务中依赖的参数子集；同时分析思维链推理为何能提升表现。

Result: 实验发现，一个任务中学到的事实常常不会在其他任务中同步出现；参数定位显示，同一事实在不同任务下依赖不同参数子集；思维链推理的效果部分来自激活评测任务之外的任务特定参数。

Conclusion: 语言模型中的事实知识并非像统一知识库那样独立、稳定地存储，而是与提问方式和任务形式在参数空间中紧密耦合。这削弱了“语言模型参数即知识库”的类比，并对事实知识的可靠性和可控性提出挑战。
...
TL;DR: 论文提出一个诊断框架，将大语言模型处理历史文本的困难拆解为分词成本、预测不确定性、语义鲁棒性与上下文敏感性四个维度；发现历史文本会带来稳定的编码成本，但语义表示仍较稳健，简单时间背景提示可显著降低困惑。


<details>
  <summary>Details</summary>
Motivation: 现有研究常把历史语言处理难度视为单一障碍，混淆了正字法差异、语言距离和预训练暴露程度。数字图书馆日益依赖LLM，因此需要更细粒度地理解模型在历史文本上的失败模式与适用边界。

Method: 作者构建四维诊断框架，并在三个跨世纪数据集上评估：17世纪意大利文本语料、19世纪高曝光意大利经典文本作为控制组、18世纪俄语民用印刷书作为正字法压力测试。指标包括tokenization inflation、surprisal、embedding similarity以及加入最小时间上下文提示后的变化。

Result: 俄语与早期现代意大利语都产生约25–30%的分词膨胀，但预测难度不同：17世纪意大利语相较现代等价文本平均surprisal高2.4倍，学术散文高达3.2倍；俄语仅小幅增加。尽管生成预测不稳定，嵌入相似度在所有数据集上仍保持高于0.85。最小时间上下文提示可将历史文本surprisal降低约60%。

Conclusion: 历史文本对LLM施加稳定的编码成本，但这不必然导致语义表示失效。数字图书馆可较安全地将LLM用于语义检索等表示型任务；但生成类应用需结合时间背景提示或其他适配策略谨慎部署。
...
TL;DR: 论文研究一种无需训练的上下文感知翻译级联方法：在最终回译阶段保留原始问题、英文译文和推理轨迹，以减少多语推理中的信息损失。


<details>
  <summary>Details</summary>
Motivation: 传统翻译级联先将非英语问题译为英语、用英语推理、再译回原语言，虽具竞争力，但每一阶段都可能丢失后续所需信息，如文化背景、语体、歧义消解线索等，导致错误传播和回答质量下降。

Method: 作者提出 context-aware translation cascade：在最终翻译模块中额外输入原始语言问题、英文翻译问题以及英文推理轨迹。该方法不需要额外训练。实验覆盖九个多语基准、多种任务类型、三个骨干模型，以及285种高、中、低资源语言，并分析不同上下文成分的贡献。

Result: 该方法在开放式生成任务上表现出显著增益，且跨模型、跨资源语言均有效。消融结果表明，原始语言问题是最主要的有益上下文来源。

Conclusion: 研究表明，多语推理翻译级联的关键在于更合理地设计信息流，以缓解阶段间信息丢失和错误传播。一个简单可行的默认策略是在整个管线直到最终回答生成前始终保留原始用户问题。
...
TL;DR: 本文提出一种面向机制的间接语言表达（ILE）分类体系，用于更稳定地识别社交媒体中的编码语言；在人工标注数据与多种大语言模型评测中，该体系优于既有分类法和无分类基线。


<details>
  <summary>Details</summary>
Motivation: 社交媒体用户会使用算法规避语、委婉语和对抗性混淆等间接表达来隐藏敏感含义。既有分类多按意图或语境划分，难以泛化到新出现的编码语言，因此需要一种关注“意义如何被编码与恢复”的统一分类框架。

Method: 作者构建了一个综合的、机制导向的ILE分类体系，将其纳入LLM提示中，并与四种已有分类体系及无分类基线比较。实验使用2,000条人工标注的TikTok与Bluesky帖子，在三个LLM上进行文档级与片段级检测评估。

Result: 所提分类体系在三个LLM上的文档级和span级表现均最强，相比最佳基准方法，准确率提升4.7%，F1提升5.4%。

Conclusion: 机制导向且覆盖全面的ILE分类体系可作为检测新兴编码语言的稳定支架，并能为内容审核任务提供有价值的结构化输入。
...
TL;DR: 本文将大语言模型用于证券抵押品合格性审核的信息抽取流程，在长篇、半结构化、双语且含OCR噪声的招股说明书中实现较高精度的文档级判定。


<details>
  <summary>Details</summary>
Motivation: 人工核验证券作为抵押品的合格性耗时耗力；传统NER方法依赖逐类人工标注数据，且容易受到OCR噪声、语言变体、双语交织和固定span抽取约束的影响。

Method: 提出生成式信息抽取流水线，将任务分解为抽取、规范化和解释三个阶段；使用LLM处理嘈杂文本和德英混合内容，并引入基于价值/语义的LLM-as-a-judge评估方法，以替代或补充传统位置匹配指标。

Result: LLM系统在文档级合格性判断上达到较高精度，最高精度达91%，表现出保守的操作特征，即倾向于降低误接受风险。

Conclusion: LLM可作为证券抵押品合格性审核中传统NER的灵活替代方案，尤其适合处理长文档、双语文本和OCR噪声；其保守性有助于降低错误放行，但仍需关注召回率、可解释性和评估可靠性。
...
TL;DR: 论文提出 PEEU 方法，通过自主探索环境并利用 hindsight experience 合成高层、严格对齐的训练数据，提升小型开源多模态大模型在网页 GUI 任务中的规划与跨网站泛化能力。


<details>
  <summary>Details</summary>
Motivation: 小型开源 MLLM 具备低成本和隐私优势，但在复杂 GUI 任务规划、任务分解以及跨网站泛化方面明显弱于商业大模型；现有训练方式难以有效提升高层规划能力。

Method: 提出 Planning Experience Exploration and Utilization（PEEU）：先让智能体自主探索网页环境以发现可复用经验，再利用 hindsight experience 构造与任务目标严格对齐的高层训练数据。同时提出 Task Decomposition Hierarchical Analysis Framework（TDHAF），从低、中、高三个任务粒度分析组合泛化行为。

Result: 实验显示，PEEU 显著提升真实网页任务表现；训练后的 7B 模型达到 30.6% 准确率，并超过更大规模的 Qwen2.5-VL-32B。TDHAF 分析表明，仅掌握低层原子技能不足以带来高层规划能力，而高层任务训练更有利于 OOD 泛化。

Conclusion: 构造 hindsight 高层任务数据并利用环境探索经验，是提升小型 MLLM 在开放域网页任务中规划能力和 OOD 泛化能力的关键。
...
TL;DR: 论文提出一种迭代式数据生成流程，用“级联样本”构造与目标行为线性相关的特征梯度，从而更可靠地发现和控制语言模型中的特定行为特征。


<details>
  <summary>Details</summary>
Motivation: 激活引导方法依赖大量对比样本来识别与行为相关的模型特征；传统简单二元对比样本难以充分解耦复杂行为特征，限制了可解释性与可控性。

Method: 作者提出迭代数据生成管线，不只使用“有/无行为”的二元样本对，而是构造表现出不同程度目标特征、且与行为强度近似线性缩放的级联样本，用于隔离导致行为的线性特征子空间，并进行检测、打分与激活 steering。

Result: 在检测并抑制语言模型的 sycophancy 行为上，级联样本发现的特征形成线性可分子空间；相比基线方法，这些激活方向更清晰对应目标行为，并在检测、确定性评分和鲁棒 steering 上达到或优于 LLM-as-a-judge 与系统提示基线，同时计算需求更低。

Conclusion: 通过构造具有线性行为梯度的级联样本，可以更好地解耦和定位模型行为特征，提升激活 steering 的可解释性、效率和控制效果。
...
TL;DR: 本文主张在基准准确率饱和后，不应简单淘汰基准，而应继续用其评估智能体的构念效度、分布外泛化、效率、可靠性、模型与脚手架贡献、人机协作增益等维度。


<details>
  <summary>Details</summary>
Motivation: 现有评测范式过度依赖准确率；当准确率饱和时通常更换更难基准，导致忽视智能体真实能力与部署价值中的关键维度，如是否利用捷径、是否可泛化、是否高效可靠，以及人机协作是否带来实际提升。

Method: 作者以科学代码计算可复现性基准 CORE-Bench Hard 为案例，分析其在强智能体下暴露出的构念效度问题；提出改进版 CORE-Bench v1.1 和分布外任务套件 CORE-Bench OOD；并在准确率饱和背景下继续测量效率、可靠性、模型性能、脚手架性能。此外，开展小规模随机实验评估真实复现任务中的人机协作增益。

Result: 研究发现 CORE-Bench Hard 存在先前较弱智能体难以暴露的构念效度威胁；即便准确率饱和，CORE-Bench v1.1 仍能区分效率、可靠性、模型与脚手架表现。人机协作实验显示完成速度约提升 2 倍且具有统计显著性，该提升可能因部分纯人工任务达到时间上限而被低估。

Conclusion: 论文提出一种比准确率中心主义更严谨的智能体评估范式：饱和基准仍有价值，应通过多维指标持续评估智能体性能、泛化能力、系统组件贡献和人机协作效益。
...
TL;DR: 本文研究指令微调聊天模型中“顺从型人格”方向与“拒答”方向的相互作用，发现顺从人格会在后期层门控并抑制拒答表达。


<details>
  <summary>Details</summary>
Motivation: 以往工作分别研究了激活空间中的拒答方向和人格特征方向，但尚未解释二者是否相互影响，以及拒答机制是否能被视为单一、孤立的线性方向。

Method: 作者在 Qwen2.5-7B-Instruct 与 Llama-3.1-8B-Instruct 中提取顺从模型人格方向和拒答方向，并进行激活干预；比较人格 steering、重新注入拒答方向、投影去除人格方向以及随机方向对照的影响。

Result: 顺从人格 steering 显著降低拒答率，例如在 Llama 中拒答率从 97% 降至 2%；在后期层重新引入拒答方向可部分恢复拒答，早期层则不能；在后期层窗口投影去除人格方向可将拒答恢复至基线，而随机方向无此效果。

Conclusion: 拒答并非单一孤立方向的直接结果，而是在计算后、后期层表达阶段受到人格方向门控；因此研究模型拒答行为需要同时考虑人格表征与拒答表征的交互。
...
TL;DR: 论文提出 AlgoEvolve：一种由大语言模型驱动的进化式程序合成框架，用于自动生成、评估并迭代改进可执行交易策略，并通过外层元进化优化提示词以提升搜索效果。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 作为语义变异算子的研究多集中于静态代码基准，而算法交易环境具有噪声强、非平稳、收益/规则高度不连续等特点，对持续程序发现提出更高挑战。因此作者希望验证 LLM 语义进化是否能在复杂动态环境中有效生成策略。

Method: 框架以内层进化循环生成 Python 交易策略代码，并通过严格测试协议评估策略表现；随后基于评估结果迭代改进策略。论文还引入外层元进化循环，对指导内层程序合成的提示词进行进化搜索，以发现更优的搜索启发式，平衡探索与利用并减少零交易失败。

Result: 实验显示系统能够产生具有涌现式市场状态自适应能力的策略逻辑，包括自主切换交易规则。外层元进化得到的提示词启发式稳定优于初始人工设计指令，并能降低零交易失败率。

Conclusion: 研究表明，基于 LLM 的语义进化可作为复杂、非平稳环境中持续程序合成的可行方法，尤其适用于需要动态适应的算法交易策略发现。
...
TL;DR: 本文提出一个由大语言模型驱动的比较分析流程，用于大规模研究 AI Agent 互操作标准中的治理话语与权力结构，并在 ERC-8004 与 Google A2A 两个案例上验证。


<details>
  <summary>Details</summary>
Motivation: 随着 AI Agent 协议快速增多，影响其互操作标准形成的治理结构尚缺乏系统的实证研究。作者希望理解不同制度设计如何影响技术标准制定中的议题优先级、参与结构与社群关系。

Method: 研究构建了一个整合 LLM 自动标注、神经主题建模和多层网络分析的治理话语分析管线；基于 4,323 条治理参与记录，对 permissionless/on-chain 的 ERC-8004 与 corporate-led 的 Google A2A 进行比较分析。

Result: 研究发现，治理形式会影响讨论的实质性主题重点；但两个治理体系都表现出相近程度的参与不平等和社群碎片化。开放式治理环境中的话语对齐更密集，说明其可能促进更强的主题收敛。

Conclusion: LLM 辅助方法能够扩展技术治理的实证研究能力；研究结果对设计更公平、更具包容性的 Agentic AI 标准具有启示意义，并且作者公开了数据与代码。
...
TL;DR: 本文提出一个保留来源信息的知识图谱多智能体框架，用于整合抗抑郁药相关的监管不良事件记录与患者在线叙事，并区分权威证据与个人经验。


<details>
  <summary>Details</summary>
Motivation: 患者越来越依赖网络获取用药安全信息，但精神科药物信息分散在监管数据库与患者社区叙事中；前者权威但抽象，后者贴近体验但未验证。若不区分来源和证据等级，可能造成误解、恐惧、安慰剂反向效应及依从性下降。

Method: 研究整合466,525条Reddit帖子、60,782条WebMD评论和20年美国FDA不良事件报告系统数据，覆盖9种抗抑郁药；使用大语言模型实体识别流程抽取药物与病症实体，并与医生标注对照评估；基于ATC-N、ICD-10和MedDRA标准词表构建Neo4j知识图谱，记录每条信息的来源与可追溯性。

Result: 实体识别在药物和病症上分别达到最高F1值0.969和0.973。两个患者社区平台之间的一致性显著高于其与监管报告的一致性，Jaccard相似度最高达0.905。以舍曲林为例，许多不良事件在社区来源中比FDA对应记录早数百天出现。

Conclusion: 来源感知的知识图谱整合能在不混淆监管事实与患者经验的前提下提升精神科用药安全信息的可审计性，并可能为早期安全信号发现提供补充；其实际临床效用和患者获益仍需前瞻性验证。
...
TL;DR: 提出DD-Elo：将漂移扩散模型引入Elo评分，利用逐步走子信息更快捕捉棋手技能变化，同时与传统Elo保持理论一致性。


<details>
  <summary>Details</summary>
Motivation: 传统Elo等评级系统主要依赖比赛胜负结果，存在响应滞后，难以及时反映棋手真实水平波动；直接利用走子级信息又面临噪声大、状态空间巨大等挑战。

Method: 作者提出Drift-Diffusion-Enhanced Elo Rating System（DD-Elo），将技能表达建模为类似认知神经科学中漂移扩散模型的决策过程，把走子级质量信号整合进评级更新；同时给出数学推导，证明DD-Elo相对传统Elo的偏差有界。

Result: 实验表明，相比传统Elo，DD-Elo能够更快适应棋手技能变化，并在响应性方面表现更好。

Conclusion: DD-Elo为国际象棋评级生态提供了一种可解释、响应更快且向后兼容的技能评估方案。
...
TL;DR: 本文提出一种面向自主 AI 代理的计算治理模型：代理可自主规划与推理，但对高风险行动不拥有直接执行权，执行必须依赖独立权威来源对前置条件的证明，并由确定性策略判定且全程可审计。


<details>
  <summary>Details</summary>
Motivation: 随着自主 AI 代理可能执行临床开方、生产软件部署等具有重大后果且不可逆的动作，仅监控其内部推理过程不足以提供可靠治理。论文借鉴人类制度治理强大自主行为者的经验：在关键行动发生点要求独立证明，而非试图持续监督其思维过程。

Method: 作者形式化提出一种计算治理架构：AI 代理保留规划与推理自主性，但被剥离指定高风险动作的执行权限；每个执行前置条件需由独立权威来源证明，并与声明意图进行密码学绑定；确定性策略引擎根据这些证明决定是否允许执行；所有决策写入防篡改日志，以支持后续独立复验。论文还给出概念验证实现，并以软件部署和临床开方作为示例场景。

Result: 论文展示了该模型可将治理重点从不可观察或难以验证的代理内部推理，转移到可验证、可审计的行动执行边界；概念验证说明该机制能够在高风险动作前强制检查独立证明、策略一致性与日志可追溯性。

Conclusion: 该工作认为，高风险 AI 代理系统的安全治理应通过外部化、可证明、可审计的执行控制来实现，而不是依赖对模型推理过程的监控。其贡献在于提出了一个制度启发的形式化治理模型，为自主 AI 在关键领域的受控部署提供了可操作框架。
...
TL;DR: 本文提出 COrigami：一个从自然语言生成折纸折痕图案的端到端 AI 辅助设计流程，用于在严格平折几何约束与主观美学目标之间实现协同优化。


<details>
  <summary>Details</summary>
Motivation: 生成式 AI 在可验证问题上表现突出，但在物理艺术设计中同时满足严格几何约束和审美需求仍很困难。折纸设计具有明确的数学约束，适合作为研究 AI 约束化艺术生成与人机共创的场景。

Method: 系统从自然语言输入出发，依次生成语义 stick figure，计算 base packing，求解满足平折性的 crease pattern，对平折后的图案进行塑形，并通过由自主美学评价循环驱动的强化学习进一步优化模型。

Result: COrigami 能生成可作为艺术家进一步扩展和塑形的结构性初始折纸方案，在多目标物理约束下提供有效的协作式设计支持。

Conclusion: 该工作表明，将算法优化与自主审美评价结合，可以帮助 AI 在数学可验证的物理约束环境中实现可靠的创造性辅助，推动面向物理艺术的约束化生成式 AI。
...
TL;DR: 论文指出，随着编码智能体生成能力增强，核心难题正从“生成解法”转向“可靠验证解法”。作者提出用可扩展性、忠实性、鲁棒性三维刻画验证信号，并通过多类奖励/验证器设计证明验证机制必须随模型能力共同演化。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为验证答案比生成答案更容易，但在当代基础模型与工程化智能体中，生成复杂候选方案已相对容易，而验证是否真正满足人类意图更困难。原因在于人类意图天然不完全明确，且训练优化会放大代理奖励与真实意图之间的差距，导致 reward hacking 或信号饱和。

Method: 作者提出从 scalability、faithfulness、robustness 三个维度评估验证信号质量，并研究四类奖励构造：通用编码任务中的测试验证器、前端任务中的 rubric 验证器、真实智能体任务中的用户验证器，以及长周期任务中的自动化智能体验证器。论文在不同任务类型和不同策略能力水平下进行分析与实验，考察奖励设计挑战及奖励信号利用方式。

Result: 实验表明，有针对性的验证设计能够抑制 reward hacking，提高任务完成质量，并在多个内部和公开基准上取得显著提升。不同类型验证器在不同任务场景中展现出各自优势与局限。

Conclusion: 固定奖励函数无法在策略能力持续增长时长期保持有效；验证机制必须与生成模型共同演化。论文的核心贡献在于将验证问题提升为编码智能体发展的中心挑战，并提出分析框架与实践经验。
...
TL;DR: 本文提出并评估面向能源市场分析任务的工具增强型LLM智能体基准，覆盖实时数据检索、法规/市场知识解释与高级量化决策建模。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准已覆盖金融、代码、法律、药物发现等领域，但能源领域评测多停留在静态知识回忆，无法反映能源行业对实时数据、专业法规与市场知识、多步骤量化推理和现实约束下决策的需求。

Method: 作者构建包含243个专家设计问题的评测环境，分为市场数据检索与分析、知识检索与解释、高级量化建模与决策分析三类。智能体配备可配置领域工具，包括美国主要ISO实时电力市场API、监管文件检索、公用事业费率数据库、资产优化模型以及面向能源市场文档的RAG系统。采用多维评估协议，从方法正确性、答案准确性、属性对齐和来源有效性等方面评分，并按问题类别路由到相应评分标准。

Result: 研究对闭源和开源LLM进行比较评估，分析模型能力与领域工具在高风险专业能源场景中的交互影响；同时公开关键产物以支持复现与后续研究。摘要未给出具体数值结果或最佳模型表现。

Conclusion: 该工作填补了能源领域智能体评测从静态知识问答到真实市场分析任务的空白，强调工具增强、实时数据接入、专业检索和量化建模对能源行业LLM应用评估的重要性。
...
TL;DR: 本文分析多模态大语言模型评测现状，指出现有基准多聚焦孤立任务，难以衡量跨模态信息整合能力。


<details>
  <summary>Details</summary>
Motivation: 随着MLLM能够处理文本、图像、音频、视频等多种输入并生成文本响应，其能力快速提升，但评测体系滞后，无法充分反映模型是否真正实现多模态融合与理解。

Method: 作者审视当前MLLM评估方式，并梳理现有评测基准的分类体系，从中识别关键缺口，如时空一致性、物理世界理解、多模态一致性和选择性注意等。

Result: 研究发现现有评测基准主要局限于单一或孤立任务，对模型跨模态整合、动态场景理解、现实物理规律把握以及关键信息筛选能力的评估不足。

Conclusion: 完善针对上述缺口的评测框架对于衡量多模态智能的真实进展、揭示模型能力边界以及推动MLLM可靠发展具有重要意义。
...
TL;DR: 本文提出 OpenFinGym，一个面向量化金融智能体开发与评测的统一 gym 环境，用单一执行与验证接口覆盖预测、市场生成、实时交易、欺诈检测等多阶段金融任务。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型智能体在量化金融中的评测多集中于孤立任务，忽视金融工作流的多阶段、强依赖特征，容易高估智能体能力，并难以暴露其在泛化、真实市场交互和金融意义决策方面的不足。

Method: 构建统一环境 OpenFinGym：整合多类量化金融任务；提供从量化金融论文自动生成可执行任务包的流水线；采用容器化运行时与宿主侧验证服务以支持规模化 rollout 并防止运行时训练-测试泄漏；加入低延迟数据流纸面交易引擎、长周期/事件市场预测的延迟解析机制，以及 SFT 和 RL 后训练集成。

Result: 摘要表明该平台能够在统一框架下支持更贴近真实金融流程的智能体开发、执行、验证和训练，但未给出具体实验指标、对比结果或性能提升数值。

Conclusion: OpenFinGym 旨在弥合现有金融智能体评测碎片化与真实金融工作流复杂性之间的差距，为量化金融智能体提供更系统、可扩展且防泄漏的评测与训练基础设施。
...
TL;DR: 本文提出并实证检验“组合式行为泄漏”（CBL）：在共享上下文窗口的提示模块间，即使不存在显式变量或执行依赖，一个模块的修改也会隐性影响其他模块行为。


<details>
  <summary>Details</summary>
Motivation: 提示组合式智能体系统在实践中出现一种反复故障：编辑某个提示模块会无声改变其他模块输出。现有评估通常关注注入攻击、认知退化、隐私泄漏等问题，缺少对共享上下文中模块间非隔离干扰的形式化度量。

Method: 作者将CBL定义为共享上下文窗口内模块间的行为干扰，并基于部署中的职位评估智能体进行144次试验。提出三通道扰动协议，对非焦点模块分别在体量、内容、形式三个维度进行扰动，并用配对效应量、Cohen's d和bootstrap置信区间评估焦点行为变化。

Result: 实验显示只有内容扰动通道产生可检测的配对效应，Cohen's d = 0.63，bootstrap 95%置信区间排除零；体量和形式扰动未表现出显著效应。虽然没有最终推荐结果翻转，但该效应处于标准QA难以发现的亚阈值区间，可能在大规模决策中累积。

Conclusion: CBL是不同于对抗注入、认知退化、多智能体故障传播和隐私泄漏的独立失败轴。论文贡献了操作性定义、可复用测试协议、可证伪预测集合和系统类别刻画，主张提示组合式智能体评估必须纳入跨模块干扰测量。
...
TL;DR: 本文认为“加速回报”可解释算力、AI、生物技术等量化能力的指数式提升，但无法自动解决科学发现中的核心难题；QES被提出用于保存、组织并增强人类在科学发现中的定性推理智慧。


<details>
  <summary>Details</summary>
Motivation: 论文旨在澄清库兹韦尔“加速回报”叙事的解释边界：即技术执行能力与基础设施能力的加速，并不等同于科学发现能力的自动突破。作者希望指出当前AI在灵活推理方面仍与人类存在显著差距，并强调保存人类科学理解方式的重要性。

Method: 作者采用概念分析与简单数学解释相结合的方法：先将加速回报理论形式化为多技术领域相互强化的近似指数增长过程，再区分“执行性/基础设施能力”与“定性科学推理能力”；同时引用ARC-AGI-3结果和Demis Hassabis关于人类意义感的观点，论证QES的必要性。

Result: 分析表明，加速回报可以解释量化能力的快速提升，但不能充分解释科学发现所需的框架反思、结构性不足识别和概念转换能力。ARC-AGI-3中人类接近满分而前沿AI低于1%的表现，被用作当前AI缺乏人类式灵活推理能力的证据。

Conclusion: 论文结论是，QES的价值不取决于AGI何时到来，而在于科学发现过程本身是一种值得保存和传播的人类智慧。库兹韦尔理论解释“能力为何加速”，QES则试图解决“科学发现如何发生”的更深层问题。
...
TL;DR: 论文提出 Narration-of-Thought（NoT）提示框架，将推理组织为主角、利益相关者、两步后果、不确定性与最终承诺五部分，以降低道德困境推理中的利益相关者遗漏与不确定性压制问题。


<details>
  <summary>Details</summary>
Motivation: 标准 chain-of-thought 在道德困境中常出现两类失败：只关注至多一个利益方的 stakeholder collapse，以及在行动承诺前不显式表达未知、条件或保留意见的 uncertainty suppression。这会削弱推理可审计性与可靠部署价值。

Method: 作者设计无需训练、参数或微调的系统提示 NoT，并在 100 个 DailyDilemmas 场景、来自三家厂商的四种生成器上评估。对比标准 CoT 与等预算 verbose-CoT 控制；使用 Cliff's delta 衡量效果；通过 section ablation 分析各子指令贡献；进一步用 textual-gradient descent 优化脚手架，并扩展到五轮多利益相关者辩论协议。

Result: NoT 将 stakeholder collapse 从最高 31% 降至低于 1%，将 uncertainty suppression 从最高 72% 降至各模型 1–24%。在三/四个生成器上，相比 verbose-CoT 仍保持 stakeholder count 的 Cliff's delta +0.79 到 +0.90、uncertainty score 的 +0.65 到 +0.93。消融显示对应子指令驱动对应改进。跨家族训练 judge 优于同家族 judge。多轮辩论中，校准集从 6% 僵局转为 95% 完全共识，复现实验达到 100% 合并收敛。

Conclusion: NoT 作为轻量级提示脚手架，可显著提升道德困境推理中对利益相关者、后果链和不确定性的显式表达，并为可审计、可靠的智能体部署提供更清晰的推理记录。
...
TL;DR: 提出一种几何感知的蒙特卡洛树搜索框架，用于求解网格点配置中的组合几何极值问题，并在多个任务上刷新已知计算结果。


<details>
  <summary>Details</summary>
Motivation: 组合几何中的网格点配置问题具有强全局几何约束，传统精确求解器面临组合爆炸；常规强化学习和 Transformer 方法又受稀疏奖励、有效性断崖以及二次 token 消耗限制，难以高效搜索可行且优质的构型。

Method: 论文提出 Geometry-Aware MCTS：通过增量维护可行动作空间严格执行几何约束；针对共线点集合类约束，将检查复杂度从 O(n^3) 降至 O(n^2)；进一步利用几何对称性进行节点扩展时的规范化剪枝，并采用对称批量转移以加速发现高质量配置。

Result: 在六个组合几何问题中的五个上取得新的最佳已知计算结果。对于 Max-N3IL，在 82≤n≤119 的网格上找到规模约为 1.8n 的配置；对于 Smallest Complete Set，找到规模约为 0.95n 的配置，并在测试网格内给出新的上界。

Conclusion: 该工作表明，将几何约束显式嵌入搜索过程并结合对称性利用的 MCTS 框架，能够有效突破组合几何配置搜索中的可行性和效率瓶颈，具有较强的适应性和发现新构型的能力。
...
TL;DR: 本文提出一种面向电动公交车队的智能体聚合器框架，将优化调度模型与监督型智能体结合，用于在扰动和价格变化下协调充电、运营可靠性与V2G灵活性。


<details>
  <summary>Details</summary>
Motivation: 电动公交运营需要同时处理线路服务可靠性、电池荷电状态、充电桩可用性、电价波动、线路能耗不确定性和车网互动机会，决策环境复杂且动态变化，因此需要一种能连接异构数据源、自动识别扰动并触发调度调整的协调框架。

Method: 论文构建了一个智能体聚合器框架：底层为基于优化的电动公交调度模型，约束线路、充电器、电池和V2G交换等物理可行性；上层为监督型智能体，负责扰动检测、电价/费率适配、调度评估，并在必要时触发实时再优化。同时，案例研究比较日前与实时运行，以及利润导向和运营导向两种协调模式。

Result: 现实车场案例表明，该框架能够维持可行调度、选择性触发再优化，并提升充电和V2G灵活性的利用效果。在服务延误、能耗偏差、电价冲击及复合扰动下，智能体聚合有助于实现更自适应的车队—电网协调。

Conclusion: 智能体聚合器可用于管理电动公交V2G运行，但其配置方式会影响价值分配。若采用利润导向定价，聚合器可能从公共交通运营方提取更多价值。因此，公共车队部署此类系统时需要透明的协调模式、可审计的费率设定和明确的价值共享规则。
...
TL;DR: 本文研究概率预测型“oracle”AI在其预测被知晓并影响行动后产生的自指/表演性问题，提出用自洽且无偏的 credal set（概率集合）替代单点概率，并用 Knaster–Tarski 不动点定理选出规范的非平凡答案。


<details>
  <summary>Details</summary>
Motivation: 传统概率 oracle 给出未来事件概率时，答案本身可能改变事件发生概率；反事实提问虽避免影响，但答案一旦被学习，其反事实前提失效，导致预测失去现实相关性。因此需要一种既承认预测会被使用、又能保持一致性的报告机制。

Method: 作者在闭 credal sets 构成的完备格上定义一个保序算子，并利用 Knaster–Tarski 固定点定理取其最小不动点作为规范答案；还提出变体：报告包含所有自洽点估计的最小不动点。理论从二元事件推广到任意随机变量，用条件分布替代条件概率。

Result: 证明了所构造答案的存在性、自洽性与非空性；在非表演性问题中退化为经典单点概率答案；在二元事件且满足自然的 hull-factoring 假设时，规范答案表现为一个区间。

Conclusion: 该工作给出了一种纯格论框架，用 credal set 处理预测影响现实导致的自指问题，相比反事实 oracle 更贴近预测被实际使用后的情形；但任意随机变量情形下区间刻画是否仍成立等问题仍待研究。
...
TL;DR: 本文评估社会科学文本分类中性能指标置信区间的估计方法，指出常用 Wald 区间和基本百分位 bootstrap 在小样本、高性能、低发生率及嵌套文本场景下覆盖率不足，并推荐更稳健的替代方法。


<details>
  <summary>Details</summary>
Motivation: 研究者常用监督分类器或大语言模型从文本中测量社会科学构念，并以召回率、精确率、F1 等指标证明测量有效性。但这些指标是受抽样波动影响的点估计，置信区间报告不充分，且常用方法在小标注集或模型性能较高时并不适用。

Method: 论文通过模拟实验比较多种性能指标置信区间方法，覆盖典型社会科学文本分类条件：小到中等样本量、低频构念、文本嵌套于个体。比较的方法包括 Wald 区间、基本百分位 bootstrap、Agresti-Coull、Wilson、Clopper-Pearson，以及一种用于 F1 等指标的伪计数正则化 bootstrap；同时考察嵌套数据下有效样本量、自由度修正、层级 bootstrap 与聚类 bootstrap 的表现。

Result: 模拟结果显示，Wald 区间和基本百分位 bootstrap 准确性最差，覆盖率有时显著低于名义 95%。Agresti-Coull、Wilson、Clopper-Pearson 和伪计数正则化 bootstrap 能提高区间覆盖准确性。对于文本嵌套于个体的数据，需要同时调整有效样本量和自由度。bootstrap 方法中，层级 bootstrap 在每个个体文本数量中等时优于聚类 bootstrap，但在每个个体文本很少时会过于保守。

Conclusion: 论文为文本分类性能指标的置信区间估计提供方法建议，强调应避免默认但不稳健的方法，采用更适合小样本、低频类别、高性能模型及嵌套数据结构的区间估计方法，以提升机器学习测量应用的透明度，并促使研究者在研究设计阶段重视验证样本量。
...
TL;DR: 本文认为监督式深度学习难以达到符号级三段论推理的严格性，即使扩大数据与训练时间也不充分。


<details>
  <summary>Details</summary>
Motivation: 球面神经网络已在无训练数据条件下实现符号级三段论推理，引出一个核心问题：数据驱动的机器学习是否能通过规模化训练达到同等逻辑推理水平。

Method: 论文从理论上分析监督深度学习的两项方法论限制：训练数据无法区分全部24类有效三段论；端到端从前提到结论的映射会在模式识别组件与逻辑推理组件之间引入矛盾训练目标。同时，实验评估Euler Net，并测试GPT-5-nano与GPT-5在四种表面形式下判断三段论陈述可满足性的表现。

Result: 实验显示Euler Net不能实现严格三段论推理；ChatGPT模型的推理性能受表面形式影响。GPT-5虽可能达到100%准确率，但仍会给出错误解释。

Conclusion: 作者认为，由于训练过程通常在达到100%准确率后停止，而准确率并不保证解释与推理过程严格正确，因此监督式机器学习系统难以获得符号逻辑推理的严密性。
...
TL;DR: 本文提出 MKG-RAG-Bench，一个面向多模态知识图谱 RAG 的跨领域基准，重点评估检索环节，并证明检索质量显著影响下游生成效果。


<details>
  <summary>Details</summary>
Motivation: 现有 RAG 基准大多忽视多模态知识图谱场景中的检索难题；而在实际 MKG-RAG 中，多模态知识具有异构性、跨模态对齐困难，传统面向非结构化语料的检索器难以有效处理，检索成为系统性能瓶颈。

Method: 作者构建了 MKG-RAG-Bench：基于两个覆盖通用与医学领域的多模态知识图谱，生成并对齐问答数据集；采用 LLM 驱动的整理流程过滤低效知识、生成具有结构依据和精确监督的查询，并系统覆盖不同模态配置；随后在多类代表性检索器与模态设置下进行实验。

Result: 实验表明，多模态知识图谱检索仍然具有挑战性；检索质量对端到端 MKG-RAG 的生成结果具有强决定作用，检索性能的不足会直接限制下游生成表现。

Conclusion: MKG-RAG-Bench 将检索作为独立且核心的评估对象，为诊断现有 MKG-RAG 系统的局限、比较不同检索方法，并推动多模态知识图谱 RAG 的发展提供了系统化基准。
...
TL;DR: 本文提出并验证了一个用于计算认知科学的自动化理论发现系统 auto-psych，可自主生成模型、设计并发布众包实验、收集与分析人类行为数据。


<details>
  <summary>Details</summary>
Motivation: AI 科学自动化可辅助假设生成、实验设计和数据分析，但数据收集仍是瓶颈。计算认知科学中的理论常以代码表示，且可通过众包平台规模化收集人类数据，因此适合探索自动化实验与理论发现。

Method: 以“人们如何判断硬币序列是否随机”为测试场景，构建嵌套式智能体发现循环：内循环负责提出、拟合和批判概率认知模型；外循环负责设计实验、在线发布众包问卷并分析数据。系统同时在合成数据和真实人类实验数据上评估。

Result: 在合成数据中，系统能快速且可靠地恢复真实理论，且嵌套结构对模型性能至关重要。在三组独立的人类实验中，系统发现的理论比来自既有科学文献的理论更好地拟合数据。

Conclusion: 该研究表明，在计算认知科学中实现自动化数据收集与理论发现是可行的，智能体驱动的嵌套实验—建模流程可有效生成解释人类行为的认知模型。
...
TL;DR: 本文提出“临床AI技能”与 Clinical Harness 运行时治理架构，将医疗AI从孤立模型转向可注册、可编排、可防护、可监测的持续性临床能力。


<details>
  <summary>Details</summary>
Motivation: 现有医疗AI多以单个、孤立模型为中心，难以满足临床照护对长期连续性、责任归属、运行时安全与可治理能力的需求。临床场景需要AI能力在患者生命周期中持续发挥作用，并接受监控与治理。

Method: 作者提出两个核心概念：临床AI技能，即面向具体临床任务的AI使能能力；Clinical Harness，即用于运行时治理的架构，负责技能注册、任务编排、安全防护与性能监测。论文以骨质疏松管理为示例，展示知识驱动、数据驱动和物理增强型技能如何协同支持生命周期照护。

Result: 摘要表明，该框架能够把多类AI技能纳入统一治理环境，使其在临床流程中被组织、调用、约束和监测，从而支持骨质疏松等慢病的连续照护。具体定量实验结果或性能指标在摘要中未提供。

Conclusion: 论文认为，医疗AI的发展应从单点模型部署转向受治理的临床能力体系。Clinical Harness 为临床AI的可问责、可持续和可监管应用提供了架构路径，尤其适用于需要长期管理的疾病场景。
...
TL;DR: 论文指出，大型推理模型与人类在跨题目层面的“难题耗时更多”现象相似，但在同一题目内的成功/失败资源分配模式相反：模型答错时生成更多推理 token，人类答错时用时更少。


<details>
  <summary>Details</summary>
Motivation: 既有研究常用跨题目反应时间或推理长度与难度的相关性来比较人类与大型推理模型，容易得出二者推理行为相似的结论。本文动机是区分“是否感知到题目难度”和“如何在成功或失败尝试中分配思考资源”，以检验这种表面相似是否掩盖了机制差异。

Method: 作者提出两个层面的分析框架：registration 指跨题目层面反应时间/推理长度是否随难度增加；allocation 指在固定题目身份后，同一智能体在答对与答错时投入资源的差异。研究使用公开的人类-LRM匹配语料，比较人类、五个 thinking LRMs 以及一个 non-thinking baseline，并采用题目固定效应、跨数据集复现和效应量分析。

Result: 人类与五个 thinking LRMs 都表现出跨题目难度对齐，即难题上投入更多资源；但在题目内分配上显著分化。所有 LRM 都在答错时比答对时生成更多 token，在 H-ARC 上 Cohen's d 为 1.47–3.13；人类则方向相反，答错时用时更少。该结果在固定题目身份和多个数据集上稳定存在，而非 thinking baseline 中不存在。

Conclusion: 论文认为，人类模式反映“投入—放弃”策略：人会在自认为可解的题目上停留更久，对不可解题目较快放弃；LRM 模式则反映由不确定性驱动的链长增长：模型越不确定越生成更长推理，而这也更可能导致失败。因此，trace length 能捕捉难度信号，却不能揭示控制策略；人类与 LRM 可能共享难度感知，却采用相反的停止策略。
...
TL;DR: 本文介绍开源 EEG 代理 NeuraDock Agent 的可复现实操教程，覆盖从安装、预处理、质量控制、Alpha 动态分析到实时 API、在线仪表盘与 LLM 解释层的完整流程。


<details>
  <summary>Details</summary>
Motivation: 现有 EEG 工具多偏向离线分析，而构建实时、质量门控的视觉认知负荷分析管线通常需要手动整合采集、质量控制、Alpha 特征提取和 Web API。本文旨在弥合离线 EEG 分析与在线认知负荷原型之间的工程落差。

Method: 论文采用教程式、逐步复现的方式，构建质量门控工作流：先进行 EEG 预处理与质量控制，通过 QC 后才计算 Alpha 动态和工作负荷指标；并演示小型公开数据集分析、组内 Rest/Task 对比、在线仪表盘、实时 API 调用以及 LLM 质量风险解释。

Result: 在小型数据集验证中，系统处理了 18 条记录，生成 10 个被试内对比；其中 7/10 个对比观察到任务相关的后部 Alpha 抑制；同时给出了被试内重复性的初步证据，并评估了本地在线 API 延迟。

Conclusion: 该教程为研究者、开发者和应用团队提供了一条透明、可复现的路径，将 EEG 文件处理扩展到实时视觉认知负荷原型；其核心贡献是将质量门控、Alpha 分析和在线部署整合为端到端流程。
...
TL;DR: 本文提出 NeuraDock Agent：一种将确定性本地 EEG 数值引擎与硬件感知语言层分离的开源架构，用于降低低通道 EEG 分析中大模型产生不受支持解释的风险。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型不了解特定传感器支持的测量、软件实际实现的算法，以及计算结果能够支撑的结论。低通道 EEG 因空间覆盖稀疏、信号质量波动，更容易诱发看似合理但缺乏依据的解释，因此需要一种能约束模型边界、保护原始数据并提升结果可复现性的系统。

Method: 系统由本地确定性 EEG 引擎和硬件感知 LLM 层组成。本地引擎负责解析记录、质量控制、执行经审查的频谱工作流，并生成机器可读结果；LLM 仅接收精简的白名单摘要和版本化上下文包。上下文包描述七通道硬件、已审查工作流、结果字段、实现边界、科学限制和参考案例，原始 EEG 与逐采样密集数组保留在本地。评估包括数值重复性、请求捕获与故障注入、以及边界感知问答基准。

Result: 12 份记录在 10 次数值重复中产生完全一致的结构化结果；完整 Rest/Task 运行在 3 次重复中得到相同的结果、报告和图像哈希。请求捕获与故障注入实验验证了数据边界，并显示在 HTTP、格式异常输出和连接失败时本地 artifacts 能被保留。边界感知基准在 36 个普通与对抗问题、4 种上下文消融和 2 个 LLM 下产生 288 个输出，用于检验系统对可接受、需限定或应拒绝问题的校准能力。

Conclusion: 硬件与实现感知的上下文约束可作为一种实用机制，帮助 EEG 智能体校准回答边界，减少越权解释，并保护本地数据与分析产物。但该工作不证明临床有效性，也未验证绝对认知负荷指数。
...
TL;DR: 本文提出一个解释 AI 系统为“智能体”的理论框架，将激进解释的哲学传统与机制可解释性工具结合，用于判定模型内部信念、欲望与意义归属何时成功。


<details>
  <summary>Details</summary>
Motivation: 随着 AI 安全需求上升，研究者希望通过理解系统目标或检测欺骗来提升可信度；但当前机制可解释性虽尝试从模型内部读出信念和欲望，却缺乏明确标准来判断这些归属是否真正成立。

Method: 论文构建了一个规范性框架，分别为表征主义和解释主义路径提出判据，并将这些判据关联到现有机制可解释性方法可执行的测试；同时强调信念、欲望和命题结构必须被联合约束与评估。

Result: 核心结论是，信念、欲望及其预设的命题结构不能被孤立归因；若方法固定其中一项再测量其他项，会继承该固定项带来的偏差。对于概念结构可能不同于人类的 AI 系统，这种整体论问题尤为突出。

Conclusion: 论文认为，机制可解释性不仅可用于测量模型内部表征，也应结合整体论解释框架来共同推断系统态度与命题结构，从而为理解目标、识别欺骗和提升 AI 安全提供更可靠的理论基础。
...
TL;DR: 本文提出 PMDformer，通过补丁均值解耦、趋势恢复注意力和近邻变量注意力，提升长期时间序列预测中跨补丁形状相似性与跨变量关系建模能力。


<details>
  <summary>Details</summary>
Motivation: 长期时间序列预测在能源、金融、交通等场景中很重要。现有基于 Transformer 的 patch 策略虽能建模长程依赖，但由于不同 patch 和变量之间存在尺度差异，注意力机制难以准确捕捉真实的形状相似性，并且跨变量相关性可能随时间变化，使用过时相关性容易过拟合。

Method: 提出 patch-mean decoupling（PMD），对每个 patch 减去均值，将趋势信息与残差形状信息解耦，使注意力更关注尺度无关的形状相似性；提出 Trend Restoration Attention（TRA），在计算注意力输出时重新整合被解耦的趋势信息；提出 Proximal Variable Attention（PVA），将跨变量注意力集中在更相关、更近期的时间片段上，以减少对过时变量关系的依赖。三者结合形成 PMDformer。

Result: 在多个长期时间序列预测基准数据集上的实验显示，PMDformer 相比现有先进方法在预测准确性和稳定性方面表现更优。

Conclusion: PMDformer 通过显式解耦趋势与形状、恢复趋势信息并约束跨变量注意力范围，更有效地捕捉长期预测中的形状相似性和变量关系，是一种面向 LTSF 的有效 Transformer 改进框架。
...
TL;DR: 本文研究利用机器学习从丙型肝炎患者数据中早期检测肝硬化，Extra Trees模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 丙型肝炎可长期进展并导致肝硬化、肝衰竭及多种严重并发症；早期识别肝硬化对延缓疾病进展和改善患者预后至关重要。既有研究虽证明机器学习可用于多种疾病诊断，但尚缺乏针对丙型肝炎患者肝硬化检测的机器学习研究。

Method: 研究使用UCI机器学习库中包含2038名埃及患者、28个属性的数据集，训练并比较四种机器学习分类模型：随机森林、梯度提升机、极端梯度提升和Extra Trees模型；同时进行了特征选择，最终使用16个关键特征评估模型性能。

Result: Extra Trees模型优于其他模型，在16个特征下取得96.92%的准确率、94.00%的召回率、99.81%的精确率以及96%的ROC曲线下面积。

Conclusion: 机器学习，尤其是Extra Trees模型，能够高效、准确地辅助检测丙型肝炎患者的肝硬化风险，具有潜在临床辅助诊断价值；特征选择也可在减少输入变量的同时保持较高预测性能。
...
TL;DR: EvoOptiGraph 通过让训练数据与模型能力闭环共进化，提升大语言模型从自然语言生成混合整数线性规划模型的能力。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 自动优化建模方法受限于两点：训练语料结构多样性不足，以及数据生成流程静态、与模型学习过程脱节，导致模型泛化和失败模式修复能力有限。

Method: 论文提出 EvoOptiGraph：将每个 MILP 表示为带属性的二部图，使用保持有效性的进化算子生成结构多样实例；再通过确定性编译与验证式反向翻译，将图转化为求解器代码和自然语言描述。训练包括初始数据上的监督微调，以及带可验证奖励的强化学习；同时利用图结构中的模型弱点信号生成针对失败案例的新实例，形成数据—模型闭环更新。

Result: 在六个公开数据集上的实验表明，EvoOptiGraph 在准确率、代码可执行性和泛化能力方面显著优于更大的通用模型、智能体式方法和专用基线。

Conclusion: 面向模型弱点的定向数据生成与模型训练共进化，是提升 LLM 优化建模能力的有效策略，尤其适合需要结构正确性与可验证性的数学优化任务。
...
TL;DR: 本文提出一种面向AI望远镜调度的多级验证与可追溯推理框架，在执行前系统检查AI决策的可靠性，并显著提升调度结果的可执行性与可信度。


<details>
  <summary>Details</summary>
Motivation: AI在处理望远镜调度中的多约束复杂决策方面具有优势，但其输出可能存在数据引用不一致、推理错误和不可执行决策等问题，难以满足高可靠性天文观测任务的要求。

Method: 框架结合数据引用验证、逻辑一致性检查、观测与仪器约束验证，对AI生成的调度决策进行过滤和修正；同时引入原子推理单元及其依赖关系，将调度决策表示为可追踪的推理步骤序列，以支持错误定位和事后分析。

Result: 实验表明，该框架提升了AI调度的可执行性和可靠性，并减少了瞬变机会损失；反馈纠正与结构化推理步骤验证尤其能在复杂场景中修复或阻断错误决策。

Conclusion: 该研究证明，通过多级验证和可追溯推理机制，可以在保持AI调度灵活性的同时显著增强其可靠性与可执行性，为AI应用于高可靠天文观测调度提供了可验证路径。
...
TL;DR: 本文提出一种基于大语言模型代理的邮件自动分发系统，可根据邮件内容判断应发送到哪些学生群组，从而减少人工转发负担。


<details>
  <summary>Details</summary>
Motivation: 大型组织中邮件数量庞大，人工阅读、筛选并通过即时通讯平台转发给目标群体效率低、易出错，并会造成生产力损失和认知压力。论文希望探索一种自动化机制，保障组织内部信息及时、准确流转。

Method: 系统采用代理框架调用大语言模型，对邮件正文内容进行语义分析与决策。通过结构化提示词将邮件内容、任务指令和上下文输入模型，由模型判断邮件应路由至哪些相关学生群组。该方法不依赖人工标注数据集。

Result: 摘要声称该系统能够根据邮件内容自动识别相关接收群组并及时分发信息，带来提升工作效率、降低人工阅读与转发负担、减少认知负荷等收益。

Conclusion: 论文认为，基于LLM代理的邮件内容分析与自动路由可作为组织信息分发的有效替代方案，尤其适用于院校等存在多群组通知需求的场景。
...
TL;DR: 本文提出一种结合大语言模型、统计方法与人机协作的多语言公共服务反馈分析框架，用于识别新兴服务质量议题及潜在服务不公平问题，并在税务服务反馈场景中优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 公共部门，尤其是税务管理机构，需要从不断增长的多语言客户反馈中及时发现服务质量问题与可能的不公平现象。传统人工审查或静态专家指标方法难以扩展，也难以捕捉文本反馈中的复杂模式。

Method: 论文构建了一个人机协同框架，结合微调与量化的大语言模型、统计相似性分析以及专家监督。该方法强调上下文感知、计算效率和人工校验，以降低模型幻觉并提升输出可靠性。

Result: 通过相似性分析和资深税务人员评估，该方法相比基线模型与专家判断具有更高一致性，并能更可靠地生成与服务质量相关的洞察。

Conclusion: 研究表明，将大语言模型与领域专家知识结合，可支持公共部门进行可扩展、循证的反馈分析，有助于提升服务质量、响应能力、公平性与公众信任。
...
TL;DR: 本文提出一种面向高风险领域智能体安全的自动形式化流程，将提示、工具描述和自然语言政策文档转化为可形式验证的 Cedar 策略，以提升策略覆盖率和安全保证。


<details>
  <summary>Details</summary>
Motivation: 现有智能体安全方法存在两类不足：概率式护栏如分类器或提示引导缺乏形式保证；人工编写的符号化策略虽可验证但难以扩展到复杂、广泛的自然语言政策规范。

Method: 作者构建基于大语言模型的生成器—批评器循环，自动解析 agent prompts、MCP 工具描述与自然语言政策文档，并生成 Cedar Policy Language 表达的形式化策略，以便进行形式验证和执行。

Result: 在 MedAgentBench 基准上，自动形式化策略相较既有人工符号化 enforcement 覆盖了更多源自然语言规范内容。

Conclusion: 该方法表明 LLM 辅助的自动形式化可在保持形式化策略执行优势的同时，提高对真实政策规范的可扩展覆盖能力，为高风险领域智能体安全提供更强保障。
...
TL;DR: 本文提出 SkillDisCo，将成功执行轨迹中的共享流程结构提炼为可复用、可调用、可验证的参数化有限状态机（PFSM）程序技能，以降低智能体重复解决相似任务的推理成本。


<details>
  <summary>Details</summary>
Motivation: 智能体在相似任务实例中常从零开始推理，造成较高推理开销和冗长执行轨迹。已有工作研究了工作流复用和可执行技能归纳，但尚不清楚哪些任务场景适合形成程序性技能，以及如何从成功轨迹中表示共享的过程结构。

Method: 作者将任务建模为由有限状态机定义的场景，把成功轨迹视为未知转移图中的路径，并将程序性技能形式化为可复用的参数化控制流子图。基于此提出 SkillDisCo 框架：先从成功轨迹中蒸馏可复用 PFSM 子图，再将其编译为可调用、可执行、可验证的程序技能。

Result: 在 ALFWorld 和 WebArena 上的实验表明，SkillDisCo 在不同基准和模型规模下均能提高任务成功率，并减少智能体交互轮数。

Conclusion: 将共享经验表示为可复用的执行结构有助于提升智能体在相似任务上的效率和性能，PFSM 子图是一种有效的程序性技能表示方式。
...
TL;DR: 该论文提出 NebulaExp：一个面向 8B 级大语言模型的透明、可复现、消融驱动的后训练流程，覆盖通用指令模型与复杂推理模型两条分支，并在 SFT、GRPO RL 与教师蒸馏/偏好优化等策略上给出系统实验。


<details>
  <summary>Details</summary>
Motivation: 现有大模型后训练研究往往不公开详细的数据构造、过滤规则和训练配方，导致社区难以复现，也限制了轻量级模型的优化。论文旨在提供一套透明、可复现的后训练实践，并分析不同能力之间的权衡。

Method: 基于 Qwen3-8B-base 构建两条正交分支：通用 Instruct 分支和复杂 Reasoning 分支。作者整理 384 万多源 SFT 样本与 20 万可验证 RL 候选池，设计响应蒸馏、多维交叉验证过滤、细粒度难度分级、任务分类和多样性感知采样等数据处理流程；训练上采用三阶段 SFT、GRPO 强化学习，以及单教师 OPD 和多教师 MOPD。

Result: Instruct 分支中，NebulaExp-Ins-SFT 将平均基准分从 Qwen3-8B-nothink 的 55.01 提升到 60.99，GRPO 进一步提升到 61.85。Reasoning 分支中，中等难度 GRPO 将平均推理分从 73.88 提升到 75.17。OPD 仅用 4K 指令样本在 IFEval 上比 RL 基线高 3.26 分，整体平均提升 4.43；MOPD 融合四个领域教师，仅用 10K 样本使平均性能较基座模型提升 4.18。

Conclusion: 论文表明，透明的数据工程、分阶段 SFT、适度难度 RL 与教师驱动的 OPD/MOPD 可有效提升 8B 级模型的指令遵循、推理和综合能力；同时，研究揭示了指令遵循、数学推理、代码生成与通用知识之间存在明显能力权衡。
...
TL;DR: 论文质疑安全护栏中使用链式思考（CoT）的必要性，提出轻量级 label-only 编码器 LeanGuard，在相近准确率下显著降低推理开销。


<details>
  <summary>Details</summary>
Motivation: 现有护栏方法常先生成 CoT 再给出判定，假设逐步推理能提升审核决策质量；但这会带来大量额外 token 生成，使系统变慢、变重，不适合端侧或实时部署场景。

Method: 作者在同一语料上训练轻量级双向编码器与推理型护栏，并在控制变量设置下仅移除推理过程、保持其他因素固定，以比较 CoT 是否真正提升审核性能。最终提出 LeanGuard，一个 395M 参数、仅输出标签的编码器模型，单次前向处理最多 512 token。

Result: LeanGuard 在公开基准上取得平均 F1 82.90±0.26，性能匹配基于更大解码器的推理型护栏，同时推理计算量约降低 100 倍；在训练标签噪声下仍保持鲁棒，并在严格假阳性率约束下比推理护栏保留更高召回率。

Conclusion: 实验表明 CoT 并未提升当前审核基准上的准确率，较重的推理型护栏也不一定更鲁棒；当前安全护栏基准可能不足以体现推理优势，CoT 对内容审核的必要性尚未被证明。
...
TL;DR: 论文提出 Kalman Prototypical Network（KPN），用于联合循环燃气轮机在标注故障数据稀缺场景下的少样本故障诊断，并在仿真泄漏故障任务中优于多种传统少样本学习方法。


<details>
  <summary>Details</summary>
Motivation: CCGT 系统具有复杂的热流体与机械耦合特性，导致故障检测困难；现实工业场景中标注故障样本通常稀缺，因此需要一种在少样本条件下仍具备稳定性、鲁棒性和泛化能力的诊断方法。

Method: 作者提出一种基于度量学习的少样本学习框架 KPN，将类别原型的演化建模为动态系统中的潜在随机状态，并利用 Kalman 思想稳定原型表示，从而降低 episodic 训练中的方差、增强嵌入表征鲁棒性。实验数据由基于 Modelica 的高保真海上 CCGT 动态仿真模型生成，覆盖正常运行和瞬态条件下的渐进泄漏故障。

Result: 在模拟泄漏故障检测任务中，KPN 在不同 support/query 配置下，相比 Matching Networks、Relation Networks 和 MAML 等方法表现出更高的准确率和更好的稳定性，同时改善了训练收敛。

Conclusion: KPN 通过稳定类别原型提升了少样本故障诊断的泛化能力和鲁棒性，适合用于标注数据有限的真实 CCGT 故障检测场景。
...
TL;DR: 本文提出 LithoDreamer：首个面向计算光刻的物理信息世界模型框架，将版图到掩模、抗蚀剂图像、显影后图像的流程建模为多步状态演化系统，并在正向预测与逆向规划任务上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 随着半导体工艺节点持续缩小，计算光刻对良率和性能至关重要。但光刻本质上是包含掩模优化、光学成像、抗蚀剂曝光与显影的连续物理过程，现有模型难以完整刻画该多阶段、物理耦合的演化过程。

Method: 作者提出 LithoDreamer，将“Layout-Mask-Resist Image-ADI”流程表述为决策驱动的多步演化系统；通过建模相邻状态间的特征变化，学习阶段特定的物理信息潜空间；并提出对比式变分优化范式，在缺乏连续监督的条件下，对比不同干预路径的潜在差异并施加变分演化约束，以引导模型生成符合真实光刻物理规律的状态演化。

Result: 实验表明，LithoDreamer 在正向演化预测和逆向规划两个任务上均达到当前最优性能。作者还公开了相应光刻数据集。

Conclusion: LithoDreamer 为计算光刻提供了一种可解释、物理约束且支持干预优化的世界模型范式，有望提升多阶段光刻流程建模、预测和反向设计能力。
...
TL;DR: 本文提出一种基于图网格自编码器与心率感知神经ODE的潜在动力学模型，用完整心动周期CMR运动表征预测心力衰竭风险，优于传统CMR指标。


<details>
  <summary>Details</summary>
Motivation: 传统心血管风险模型通常只使用少量来自特定心动相位的影像指标，未充分利用CMR中关于双心室结构与全周期运动的丰富时空信息，因此可能遗漏具有预后价值的心脏表型。

Method: 模型将双心室解剖结构和cine CMR全周期运动编码为连续潜在轨迹；采用图结构网格自编码器重建解剖一致的3D+t心室运动，并用心率感知神经常微分方程建模潜在动力学。通过协变量条件先验定义期望的舒张末期潜在状态，再用Cox比例风险模型评估个体相对该先验的偏离是否预测新发心力衰竭。研究对象为72,386名无基线心血管疾病的UK Biobank参与者，其中367例发生新发心力衰竭。

Result: 在留出评估子集中，将潜在评分加入重新拟合的 pooled cohort equations 后，分层C-index由0.704提升至0.785；相比之下，加入7个既有心脏标志物后的C-index为0.764。与非图结构和非ODE方法相比，该模型在重建保真度、生成真实性和下游预后性能之间取得最佳权衡。

Conclusion: 连续建模全心动周期心室运动能够提取超越传统CMR摘要指标的预后信息，有望形成更丰富的心脏影像表型；但在用于临床风险预测前，仍需在更具代表性的外部患者队列中验证。
...
TL;DR: 本文提出 AHOIS：一种将苏格拉底式质询嵌入闭环实验的多智能体 AI 科学家系统，用于推动从固定流程自动化走向能自我修正的自主科学发现。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 科学系统虽能操作仪器、优化参数和生成假设，但多数仍依赖人类预设流程，缺乏根据证据构建、质疑和修正物理解释的“认识论自主性”。论文旨在解决复杂物理环境中 AI 如何自主形成可检验解释并改进实验策略的问题。

Method: 作者构建 AHOIS 多智能体系统，引入物理批判智能体，通过因果追问、约束检查、反例生成和可证伪标准制定来审查假设，并在真实多模光纤光学平台上进行闭环实验验证。该平台具有高维波变换、间接探测、环境漂移和多模态采集等复杂性。

Result: AHOIS 在无先验编码方案、分类器或散斑模型的条件下，提出并验证了随机干涉编码假设，发现任务自适应稀疏测量策略，诊断出编码不稳定、荧光污染和探测器噪声等失败模式，并将已有成像协议迁移为非原始配置下的可执行流程。其编码实现了 16×16 测量、有效秩 56.9，在 MNIST 和 Fashion-MNIST 上分别达到 76.97% 与 83.17% 分类准确率。消融实验显示苏格拉底式质询提升了物理一致性、假设完整性、不确定性校准和实验计划有效性。

Conclusion: 研究表明，将批判性质询与闭环实验结合，可增强 AI 系统的证据驱动、自我纠错和自主发现能力，为复杂物理系统中的自主科学探索提供了可行路径。
...
TL;DR: 本文将科学发现形式化为“元优化”问题，提出共识目标聚合方法，用LLM生成并融合评价目标，在3-SAT算法发现中显著降低复杂度标度并提升速度。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的自动科学发现多关注在理论与实验的状态空间中搜索候选方案，但评价标准本身通常被视为固定。作者认为，科学研究过程中对“什么是好结果”的判断会随理解深化而变化，因此需要同时优化搜索对象与评价目标。

Method: 论文提出元优化框架，将研究目标函数也纳入优化过程。核心方法是“共识目标聚合”：由LLM生成多个目标函数，再通过基于相关性的加权投票进行组合，形成更稳定、可自我校正且可演化的评价准则。该框架被应用于基于数字MemComputing机器的3-SAT算法发现。

Result: 在3-SAT问题实验中，该方法将基线随问题规模N的标度从约N^2.51降低到约N^1.33，并在最大测试实例上实现约67倍加速。

Conclusion: 该工作表明，自动科学发现不仅应优化候选理论或算法，也应动态优化评价标准。共识目标聚合为构建稳健、可演化的研究评价机制提供了可行路径，并可能作为问题无关框架促进更广泛的科学发现。
...
TL;DR: 本文提出 EGG，一个专家引导的多智能体 GPU Kernel 生成框架，通过将优化流程拆分为算法结构设计与硬件特定调优两个阶段，引导 LLM 更稳定地生成正确且高性能的内核。


<details>
  <summary>Details</summary>
Motivation: LLM 训练与推理带来快速增长的计算开销，高性能 GPU Kernel 对降低成本至关重要；但现有 Kernel 开发依赖专家手工调优，而 LLM 自动生成方法缺乏领域优化指导，难以同时保证正确性与性能。

Method: EGG 将专家优化原则融入 LLM 决策流程，并模拟专家工作流，将 Kernel 生成分为两个层次化阶段：首先进行算法结构设计，确定高质量计算结构；随后进行硬件特定调优，包括并行映射、张量分块和内存优化。同时设计阶段感知的多智能体协作机制，用于管理阶段内与阶段间上下文，稳定优化轨迹。

Result: 在 KernelBench 和真实工作负载上的实验表明，EGG 相比 PyTorch 平均获得 2.13 倍加速，并优于已有基于智能体和强化学习的方法。

Conclusion: EGG 证明了将专家优化知识、分阶段设计空间分解与多智能体协作结合，可以显著提升 LLM 自动生成 GPU Kernel 的性能与稳定性，是面向 LLM 计算优化的有效自动化方案。
...
TL;DR: 本文提出 ResilPhase，将扩散 Transformer 的加速推理重构为 ODE 空间中的稳定宏轨迹外推，通过全局漂移对齐、无导数重心 Lagrange 外推和有界相位映射，在高加速比下提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有“cache-then-forecast”方法虽能降低 DiT 推理延迟，但在高加速比下质量显著下降。作者指出根因在于：在与连续扩散轨迹不对齐且数值不稳定的中间表示上做离散外推，导致空间误差累积、噪声导数放大和高阶不稳定。

Method: 作者将加速推理表述为 ODE 空间中的宏轨迹外推，不再预测中间特征，而是对齐模型端到端状态演化的 Global Drift，以减少特征不一致和内存开销；同时采用无导数的重心 Lagrange 外推器避免高阶导数噪声；并引入有界 Phase Mapping 正则化外推域，抑制振荡误差增长。

Result: 在 FLUX.1-dev 和 HunyuanVideo 上的实验表明，ResilPhase 在激进加速比下相较已有方法取得了更优的保真度，达到当前先进水平。

Conclusion: ResilPhase 通过从特征级导数预测转向 ODE 宏轨迹外推，并结合无导数稳定外推与有界相位映射，有效缓解高加速扩散推理中的数值不稳定与质量退化问题。
...
TL;DR: 本文提出“记忆深度”概念，并用 loop-drift 协议评估一种基于惊讶度与效价门控的 LoRA 参数化巩固方法 EVAF，表明其能在上下文卸载后更好保持目标导向行为，与检索式记忆互补。


<details>
  <summary>Details</summary>
Motivation: 长程语言智能体仅依靠检索系统不足以维持持续行为：检索能在查询时取回事实，但无法决定哪些经验应在工作上下文卸载后继续影响行为。因此论文关注独立于检索访问的“记忆深度”，即将目标条件化倾向持久写入小型参数存储。

Method: 作者提出 loop-drift 协议作为受控压力测试：保持检索索引完整，同时卸载工作上下文，并引入长循环干扰，考察目标条件行为能否持续。方法上评估 EVAF，一种由惊讶度和效价门控的 LoRA 巩固机制；并通过随机门控、固定内环写入强度、匹配门控反转等机制控制实验，在 GPT-2、TinyLlama、Mistral-7B 等模型上分析选择与执行两个维度。

Result: 结果显示，检索在浅层事实回忆上最强，短事实准确率达到 0.956--0.973；EVAF 在目标持久性与上下文卸载后的恢复上表现更优，得分为 0.812--0.904，并且每 200 个事件仅需 2--3 次参数写入。机制实验表明，选择性巩固可分解为“选择”和“执行”两个可控维度；内环写入强度具有模型依赖性，且在执行校准错误时会出现不对称的选择-执行耦合。

Conclusion: 论文认为，选择性参数化巩固提供了一种区别于检索访问、且与其互补的记忆能力，可增强长期语言智能体在上下文卸载后的目标保持能力。不过，外部 Memora 事件流诊断暴露出陈旧记忆失效/撤销仍是未解决边界问题。
...
TL;DR: 论文提出一种让大语言模型在生成过程中自动从知识库检索事实知识的方法，以提升事实性、可更新性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的事实知识通常存储在参数中，更新成本高、来源难追踪，且小模型在事实准确性上往往弱于大模型。研究希望通过外部知识库介入生成过程，使模型输出的事实既可动态更新，又可溯源验证。

Method: 核心方法是训练模型在生成 token 时产生特殊触发 token；当触发 token 出现后，系统向知识库发起查询，并将检索到的事实知识用于后续生成，从而把参数化知识与外部结构化知识结合起来。

Result: 实验表明，该方法在短文本和长文本生成任务中都提升了事实 grounding；同时，事实修订可以通过编辑知识库生效，而不需要重新训练或更新模型参数。

Conclusion: 该方法为提升 LLM 的事实准确性、透明性和知识更新效率提供了有效路径，并可能使较小模型在事实性方面接近较大模型的表现。
...
TL;DR: 本文分析40名30–50岁健康成人的心率变异性（HRV）指标，评估其分布、稳定性、相关性、可重复性与跨研究一致性，并筛选更适合代表HRV不同成分的指标。


<details>
  <summary>Details</summary>
Motivation: HRV是反映心脏自主神经调节和生理状态的重要指标，可辅助疾病诊断；但健康人群HRV参数研究不足，且缺乏公认金标准，限制了其临床和研究应用。

Method: 研究纳入40名健康成人（男女各20名），采用计算信号处理与数据分析方法，对时域、频域和非线性HRV指标进行分析，围绕正态性、稳定性、相关性、可重复性和一致性五个问题展开，并与Fantasia数据库进行比较。

Result: 时域和非线性指标，尤其全局及低频相关指标较多服从正态分布，并存在性别差异；除高频相关指标外，多数指标较稳定；高频相关指标之间高度相关，提示冗余；与Fantasia数据库相比，多数指标误差低于10%，但女性SD2和SDNN误差超过15%；时域和非线性指标跨研究变异较低，频域指标变异较高。

Conclusion: ApEn和IRRR可代表全局变异性，HRVi和SD2可代表低频成分，MADRR或rMSSD可代表高频成分；这些指标更适合用于健康人群HRV表征，有助于提升HRV在临床和科研中的实用价值。
...
TL;DR: 论文提出 Capability Frontier，用以衡量在给定成本下通过跨模型与多次生成的最优选择可达到的集体 LLM 能力；实验显示传统单模型单次评测显著低估真实能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常只报告单个模型单次运行的准确率，无法反映真实部署中多模型各有所长、同一预算下可多次采样并筛选答案的情形，因此可能系统性低估 LLM 在异构任务分布下的总体能力。

Method: 作者构建 Capability Frontier，即在模型集合上形成成本—性能的 Pareto 前沿，并通过 oracle 在不同模型与多次生成之间进行最优选择；同时校正两类偏差：单模型评测导致的低估，以及对噪声样本取最大值导致的高估。实验覆盖 21 个 LLM 和 16 个常用基准，并辅以受控概率模拟分析查询主题熵对 oracle 路由收益的影响。

Result: 相较各基准的最佳单模型，在匹配成本下，校正单模型评测带来 54% 的错误率降低；进一步校正单次运行后性能提升达 82%；同时可在降低 85% 成本的情况下匹配 SOTA 准确率。模拟表明，查询主题熵越高，oracle 路由相对于最佳单模型的性能差距近似单调增大。

Conclusion: 集体 LLM 能力被现有评测范式显著低估。对于数据分布异构、多领域部署场景，评测与系统设计应考虑跨模型路由、多次采样和成本—性能前沿，而非仅依赖单模型单次准确率。
...
TL;DR: 本文提出 CASOP 框架，用于在给定仓库上下文下自动筛选、组合并评估订单履约优化算法流水线，以支持人工拣货到货模式仓库中的算法选择与流程设计。


<details>
  <summary>Details</summary>
Motivation: 人工 picker-to-goods 仓库的订单履约涉及物品分配、订单批处理、拣货路径等相互关联的决策。尽管集成模型能刻画这些交互，但实际系统常因组织边界、职责分工或数据受限而采用分解式方法。现有研究多聚焦单一子问题或固定组合，缺乏通用机制来判断算法适用性、组合合法流水线并系统评估性能。

Method: 作者提出 Context-Aware Synthesis of Optimization Pipelines（CASOP）框架，包括：模块化算法库、描述仓库上下文与算法需求的语义数据卡和算法卡、订单履约问题分类体系、根据上下文识别适用算法并组合所有有效流水线的合成器，以及评估流水线性能的评估器。

Result: 在覆盖四类问题的 7 组基准实例上进行验证，框架生成了 1,063,044 条有效优化流水线，并可对这些流水线进行性能评估。相关软件以开源形式发布。

Conclusion: CASOP 为研究者和实践者提供了一种面向上下文的通用方法，可自动构建、筛选和评估仓库订单履约中的有效高性能算法流水线，弥补了现有研究在算法配置与流水线合成方面的不足。
...
TL;DR: 本文提出一种面向生命周期评价（LCA）解释阶段的“视角条件化检索增强生成”框架，用于将环境热点与改进机会转化为可执行的战略路径。


<details>
  <summary>Details</summary>
Motivation: 传统LCA解释阶段缺乏结构化机制，难以在技术、社会和政策不确定性下，把量化的环境改进机会转译为面向实施的行动方案；同时，直接使用大语言模型存在幻觉和证据不足风险。

Method: 研究构建了多视角融合RAG架构，覆盖学术、产业、公众话语与欧盟资助数据集。流程包括：设定情景锚点以明确系统边界和脱碳目标；生成视角特定的微查询并进行受约束检索；在不再检索的条件下，仅基于账本记录输出进行中立综合。案例采用意大利苹果生产设施中氢能替代柴油的减排场景，并使用GPT-5 nano作为推理模型。

Result: 该框架通过结构化检索与受控综合，在保持跨领域信息多样性的同时降低模型幻觉风险；案例展示了其可将LCA影响结果进一步组织为战略实施路径。

Conclusion: 该概念验证表明，AI辅助且证据支撑的LCA解释方法可增强传统LCA之外的实施导向决策能力，尤其适用于可能规模化部署的技术方案评估。
...
TL;DR: 本文摘要提出 AgentX：一个用于推荐算法迭代的生产部署多智能体闭环系统，能够自动生成想法、实现代码、上线评估并从实验中学习。


<details>
  <summary>Details</summary>
Motivation: 推荐算法迭代虽正从手工工程流程走向工业化研究闭环，但仍受制于结构性执行瓶颈：假设生成、生产代码修改、A/B 实验上线和结果归因依赖人工工程师，导致创新速度随人力线性扩展，难以充分利用证据、算力与历史实验知识形成复利式增长。

Method: AgentX 由四个紧耦合阶段组成：Brainstorm Agent 基于历史实验、系统架构、数据分析和外部研究生成并排序可执行方案；Developing Agent 结合代码仓库语境生成生产级代码，并进行多维可靠性验证；Evaluation Agent 通过安全在线灰度与带护栏否决机制的 A/B 判断评估效果，并将成功和失败沉淀为结构化知识；Harness Evolution 层使用 SGPO 将执行轨迹蒸馏为语义梯度更新，持续改进各智能体。

Result: 摘要声称该系统已生产部署，能够以超出人工流程可持续范围的规模和速度自主执行推荐实验，并把实验结果转化为知识资产以支撑后续迭代。

Conclusion: AgentX 的核心贡献在于把推荐算法研发从人工驱动的线性流程重构为自动化、自学习的闭环开发引擎，有潜力显著提升推荐系统实验迭代效率与知识复用能力。
...
TL;DR: 提出 TAVR-VLM 框架，通过风险条件因果 grounding 注意力减少 TAVR 规划中多模态大模型的诊断幻觉，并在 M³TAVR 队列上取得更优性能。


<details>
  <summary>Details</summary>
Motivation: TAVR 术前规划依赖复杂的多模态解剖推理，但现有 MLLM 在高风险医疗场景中容易产生缺乏解剖依据的诊断幻觉，限制了其临床可信度与可解释性。

Method: 构建 TAVR-VLM，引入 Risk-Conditioned Causal Grounding Attention（R-CGA），形成“风险→区域→词”的结构化 grounding 路径。该方法将多模态输入压缩为因果风险瓶颈，生成全局风险掩码，并在自回归生成阶段通过支持投影的因果一致性目标约束词级 grounding。

Result: 在包含 1,482 名患者的 M³TAVR 队列上，模型达到 AUROC 0.896、CIDEr 0.936，并将幻觉率降低至 8.1%，刷新当前最佳表现。

Conclusion: TAVR-VLM 通过风险约束的解剖 grounding 提升了多模态生成的准确性、可解释性与安全性，为循证外科 AI 和 TAVR 术前规划提供了更可靠的技术路径。
...
TL;DR: 本文提出一个用于生成合成临床病历文本的模块化流水线，并发布包含70名合成患者、每人20–50份临床记录的数据集，用于在不依赖真实患者数据的情况下开发和评估临床AI系统。


<details>
  <summary>Details</summary>
Motivation: 真实临床文档高度敏感，医疗领域AI系统开发与评估常受数据访问、隐私和合规限制影响；因此需要既能降低隐私风险、又具备纵向一致性与临床真实感的合成临床笔记数据。

Method: 作者构建模块化合成数据流水线，结合结构化患者生成、半结构化患者就医旅程模拟，以及基于大语言模型的非结构化临床笔记生成；同时引入LLM验证与数据增强机制，以提升记录的忠实性、现实感、多样性，并保持纵向患者记录内部一致。

Result: 发布了一个包含70名合成患者的数据集，每名患者对应20–50份覆盖完整住院旅程的临床笔记；数据集提供多个验证级别，使用户可根据应用需求在真实性与规模化之间权衡。

Conclusion: 该工作为临床AI工具的开发、测试和评估提供了可用的合成临床文本资源，适用于摘要、医学编码、决策支持等任务，并有助于减少对真实患者数据的依赖和隐私风险。
...
TL;DR: 本文提出“模式保持的属性检索”任务，并用连续生成式检索框架 MO-DiT+HPPO 在保持种子集合细粒度模式的同时提升目标属性命中，实验显示在多个领域与泛化协议下优于基线。


<details>
  <summary>Details</summary>
Motivation: 传统嵌入检索通常返回与查询最相似的最高分物品，但生产场景中常需要基于种子集合扩展更多物品：既满足目标属性，又不偏离种子集合所体现的细粒度模式。单纯平均种子可保留模式但属性不足，全局属性检索则容易漂移到无关模式，因此需要兼顾属性提升与模式纯度的新方法。

Method: 作者形式化提出 pattern-preserving attribute retrieval，并采用连续生成式检索：模型读取物品嵌入序列，生成用于近邻搜索的查询嵌入。框架 MO-DiT+HPPO 包含原始序列预训练、多领域按指标排序的继续预训练、尾部质心微调以及 HPPO。指标排序训练将稀疏在线检索标签转换为从低到高预测属性密度的模式内轨迹；HPPO 用混合候选池和在线交集指标构造偏好标签，通过参考锚定偏好优化对齐真实目标，并用 Pareto pair filter 过滤会降低同模式纯度的偏好对。

Result: 在四个属性领域、item-holdout 与 pattern-holdout 协议下，metric-ordered DiT 相比预训练生成式检索器提升交集指标，HPPO 进一步提升；八个领域划分单元中七个取得显著增益，最困难划分中为边际持平。验证实验、顺序消融、CPT/SFT 对比和候选策略消融解释了性能提升来源。

Conclusion: 该研究表明，通过指标有序的生成式检索训练与面向在线目标的偏好优化，可以有效缓解属性提升与模式保持之间的冲突，在跨领域和保留模式泛化设置中实现更稳健的模式保持属性检索。
...
TL;DR: 本文提出 ReTeX，从单一合并模型检查点中预测参数偏移以恢复各任务专家性能，减少动态合并在推理时对冗余专家组件的依赖，并在视觉与 NLP 任务上恢复超过 95% 的专家性能。


<details>
  <summary>Details</summary>
Motivation: 多任务模型合并希望把多个任务专属专家整合为统一模型，但静态合并易产生参数干扰；动态合并虽能缓解问题，却通常需要在推理阶段存储和加载多个冗余专家组件，带来较高成本。本文将合并过程中的参数干扰视为施加到每个任务专家上的参数扰动，目标是在不保留完整专家的情况下恢复专家能力。

Method: 作者将参数扰动建模为仿射变换，并近似为可预测的加性偏移，提出 Recover Task eXpert（ReTeX）框架来预测这些 offset，从而抵消合并带来的参数干扰。对于任务身份未知的场景，方法在推理前离线计算各任务的 SVD 子空间签名；推理时无需路由器，通过选择输入在各任务子空间中投影残差最小的任务来进行任务识别，并据此恢复相应专家。

Result: ReTeX 在视觉和 NLP 领域均能恢复超过 95% 的单独专家性能，并显著提升对未见任务的泛化能力。实验还显示，参数偏移预测会产生对专家知识的自适应插值能力，尤其有助于处理分布外任务。

Conclusion: ReTeX 提供了一种高效的多任务模型合并恢复机制：仅依赖单一合并检查点，通过预测参数偏移与无路由任务识别来减少参数干扰，同时兼顾已见任务性能恢复和未见/OOD 任务泛化。
...
TL;DR: 论文指出长程智能体式大语言模型的 OOD 泛化差，核心原因之一是“任务不敏感”：模型在相似但不同的任务中沿用训练模式。作者提出轻量级对比正则 Task-Perturbed NLL Optimization，以增强动作对任务指令的依赖，从而提升任务敏感性和 OOD 泛化。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型虽能作为长程决策智能体，但在分布外任务上容易失败。作者关注一种具体失败机制：模型可能忽视当前任务指令，依赖训练中学到的局部观察或动作模式，导致面对语义损坏或替换后的任务描述时仍输出原任务相关动作。

Method: 作者首先通过实验分析模型在语义损坏指令、相似但不同任务替换等设置下的行为，并观察训练期间注意力从任务 token 漂移到局部观察的现象。为缓解该问题，提出 Task-Perturbed NLL Optimization：一种轻量级对比式正则方法，通过扰动任务描述并调整负对数似然优化目标，显式鼓励模型动作依赖任务指令。

Result: 实验显示，该方法能够提升模型对任务描述变化的敏感性，改善分布外泛化表现，同时使模型在训练和推理中对任务 token 保持更稳定的注意力分配。

Conclusion: 论文认为，任务不敏感是长程 LLM 智能体 OOD 泛化不足的重要原因；通过在训练目标中显式约束动作与任务指令之间的依赖关系，可以减少快捷模式学习，提高任务理解与泛化能力。
...
TL;DR: 本文研究CoT训练究竟提升了模型的显式推理作用，还是提升了模型不依赖CoT、直接从提示中预测动作的能力。结果表明，CoT训练主要显著改善prompt action质量，而非扩大CoT相对优势。


<details>
  <summary>Details</summary>
Motivation: 已有研究指出，语言模型生成的链式思维（CoT）未必忠实，可能只是事后合理化；因此需要厘清CoT训练带来的性能提升来自“推理改变动作”的能力，还是来自“直接根据提示预测动作”的能力。

Method: 作者比较两类动作预测：不生成CoT时直接预测的prompt actions，以及生成CoT后预测的CoT actions；跨训练检查点评估二者在环境交互中的质量和相对优势，并分析后期模型是否更少根据CoT修正动作。进一步提出在部分训练样本中选择性遮蔽动作token监督的干预方法。

Result: 随着训练推进，prompt-action质量显著提升；CoT actions相对prompt actions的优势基本保持稳定，并未随CoT训练扩大。后期检查点更少因CoT而修改动作，显示模型更依赖提示本身。选择性遮蔽部分动作token监督可提升域外泛化。

Conclusion: CoT训练的收益很大程度上可能来自增强模型从提示直接预测动作的能力，而不是增强生成推理对行动选择的因果影响。减少对动作token的直接监督可缓解这种倾向，并改善域外泛化。
...
TL;DR: 本文提出 Look-Before-Move 相机规划框架，使具身相机在动态 3D 故事世界中先决定应观察什么，再生成平滑、可行且符合叙事意图的运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有视觉感知与相机运动方法往往被动解释已有观测，或直接生成相机轨迹，缺少在动态 3D 环境中根据叙事目标主动选择观察对象、构图方式与注意力转移时机的能力。

Method: 作者提出 Narrative-Grounded World Visual Attention 问题定义，并设计 Look-Before-Move 框架：首先用 Semantic Observation Contract 将导演式叙事意图转化为可执行视觉约束；其次通过 Monte Carlo Viewpoint Search 搜索满足叙事与几何约束的视点；最后使用 Semantic Trajectory Grounding 将选定视点连接成连续、避碰且时间一致的相机轨迹。同时构建基于 StoryBlender 的动态 3D Story World Benchmark。

Result: 在包含 50 个故事、457 个场景和 1585 个镜头的基准上，实验表明该框架相较代表性基线提升了主体感知、意图一致性与轨迹质量。

Conclusion: 研究证明，在生成相机运动之前先组织视觉注意力，对于动态 3D 叙事环境中的具身视觉与相机规划至关重要。
...
TL;DR: 论文提出 Einstein World Models（EWM）：让大语言模型在推理链中调用可生成视觉-时间展开的世界模块，把可检查的场景假设作为中间推理依据，以增强纯文本推理难以覆盖的复杂思考能力。


<details>
  <summary>Details</summary>
Motivation: 作者关注智能是否需要超越直接经验进行反事实推理，并质疑仅依赖语言是否足以表达某些复杂思维；因此探索“可视化反事实事件”是否能补充语言，提升 LLM 的复杂推理能力。

Method: 提出 EWM 作为一种 LLM 推理系统蓝图：LLM 在推理过程中调用 world-module 生成短时序场景 rollout；该 rollout 不直接作为最终答案，而是作为可检查、可被后续推理利用的假设性视觉-时间证据，类似将工具调用从搜索/代码扩展到视觉思想实验。

Result: 摘要未报告具体实验结果或量化性能，只说明该框架旨在扩展 LLM 的工具调用能力，使其能够利用视觉化的时序假设辅助推理。

Conclusion: EWM 的核心结论是：在 LLM 推理链中嵌入视觉-时间模拟，有潜力弥补纯文本推理的局限，为复杂推理和反事实思考提供新的系统设计方向。
...
TL;DR: 论文提出 AURORA-AI：一种面向非平稳部署环境的自适应资源编排框架，通过闭环控制在异构 AI 模型间动态分配计算预算，以同时优化性能、公平性、成本、延迟、鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现代 AI 系统常在计算资源、用户/群体分布和运行条件持续变化的环境中部署，静态资源分配会导致预测性能下降，并损害公平性、可解释性等以人为中心的属性。因此需要一种能在扰动、漂移和突发事件下保持稳定与公平的自适应编排机制。

Method: AURORA-AI 将 Hamilton-Jacobi-Bellman 反馈控制、Lyapunov 稳定性监测和公平性感知的复合效用函数统一到闭环策略中。该策略持续在一组异构 AI 模型之间重新分配计算预算，使全局效用在预测性能、人口统计均等性、成本、延迟、鲁棒性和可解释性等目标上最大化。实验采用离散时间仿真，同时注入人口偏差冲击、渐进概念漂移和突发黑天鹅扰动，并与 Static、Round Robin、Greedy、LinUCB 和基于 PPO 的深度强化学习控制器比较。

Result: 相较于 Static 基线需要 88 个时间步、PPO 需要 22 个时间步恢复，AURORA-AI 在黑天鹅事件后实现即时恢复；alpha 分位数和 super-quantile 分别提升 29% 和 25%；同时降低平均与最大人口统计均等性差距，并提高 Lyapunov 稳定运行步数比例。

Conclusion: 结果表明，将公平性感知效用优化与稳定性理论结合的自适应资源编排，是实现韧性、可靠且以人为中心的 AI 部署的一条具有实践价值和理论依据的路径。
...
TL;DR: 本文研究多智能体 LLM 迭代循环的语义早停机制，用语义变化与质量改进信号替代固定迭代上限；在 HotpotQA 上，免评审语义早停可在质量基本不变的情况下显著节省 token。


<details>
  <summary>Details</summary>
Motivation: 现有 Writer-Critic 等多智能体 LLM 循环通常依赖固定 max_iterations 停止，这种语法式终止条件无法感知答案是否仍在改进，导致简单问题浪费 token、困难问题可能被过早截断。

Method: 提出基于连续草稿嵌入余弦距离与 patience window 的语义早停，并结合答案质量是否继续提升的质量门控变体；理论上证明确定性终止与良定义性，并用机器检查验证；实验上生成完整轨迹后重放不同停止策略，缓存 LLM judge 调用，区分运行 token 与评估 token。

Result: 在 HotpotQA 60 个问题测试集上，judge-free 语义停止器相对 max_iterations 减少 38% 运行 token，质量保持同等水平（Delta-IS=-0.004, p=0.81）；完整质量门控版本因每轮评审成本过高而得不偿失；oracle 最优轮选择较所有实际策略提升 +0.115 Information Score（p≈4e-11）。

Conclusion: 语义早停是比固定迭代上限更高效的实际终止策略，但将 LLM judge 纳入每轮决策可能不经济；未来关键问题从“何时停止”转向“如何识别哪一轮答案最好”。
...
TL;DR: 本文综述有真实标签时用于聚类评价的常见外部有效性指标，重点讨论基于集合匹配的度量，并给出不同需求下的指标选择建议。


<details>
  <summary>Details</summary>
Motivation: 在聚类任务中，当存在 ground truth 时，需要选择合适的外部评价指标；不同指标在可解释性、粒度、归一化以及对簇大小的敏感性方面存在差异，因此有必要进行系统比较并提供使用建议。

Method: 文章回顾常见外部有效性指标，特别关注 set-matching-based measures，并从簇级可解释性、点级精细度、簇大小偏置和样本等权性等角度比较 CI、PSI、ACC 等指标。

Result: 作者推荐 centroid index（CI）作为直观、可解释的簇级指标；若需要更细粒度的点级评价，可选择 PSI、ACC 或其他集合匹配指标。PSI 提供归一化分数且不受簇大小偏置影响；ACC 适合所有样本点应被同等重视的场景。

Conclusion: 外部聚类评价指标的选择应依据评价目标：若强调簇级直观解释，优先使用 CI；若强调点级精细评价且避免簇大小偏差，可使用 PSI；若希望所有点等权参与评价，可使用 ACC 或类似集合匹配度量。
...
TL;DR: JERP 将经验规则学习与策略参数更新联合起来，使 LLM Agent 能在多步交互任务中更有效利用历史轨迹经验。


<details>
  <summary>Details</summary>
Motivation: 多步交互环境中的 LLM Agent 需要利用累积经验。现有方法要么将经验保存为外部自然语言规则用于提示，具备可解释性但可能与不断变化的策略不同步；要么用轨迹和反馈更新模型参数，能提升整体策略但在稀疏奖励场景下对局部错误纠正有限。论文旨在统一这两类经验利用方式。

Method: 提出 JERP（Joint Learning of Experiential Rules and Policies）。该方法从同一批交互轨迹中同时更新长期经验规则池与策略模型。决策时，系统检索与当前任务相关的规则，并将其与交互历史一起作为条件输入给 Agent。每轮任务结束后，JERP 使用收集到的轨迹优化策略，并通过比较当前 rollout 与参考成功轨迹来修订规则池。

Result: 在 AlfWorld 和 WebShop 两个复杂交互任务基准上，JERP 相比已有方法取得了稳定的决策性能提升。

Conclusion: JERP 通过耦合经验规则更新和策略学习，使规则池与演化中的策略保持一致，同时让稳定有效的行为逐步被模型参数吸收，从而提升 LLM Agent 在复杂多步交互环境中的表现。
...
TL;DR: 本文提出 PAVE 分步标注协议与 OpenRCA 2.0 基准，用于更严格评估 LLM Agent 的根因分析能力，揭示仅看最终根因会掩盖“未被因果路径支撑的诊断”问题。


<details>
  <summary>Details</summary>
Motivation: 现有 RCA 数据集通常只标注根因，而缺少从根因到观测症状的因果传播路径，导致任务容易退化为模式匹配，难以真实评估 LLM Agent 的长上下文理解、多步推理和工具使用能力。

Method: 作者提出 PAVE，一种分步标注协议，利用故障注入中的已知干预信息，通过从原因到结果的前向验证来重构因果传播路径；基于该协议构建 OpenRCA 2.0，包含 500 个跨系统 RCA 实例，并带有分步因果注释。

Result: 在 11 个前沿 LLM 上，精确恢复根因集合的平均成功率仅为 20.7%；放宽标准后，模型在 76.0% 的案例中能识别至少一个正确根因服务，但只有 61.5% 能将该服务与经验证的因果传播路径及观测症状联系起来。

Conclusion: 仅评估最终输出会掩盖 LLM Agent 在因果归因上的缺陷；分步因果真值对于可信的 LLM 驱动 RCA 评估至关重要。
...
TL;DR: 本文提出 TOPS：一种无需训练、模型无关的视觉 token 剪枝模块，从信息论第一性原理出发，用任务相关性、信息覆盖与语义多样性三项原则构建最优保留集合，在大幅减少视觉 token 的同时保持甚至提升 MLLM 性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型因视觉 token 数量庞大而计算开销高。现有剪枝方法存在缺陷：基于注意力的方法容易保留冗余 token，基于多样性的方法又常忽略用户指令；混合方法也缺少对剪枝内在目标的原则性建模。因此需要一个更系统、可解释且通用的视觉 token 选择框架。

Method: 作者将视觉 token 剪枝形式化为构建 Token Optimal Preservation Sets，并通过自顶向下的信息论分析提出三项基本原则：Task Relevance、Information Coverage 和 Semantic Diversity。在此基础上设计 TOPS，一个训练自由、模型无关的剪枝模块，可插入不同 MLLM 骨干模型中，在多种剪枝比例下选择更有效的视觉 token 子集。

Result: 在 7 个 MLLM 骨干和 14 个基准测试上，TOPS 在不同剪枝设置下均优于已有方法。尤其在 LLaVA-NeXT 上，TOPS 可移除 77.8% 的视觉 token，同时在 7B 与 13B 模型上分别保持 100.0% 和 100.6% 的性能。

Conclusion: TOPS 表明视觉 token 中存在大量冗余信息，基于任务相关性、信息覆盖和语义多样性的原则化剪枝可显著提升效率且不损害性能；在部分情况下，剪除冗余 token 还能缓解幻觉现象，为轻量化 MLLM 设计提供启发。
...
TL;DR: 论文提出“process harness”机制：在不替换既有确定性工作流引擎的前提下，为遗留流程外加一个受策略约束的智能体层，使其具备推理、适应与监督能力。


<details>
  <summary>Details</summary>
Motivation: 现有企业流程通常依赖确定性工作流引擎，能保证结构化合规与可控执行，但难以处理知识密集型任务、动态决策和运行时流程调整。直接替换遗留引擎成本高、风险大，因此需要一种可渐进升级到 Agentic BPM 的机制。

Method: 作者提出 Task-Decision-Flow（TDF）模型，定义数据模式与执行语义，并将 LLM 推理拆分为三类受策略治理的智能体：TaskAgent 负责知识密集型任务执行，DecisionAgent 负责个案级网关路由，FlowAgent 通过 hook 机制负责运行时流程适配。所有智能体调用均受 process FRAME 中显式策略约束。随后实现 CUGA FLO 作为 TDF 的设计与实现载体。

Result: 论文通过贷款审批工作流演示 CUGA FLO，覆盖三类智能体的作用，并展示基于 hook 的监管覆盖/流程调整能力，说明该机制可在保持原有工作流引擎结构控制权的同时引入智能体能力。

Conclusion: process harness 在确定性流程执行与策略约束下的智能体自治之间建立折中：前者满足强制性、结构性合规要求，后者满足规范性、上下文相关的推理与适应需求，从而为遗留 BPM 系统向 Agentic BPM 演进提供一种低侵入路径。
...
TL;DR: 本文提出 GAversary，一种基于混合遗传算法的黑盒 NLP 对抗样本生成方法，仅依赖目标模型输出的 logit 引导搜索，并利用 GloVe 嵌入生成更语义相近的词替换候选。


<details>
  <summary>Details</summary>
Motivation: 深度学习 NLP 模型容易受到通过少量语义相近词替换构造的对抗输入攻击；现有高精度方法常依赖模型内部信息，而实际场景中攻击者可能只能黑盒访问模型，因此需要一种在低访问假设下仍有效的攻击方法。

Method: GAversary 将目标模型视为黑盒，以模型 logit 输出作为适应度信号，通过遗传算法搜索可误导模型的文本扰动；其关键改进是在变异算子中使用 GloVe 词向量提出替换词，以提升候选扰动与原文本的语义相似性。方法在多个基准数据集和常见目标模型上与 BAE、A2T 等攻击方法进行比较。

Result: 实验显示，GAversary 能比 BAE 和 A2T 更显著降低目标模型测试准确率；最佳情况下可将 76.8% 的准确率降至 5.8%，而 BAE 降至 27.6%。代价是其扰动词数接近另外两种方法的两倍，语义相似度略低，运行时间约增加 5%。

Conclusion: GAversary 证明了基于 GloVe 语义替换与遗传搜索的黑盒攻击在 NLP 模型上具有较强有效性，但其攻击强度与文本自然性、扰动幅度和效率之间存在明显权衡。
...
TL;DR: BINEVAL 将开放式生成评价拆解为原子级二元问题，并聚合为可解释的多维评分，在多个基准上达到或超过强基线。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 输出评估存在瓶颈：人工评估成本高且慢，词汇指标与人类判断相关性弱，整体式 LLM 裁判给出的分数不透明、难以诊断。

Method: 提出任务无关、无需训练的 BINEVAL 框架：根据任务提示由元提示生成细粒度评价问题；LLM 对每个输出独立回答这些二元问题；再将问题级判定聚合为校准后的总体与多维评分，并提供透明反馈用于诊断和提示优化。

Result: 在 SummEval、Topical-Chat 和 QAGS 上，BINEVAL 匹配或优于 UniEval、G-Eval 等强基线；在 QAGS 等事实一致性评测上表现尤其突出，同时更贴近人类评分分布，减少以往 LLM 裁判常见的天花板效应。

Conclusion: BINEVAL 通过可检查、可诊断的二元问题分解，实现了兼具强经验性能、可解释性与实际优化价值的 LLM 输出评估框架，并能支持迭代式提示改进。
...
TL;DR: 本文提出 EO-WM：一种用于多光谱地球观测预测的视频扩散 Transformer，将气象因素作为物理启发的条件信号，以更好建模未来地表变化的不确定性与天气响应。


<details>
  <summary>Details</summary>
Motivation: 现有 EO 预测方法存在两类不足：确定性模型只能给出单一未来结果，难以表达不确定性；扩散方法虽能建模多样性，但通常把天气变量作为普通条件输入，缺少对气候基线、天气异常和持续胁迫效应的区分。同时，现有评测更关注像素重建误差，缺乏检验模型是否能对变化的天气驱动做出正确响应的基准。

Method: 作者提出视频扩散 Transformer 模型 EO-WM，并设计物理启发的气象条件框架：将气象强迫分解为气候态基线、天气异常和随时间累积的物理胁迫信号。模型使用不同条件路径分别编码基线与异常，并累积异常强迫以刻画持续高温和干旱对植被状态的影响。此外，提出两个诊断性基准：Extreme Summer Benchmark 用于评估极端天气下植被退化幅度预测；Seasonal Matched-Pair Benchmark 用于测试改变天气强迫时预测响应是否一致。

Result: 实验显示，EO-WM 在预测 NDVI 下降幅度误差上相对降低 5.63%，方向命中率相对提升 7.80%，同时在常规像素级指标上保持竞争力。

Conclusion: EO-WM 将 EO 预测建模为部分可观测、受天气驱动的世界模型问题，通过更结构化的气象条件建模与专门的天气响应评测，提高了模型对极端和变化天气强迫下地表动态的预测可靠性。
...
TL;DR: 本文研究用基于神经后验估计的仿真推断（SBI）替代传统 MCMC，对机械性 SECIR 流行病学模型进行贝叶斯校准，并在德国 2020 年 COVID-19 ICU 占用数据上验证其效率与准确性。


<details>
  <summary>Details</summary>
Motivation: 机械性流行病学模型常用于传染病预测和公共卫生决策，但高维非线性系统及近实时重复分析中，MCMC 贝叶斯校准计算成本高、速度慢，因此需要更可扩展的推断方法。

Method: 作者采用 simulation-based inference（SBI）中的 neural posterior estimation，对 SECIR 模型进行贝叶斯校准；以 MCMC 为基准，在多个疫情阶段、31 天推断窗口和更复杂的 201 天多传播变化点重建任务中比较两者。评估指标包括 Wasserstein 距离、KL 散度以及后验预测检验。

Result: 在 31 天窗口中，SBI 得到的后验分布与 MCMC 高度一致，并能准确复现 ICU 轨迹；在 201 天任务中，SBI 仍保留主要后验结构但不确定性增加。计算时间方面，31 天问题 MCMC 约需 1000 秒，SBI 在单 GPU 上约 60–70 秒；201 天问题 SBI 平均 157 秒，MCMC 超过 19000 秒。

Conclusion: SBI 能以显著更低的计算成本实现与 MCMC 相近的后验和预测性能，是机械性流行病学模型贝叶斯校准的快速、高效框架，适合近实时重复推断和快速疫情分析。
...
TL;DR: 本文研究自动化简历筛选中的提示注入现象，发现其在少数候选人使用且简历质量差异较小时最有效，但随着使用者增多效果迅速下降，并可能引发公平性问题。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型被用于求职者筛选与排序，候选人可能通过不增加真实资质的自我宣传性文本操纵模型评估。论文旨在系统评估这种提示注入对招聘排序结果的影响及其公平性风险。

Method: 作者在自动化简历筛选场景中开展受控实验，将提示注入定义为不引入新资格、但旨在影响LLM评价的隐性自我推广文本；分别考察候选人质量同质/异质以及注入者比例变化对排名结果的影响。

Result: 在简历质量同质且少数候选人注入时，提示注入能稳定提升排名；当更多候选人采用注入时，其收益快速减弱并在广泛操纵时崩溃。质量异质场景下，平均效果较弱，但偶尔会使低质量候选人超过高质量候选人。

Conclusion: LLM简历筛选在操纵罕见且候选人质量差异较小时最脆弱；提示注入虽然不是普遍有效，但足以带来排序失真和公平性担忧，说明招聘系统需要抗操纵设计与鲁棒评估机制。
...
TL;DR: 本文指出，多模型LLM系统（路由、投票、级联、融合、MoA等）的准确率提升存在由“共同全错率”β决定的上限；若所有候选模型在同一查询上都错，则任何只输出成员模型答案的策略都无法纠正。


<details>
  <summary>Details</summary>
Motivation: 现有研究常用平均成对错误相关性ρ评估模型互补性，但ρ无法刻画所有模型同时失败的尾部风险；因此需要一个能直接约束多模型系统最大可达收益的指标。

Method: 作者从理论上证明任意输出成员模型答案的策略准确率不超过1−β，并说明相同边际错误率与成对相关性可对应不同β；进一步用Clopper-Pearson置信界给出有限样本下β的保守估计，并在67个模型、21个提供方及数学、代码、GPQA-Diamond等任务上进行实证分析。

Result: 实验发现单因子高斯copula模型显著低估共同全错尾部：开放式数学任务中观测β为0.052，而67模型copula估计为0.023，低估约2.5倍；代码任务β为0.079；GPQA-Diamond改为自由回答后β升至0.127。低相关异质集成优于高相关Self-MoA，但在可检验任务中，若缺乏强查询级路由信号，多模型组合通常难以超过单一最佳模型。

Conclusion: 多模型收益主要来自不同模型在不同问题上失败，而不是简单增加模型数量；评估和设计多模型系统时应报告并控制共同全错率β，而不能仅依赖平均成对错误相关性ρ。
...
TL;DR: 论文提出一种基于语言的数字孪生框架，用LLM模拟老年人的对话行为，并用多头cVAE评估其身份保真度与认知一致性。


<details>
  <summary>Details</summary>
Motivation: 个性化医疗中的数字孪生可用于建模个体健康轨迹；在认知健康场景中，轻度认知障碍（MCI）早期检测仍具挑战，而语言和对话模式可作为非侵入式生物标志物，因此需要可扩展、个体化的连续监测方法。

Method: 作者构建基于LLM的语言数字孪生，融合个人文体特征（stylometric cues）和上下文元数据以模拟老年个体的会话行为；同时提出多头条件变分自编码器（cVAE），联合评估文本重构质量并预测认知评分，用于衡量生成对话的保真度和认知一致性。

Result: 在I-CONECT数据集上的实验表明，该数字孪生能保留身份相关语言特征，其重构误差和MoCA预测误差接近真实数据，并优于基线GPT生成回复。

Conclusion: 基于语言的数字孪生具备用于个性化、连续、非侵入式认知健康监测的潜力，尤其可辅助MCI等认知变化的早期识别。
...
TL;DR: 本文研究 ℓ-符号读信道中固定数量 ℓ-mer 删除的纠错码，给出删除效应的结构刻画，并基于校验模式与幂和综合构造低冗余删除纠错码。


<details>
  <summary>Details</summary>
Motivation: ℓ-符号读信道输出发送序列所有连续 ℓ-mer，适用于纳米孔测序、连续读头赛道存储以及相关序列标记问题；实际读向量可能发生若干 ℓ-mer 删除，因此需要面向对抗性删除的高效纠错码。

Method: 作者首先分析在读向量中删除固定数量 ℓ-mer 后，为恢复一致性所需插入的最少 ℓ-mer 结构，证明所得序列等价于在发送序列的特定周期子串中删除符号；当 t≤ℓ−2 时，这些删除对应完整最小周期。随后引入 check patterns，并利用 power-sum syndromes 构造 ℓ-read 删除纠错码，同时针对若干特殊参数组合设计改进构造。

Result: 对任意 ℓ≥2，得到单删除纠错码，冗余为 log⌊(n+2ℓ)/(ℓ−1)⌋。对 2≤t≤ℓ/2，构造 q 元 ℓ-read t 删除纠错码，冗余为 t log n+O(1)。对 ℓ=2t−1 且 t≥3，构造冗余为 (2t−1)log n+O(1) 的码。对特殊参数 (2,2)、(3,2)、(3,3)，进一步得到多种改进结果，包括二元 ℓ-read 2 删除码冗余 2log n+O(1)，非二元 3-read 2 删除码冗余 3log n+O(1)，二元 3-read 3 删除码冗余 5log n+O(1)，非二元 3-read 3 删除码冗余 7log n+O(1)。

Conclusion: 该工作建立了 ℓ-symbol read 信道删除错误与周期子串符号删除之间的关键结构联系，并据此系统构造了多类低冗余删除纠错码，显著推进了重叠读机制下序列纠错编码的理论与构造方法。
...
TL;DR: 本文提出面向非相干 Grassmannian（酉空时）信号的低 PAPR 常模星座约束与相位盲子空间定时误差检测方法，在无需载波相位/偏振恢复和信道估计的条件下实现稳健定时恢复与多分支分集增益。


<details>
  <summary>Details</summary>
Motivation: 既有 Grassmannian 星座研究多假设线性无失真信道/发射机以及已完成符号定时，未充分处理实际系统中的发射端 PAPR/非线性约束和接收端符号定时获取问题。

Method: 在复用现成 Grassmannian packing 的基础上加入常模约束，量化 PAPR 与 chordal distance/编码增益之间的权衡；同时利用 GLRT 投影能量对未知载波相位的不变性，推导相位盲子空间 TED，并设计前馈捕获度量用于无载波/偏振预恢复的时钟恢复。

Result: 常模设计将 0.1% PAPR 从 6.1 dB 降至 3.6 dB，低于 16-QAM 的 5.2 dB，但高 SNR 编码增益约损失 1.8 dB；TED 在滚降系数低至 beta=0.1 时仍有清晰 S 曲线和稳定锁定点；块衰落下可接近 genie-timing SER 并恢复全分集，而未校正 0.35 符号定时偏差会使误码率在约 0.4 附近出现平台。

Conclusion: 该方案同时结合低 PAPR、非相干接收的免相位恢复特性和多分支分集能力，并补足了 Grassmannian 信号在实际发射约束与定时恢复方面的关键缺口，但完整光纤、调制器和相位噪声建模仍待后续验证。
...
TL;DR: 该论文提出一种面向实测辐射方向图的 pinching-antenna（PA）系统分析框架，将实际方向性增益、波导衰减与沿波导顺序功率提取纳入统一建模，并用于优化单/多 PA 的位置与耦合效率。


<details>
  <summary>Details</summary>
Motivation: 现有 PA 系统分析常假设各天线各向同性辐射，并采用简化的耦合效率设置，难以反映 PA 几何结构导致的实际方向图差异，以及多个 PA 沿波导依次提取导波功率所带来的功率分配效应。论文旨在弥补这些建模缺口，使 PA 部署与功率/耦合设计更贴近实际硬件。

Method: 作者构建了一个实测辐射方向图感知的 PA 框架，引入外部测得的 PA 辐射方向图、波导传输衰减和耦合相关的功率提取机制。针对单 PA，推导位置选择规则以平衡方向性增益、波导损耗和自由空间路径损耗，并给出相对固定各向同性天线取得性能优势所需的耦合效率阈值。针对多 PA，研究相位匹配部署及耦合效率设计：均匀耦合情形下得到一维最优性条件；独立可控耦合情形下推导闭式功率分配结构。

Result: 数值结果基于代表性的实测 PA 辐射方向图，表明若忽略实际方向图或顺序功率提取，会导致位置和耦合效率设计偏离最优。均匀耦合时，参与相干合并的相位匹配 PA 数量越多，优选耦合效率越低；独立耦合时，有效方向性信道更强的 PA 应分配更大的辐射功率比例。

Conclusion: 论文证明，在 PA 辅助无线传输中，必须联合考虑实测辐射方向图、波导损耗和耦合效率优化。所提框架比理想各向同性模型更具实践意义，可为单 PA 放置、多 PA 相干合并和可控耦合功率分配提供理论设计准则。
...
TL;DR: 本文研究系统型最优距离局部可修复码（LRC）之间的稳定码转换，在全局拆分场景且 g^I,g^F≤r 的完整参数范围内，刻画了最优读带宽成本。


<details>
  <summary>Details</summary>
Motivation: 分布式存储系统规模、可靠性需求与磁盘故障率会变化，导致纠删码参数需要调整。码转换可将初始编码数据转换为目标编码数据，但关键挑战是如何在保持 LRC 最优距离等性质的同时降低转换过程的读带宽开销。

Method: 论文聚焦系统型最优距离 LRC 的全局拆分转换，针对 g^I,g^F≤r 的参数范围，采用信息论方法推导稳定 LRCC 的读带宽下界，且不假设初始码、最终码或转换过程具有线性性；随后基于具有指定修复性质的 MDS array codes 构造可达下界的 LRCC，并按 g^F=g^I、g^F>g^I、g^F<g^I 三种情况分别设计。

Result: 在三类全局校验节点数量关系下，所提出构造均达到相应的信息论读带宽下界，因此证明这些下界是紧的。

Conclusion: 该工作完整刻画了 g^I,g^F≤r 范围内稳定最优距离 LRCC 在全局拆分场景中的最优读带宽成本，为动态分布式存储系统中的高效 LRC 参数转换提供了理论界限与可实现构造。
...
TL;DR: 论文提出并分析分组反向重要性采样（GRIS），通过对样本成组并设计联合权重函数来降低配分函数估计的均方误差；非重叠与滑动窗口变体在数值实验中均优于普通RIS。


<details>
  <summary>Details</summary>
Motivation: 普通RIS对每个样本单独加权，可能无法充分利用样本间的联合结构。论文旨在寻找可计算且能最小化MSE的分组联合权重函数，以更准确估计Boltzmann分布的配分函数。

Method: 将样本按大小k≥2分组，构造联合权重函数；用归一化MSE与联合权重分布和k重独立能量和分布之间的卡方散度建立关系；分析乘积型权重、仅依赖总能量的group-energy权重，并提出非重叠分组、固定权重滑动窗口和可变权重滑动窗口三类方案。

Result: 理论上证明任何优于普通RIS的权重必须耦合组内分量，乘积型权重会恶化MSE；对非重叠分组而言，在不损失最优性的情况下只需考虑依赖组内总能量的权重。实验中k=2和k=3的非重叠GRIS在三个例子中将MSE降低20%–65%，滑动窗口方案进一步改进，其中VSW优于FSW。

Conclusion: GRIS通过组内联合加权显著提升配分函数估计效率；总能量权重的最优性结论降低了理论与实践复杂度，滑动窗口尤其可变权重方案具有更强性能潜力。
...
TL;DR: 本文提出并分析一种面向随机预编码线性高斯多用户通信系统的 Bayes 最优迭代信号恢复框架。


<details>
  <summary>Details</summary>
Motivation: 已有 OAMP/VAMP 等方法主要处理较简单或可分离先验场景；对于带随机预编码、一般时变/频散信道、多用户且信号分布非可分离的编码系统，如何构造 Bayes 最优的迭代恢复算法仍是开放问题。

Method: 作者建立接收模型 y=Σu HuΞusu+n，假设预编码矩阵来自右酉不变随机矩阵族，提出新的 OAMP/VAMP 型算法框架。该方法可解释为 Minka 的期望传播 EP 与 OAMP 之间的插值，并给出高维有限样本分析；同时使用新的 disorder-averaging 技术，在 replica-symmetric 假设下分析对应推断问题。

Result: 严格的高维算法分析与 RS ansatz 推导得到一致的解耦原理，说明多用户非可分离系统在极限下可分解为等效的标量/较简单推断问题；在 RS 假设成立时，所提算法达到渐近 Bayes 最优。

Conclusion: 论文解决了该类线性高斯多用户随机预编码系统中 Bayes 最优迭代恢复算法构造的开放问题，并提供了算法、有限样本高维理论分析及与统计物理 RS 理论一致的最优性证据。
...
TL;DR: 本文提出语义信息价值（SVoI）度量，用于量化时空相关语义信息在预测未知系统状态时带来的不确定性降低。


<details>
  <summary>Details</summary>
Motivation: 无线网络规模和数据量快速增长导致频谱资源受限；语义通信通过传输面向任务的重要语义信息缓解带宽瓶颈。实际实时无线应用中，信息语义受源内在动态与环境外在动态影响，呈现复杂时空相关性，因此需要可量化其价值的理论指标。

Method: 基于互信息构建SVoI框架，将其定义为利用历史时空相关语义观测预测未知系统状态时的不确定性减少量；针对一般高斯马尔可夫模型推导SVoI闭式表达，并从解析角度研究可分离与耦合时空相关对SVoI的影响。

Result: 推导得到高斯马尔可夫模型下SVoI的闭式形式，并通过数值仿真验证理论分析及其边界；结果表明SVoI能够反映语义时空相关性、信息时效性和信道条件对信息价值的联合影响。

Conclusion: SVoI可作为下一代语义感知通信系统设计中的有效优化目标，为在受限频谱和动态环境下选择、传输与调度高价值语义信息提供理论依据。
...
TL;DR: 本文研究半径为1的有限置换信道的零错误块连接码，提出局部可验证准则、证明其容量意义下的速率完备性，并给出纠错与检错码构造及验证方法。


<details>
  <summary>Details</summary>
Motivation: 全局判定块连接码在所有长度下是否零错误纠错非常困难；已有二进制串连接构造速率距离上界仍有差距。同时，反馈场景下也需要研究相邻交换错误的检错能力。

Method: 提出q元两阶段局部判据：先做同长度截断球测试，再对不等长情形做前缀测试；证明该充分判据在渐近速率上完备；构造产品自动机验证器；并针对检错问题建立配对上界和局部检错准则。

Result: 二进制纠错块连接码达到速率0.649872、0.652018和0.653618，优于先前0.642805，距2/3上界仅0.013049；二进制检错块连接码速率达到0.756707，上界为1/2 log2 3≈0.792481。

Conclusion: 该工作给出了可操作的局部认证框架，既能提升已知构造速率，又在容量层面无渐近损失，并扩展到检错问题，为有限置换/相邻交换信道的零错误编码提供了系统工具。
...
TL;DR: 本文分析加性无记忆信道中确定性与随机化噪声猜测译码的错误指数和复杂度指数，比较失配、匹配、α-倾斜与通用度量的性能。


<details>
  <summary>Details</summary>
Motivation: 噪声猜测译码的性能不仅取决于错误概率，还取决于译码复杂度；在实际场景中译码度量可能与真实信道不匹配，因此需要系统刻画不同度量下的错误—复杂度指数权衡，并寻找无需信道先验的通用最优度量。

Method: 论文在加性无记忆信道模型下，分别研究确定性与随机化噪声猜测译码；先推导失配译码度量下的错误指数和复杂度指数，再将结果特化到匹配度量、α-倾斜度量以及基于噪声序列经验熵的通用度量。

Result: 在确定性译码中，α-倾斜不会改变性能，所有α>0的倾斜度量与匹配度量等价，并在平均错误率和复杂度方面最优；在随机化译码中，匹配度量对复杂度指数并非最优，需根据码率调节α才能同时达到最优错误指数和复杂度指数；经验熵通用度量可在无需信道分布的情况下，在所有码率上同时达到两类最优指数。

Conclusion: 确定性噪声猜测译码对α-倾斜具有鲁棒性，而随机化版本存在码率相关的度量调参需求；基于经验熵的通用译码度量提供了更强的适应性，可统一实现错误与复杂度指数的最优性。
...
TL;DR: 本文研究无间隙、无噪声、非自适应阈值群测试的精确恢复问题，提出一种基于空间耦合测试设计的高效推断算法，达到常数列设计的信息论最小测试数阈值。


<details>
  <summary>Details</summary>
Motivation: 阈值群测试需要从 pooled test outcomes 中精确恢复稀疏二元向量。已有研究表明，在常数列测试设计下该问题存在尖锐的信息论相变，但缺少能够以高效算法达到该阈值、且分析较简洁的方法。

Method: 作者构造空间耦合的非自适应测试设计，并设计相应的高效推断算法。该算法利用空间耦合带来的逐步传播/解码机制，实现高概率精确恢复；分析避免了以往二元群测试算法中复杂加权和的处理。

Result: 在满足特定解析条件的假设下，该算法使用常数列设计所需的最小非自适应测试数量即可高概率实现精确恢复，匹配自然基准设计的信息论阈值，并具有近最优性能保证。

Conclusion: 论文证明空间耦合设计可为无间隙阈值群测试提供高效、近最优且分析更简单的恢复方案，弥合信息论最优性与算法可实现性之间的差距。
...
TL;DR: 本文研究任意标量线性码在合并场景下的码转换成本，提出基于广义汉明重量的通用读写下界，并应用于 Reed-Muller 码构造可转换码。


<details>
  <summary>Details</summary>
Motivation: 分布式存储系统需根据节点失效率动态调整纠删码参数；直接重新编码资源开销大，而现有码转换研究多局限于特定码类，缺乏面向任意线性码的统一理论。

Method: 在合并转换场景中分析任意标量线性码的转换过程，推导以未改变符号和读取符号刻画的写入与读取成本下界；进一步利用广义汉明重量描述子码支撑增长性质，以获得比仅依赖最小距离更精细的界；并通过 Plotkin 分解构造 Reed-Muller 可转换码。

Result: 所得框架可恢复若干重要特例中的已知下界；当终码的广义汉明重量层级存在非平凡跳跃时，新界严格更强。对 Reed-Muller 码的应用表明，在一个自然参数区间内，构造达到写入成本下界；读取成本方面，对一个初始块的广义汉明重量分析是紧的，而另一个块仍存在间隙。

Conclusion: 论文建立了任意线性码合并转换的统一成本下界理论，并证明广义汉明重量是分析码转换读写成本的有效工具；Reed-Muller 构造验证了理论的可达性与局限性。
...
TL;DR: 本文提出并刻画了多分布情形下 Rényi 散度的规范推广：任何满足数据处理单调性与独立乘积可加性的 W 元分布泛函，都可表示为多路 coincidence divergence 的正积分。


<details>
  <summary>Details</summary>
Motivation: 传统 Rényi 散度完整解决了两个概率分布之间的比较问题，但许多现代任务需要同时比较多个分布，如多群体公平性、多先验 PAC-Bayes 界和多假设检验。多分布场景下何为“正确”的 Rényi 型散度族此前尚未明确。

Method: 作者从结构公理出发，要求泛函满足数据处理不增性和独立乘积可加性，证明其可表示为参数空间上多路 coincidence divergences C_α 的正积分。参数空间包含单纯形内部、混合符号指数锥、热带无穷边界以及单纯形顶点处的成对 KL 边。论文还从 Kolmogorov-Nagumo 均值、经典熵刻画、多假设检验误差指数和多彩票投注解释等角度验证该族的自然性。

Result: 论文给出了完整表征定理：所有满足上述两条核心性质的 W 元分布泛函均由多路 coincidence divergences 生成；四类参数分层均是必要的，且每一层都可作为单纯形内部原子的极限得到。二分布情形退化为标准 Rényi 散度结果，并通过 W=3 示例、数值验证和条件版本扩展展示适用性。

Conclusion: 该工作为多分布比较建立了统一的 Rényi 型散度演算，表明 multi-way coincidence divergences 是多分布 Rényi 理论的规范对象，而非某一套公理选择下的偶然产物。
